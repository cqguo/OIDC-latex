...
\maketitle
\begin{abstract}
Single sign-on (SSO) simplifies user authentication by allowing a user to maintain only the credential at an identity provider (IdP) to log in to multiple relying parties (RPs).
However, SSO introduces new privacy threats, such as the curious IdPs potentially tracking a user's visits to various RPs and colluding RPs potentially gaining access to a user's online profile by linking identities across RPs. 
To address these concerns, we propose a privacy-preserving SSO system, called \emph{UPPRESSO}. %, to protect a user's online profile against a curious IdP and colluding RPs.
Our system uses an identity-transformation approach that involves negotiating an ephemeral pseudo-identity for each RP, known as $PID_{RP}$, in each SSO login instance. 
This $PID_{RP}$ is sent to the IdP and designated in the identity token, so the IdP is not aware of the visited RP.
...
The RP receives the token and transforms $PID_U$ into a \emph{permanent account} of the user, denoted as $Acct$. 
  %   by a trapdoor in the negotiation.
Because each account at each RP is unique and different from $ID_U$, colluding RPs cannot link a user's identities across RPs. We have built a prototype of UPPRESSO using MITREid Connect, an open-source SSO system. The extensive evaluations show that UPPRESSO fulfills security and privacy requirements and introduces reasonable overheads.
\end{abstract}
