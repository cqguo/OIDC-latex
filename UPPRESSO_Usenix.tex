...
\usepackage{float}%%%%Ìá¹©ž¡¶¯ÌåµÄ[H]Ñ¡Ïî£¬œø¶øÈ¡Ïûž¡¶¯
\usepackage{caption}%%Ìá¹©\captionofÃüÁî
\pagestyle{plain}
\usepackage{graphicx}
\usepackage{verbatim}
\usepackage{caption}
...
%\author{
%{\rm Chengqian Guo$^{\sharp\nabla}$, \ \ Jingqiang Lin$^{\ddag}$, \ \ Quanwei Cai$^{\P}$, \ \ Fengjun Li$^{\S}$, \ \ Wei Wang$^{\nabla}$,}\\
...
\input{chap_Compact/appendix-new}
