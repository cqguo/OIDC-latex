...
However, SSO introduces new privacy threats such as {\em (a)} the curious IdPs potentially tracking a user's visits to various RPs and {\em (b)} colluding RPs potentially gaining access to a user's online profile by linking identities across RPs. 
...
We propose an identity-transformation approach that involves negotiating an ephemeral pseudo-identity for each RP, known as $PID_{RP}$, in each SSO login instance. 
