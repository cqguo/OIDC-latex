...
% but none of them well addresses both of these privacy leakage threats at the same time.

This paper proposes a privacy-preserving SSO system, called \emph{UPPRESSO},
 to protect a user's login traces against both the curious IdP and collusive RPs.
...
Meanwhile,
    $PID_{RP}$ is also used by the IdP to transform the \emph{permanent user identity} $ID_U$
...
These identity transformations are independent of the authentication between a user and the IdP,
    so UPPRESSO works compatibly with various types of user credentials (e.g., password,
one-time password, multi-factor authentication, and smart card)
as widely-used OpenID Connect (OIDC), OAuth 2.0 and SAML.
...
The extensive evaluation shows that UPPRESSO introduces reasonable overheads
