...
%\author{
%{\rm Chengqian Guo$^{\sharp}$, \ Jingqiang Lin$^{\ddag}$, \ Quanwei Cai$^{\P}$, \ Wei Wang$^{\nabla}$, \ Fengjun Li$^{\S}$,}\\
...
This paper presents a privacy-preserving SSO protocol, called \usso, to protect users' online profiles against (\emph{a}) an honest-but-curious IdP and (\emph{b}) malicious RPs and users who could collude.
