...
This paper presents a privacy-preserving SSO protocol, called \usso, to protect users and their online profiles against either (\emph{a}) an honest-but-curious IdP or (\emph{b}) malicious RPs and malicious users who may collude.
...
\newc
This approach protects the unique identities of the user and the RPs that she requests to log into while supporting SSO services and satisfying all the security requirements.
Compared with existing privacy-preserving SSO protocols, \usso\ is among the first to prevent both types of privacy threats. Compared with existing identity management schemes based on anonymous credentials, \usso\ supports all the desired SSO features while providing sufficient privacy protections.
\oldc
