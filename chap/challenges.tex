...
which prevents the IdP-based access tracing and RP-based identity linkage under the prerequisites that satisfying the basic requirements of SSO systems. 
...
UPRESSO introduces an efficient one-way (trapdoor) function $F_{PID_{U}}(ID_U, PID_{RP})$.
 It allows IdP to  compute $PID_U$ easily,  avoiding the generation of $PID_U$ to be the bottleneck at a high-throughput IdP; 
   and also prevents the RP from finding any information about $ID_U$, which is required by RP-based identity linkage.  
