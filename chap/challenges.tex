...
\begin{itemize}
  ...
  Each $PID_{RP}$ must be globally unique, i.e., only assigned to one RP,  for achieving the \textbf{receiver designation}. 
        ...
        Therefore, we can assume that $PID_{RP}$ is generated through a function $F_{PID_{RP}}(ID_{RP})$.
  \item The \textbf{receiver designation} further requires that  $PID_{U}$ is bound with either a non-null $PID_{RP}$ or $ID_{RP}$ in identity proof.
        When $PID_{RP}$  is non-null, IdP builds the identity proof separately and the \textbf{integrity} is ensured.
...
Then, we analyze the requirements on $PID_{RP}$ and $PID_{U}$     
the design of a secure and privacy-preserving SSO system becomes a dilemma.
