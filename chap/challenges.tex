...
In details, the privacy-preserving SSO system should prevent the curious IdP from obtaining any information that could identify or link the user's accessed RP (e.g., RP's identifier and URL),
...
The above informal analysis demonstrates that the identifers of the user and RP need to be carefully processed in SSO systems.
Here, we systematically analyze the forms of the user and RP identifiers required by privacy protection.
 ...
 Here the privacy-preserving identifers ($PID_{RP}$ and $PID_{U}$) in multiple sessions will never leak the globally unique identifer ($ID_{RP}$ and $ID_{U}$) nor be correlated to link the sessions from an entity (the RP and user).

Then, the design of a secure and privacy-preserving SSO system becomes a dilemma. 
 We demonstrate this problem through an SSO authentication session, as shown in Figure~\ref{fig:IDCorrelation}. 
The IdP who knows $ID_U$ and $PID_{RP}$, generates $PID_{U}$, which is included in the identity proof;
 the  $PID_{U}$ is bound with the non-null $PID_{RP}$ or the $ID_{RP}$ in the identity proof;
  and finally RP checks the binding and calculates $Account$.
The $PID_{U}$ in the identity proof must be generated by the IdP, otherwise the adversary could act as a user to inject any incorrect user's identifier in the identity proof.
The RP's identifier in the identity proof must be checkable to the RP,  otherwise the adversary could act as a RP to misuse the received identity proof to impersonate the user at any RP.
Therefore, we assume that $PID_{U}$ is generated by IdP with the function $F_{PID_{U}}(ID_U, PID_{RP})$;
  $PID_{RP}$ is generated by RP (or with the user) with the function $F_{PID_{RP}}(ID_{RP}$;
  and the $Account$ is generated by the RP with the function $F_{Account}(PID_{U}, ID_{RP}$.

$PID_{U}$ can never be null, otherwise user identification is not satisfied as RP fails to derive a local unique $Account$.
$PID_{RP}$ may be null, then the identity proof should be bound with the $ID_{RP}$ to ensure that RP could perform the check, and the binding should be completed by the user under the grant by the IdP.

For user identification, 
IdP should provide $PID_U$s for the user $ID_U$s with the null $PID_{RP}$ or different $PID_{RP}$s,
 and the  $PID_U$s could be used to derive one $Account$ for a user's multiple logins. 
