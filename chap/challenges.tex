...
Here, we systematically analyze the forms of the user and RP identifiers in each SSO authentication session, 
required by a secure and privacy-preserving SSO system, as shown in Figure~\ref{fig:IDCorrelation}.
For privacy protection, IdP, who knows the user's globally unique identifer ($ID_U$), should only obtain a privacy-preserving identifer for an RP ($PID_{RP}$) (may be null) in a session, and fail to link the $PID_{RP}$s for an RP in multiple sessions;
