...
\label{sec:analysis}
In this section, we first prove the privacy of Recluse, i.e., avoiding the identity linkage of the colluded malicious RPs, and preventing the curious IdP from inferring the user's accessed RPs.
...
The curious IdP fails to derive the user's accessed RPs, as:
\begin{itemize}
  \item It fails to derive RP's identifying information (i.e., $RPID_O$ and correct endpoint) directly. the $RPID_O$ from the $RPID_T$ contained in the identity proof (and request).
  \item It cannot infer the user's accessed RPs indirectly by performing the correlation analysis on  the users' traces.
\end{itemize}
 
The colluded malicious RPs fail to link a user through RPs, as:
\begin{itemize}
