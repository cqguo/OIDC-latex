...
An ideal privacy-preserving SSO authentication session is as follows: %n ideal privacy-preserving
 on receiving the user's login request, the RP calculates $PID_{RP}=F_{PID_{RP}}(ID_{RP})$ to generate the authentication request;
  then IdP computes $PID_{U}=F_{PID_{U}}(ID_U, PID_{RP})$ and uses $PID_{U}$ with $PID_{RP}$ to construct the identity proof;
     finally, the RP obtains $Account$ using $Accout=F_{Accout}(PID_U, ID_{RP})$.
