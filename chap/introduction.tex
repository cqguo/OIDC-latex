...
And, each RP has one unique identifier (denoted as $ID_{RP}$).
%and may transform it to multiple identifiers (denoted as $PID_{RP}$) to  request for identity proof at IdP.
An ideal privacy-preserving SSO authentication session is as follows: %n ideal privacy-preserving
 on receiving the user's login request, the RP calculates a privacy-preserving RP identifier $PID_{RP}$ with the function $PID_{RP}=F_{PID_{RP}}(ID_{RP})$ and generates the authentication request;
  then IdP computes a privacy-preserving user identifier $PID_{U}$ with the function $PID_{U}=F_{PID_{U}}(ID_U, PID_{RP})$ and uses $PID_{U}$ with $PID_{RP}$ to construct the identity proof;
     finally, the RP obtains $Account$ using the function $Accout=F_{Accout}(PID_U, ID_{RP})$.
While the functions $F_{PID_{U}}$, $F_{PID_{RP}}$ and $F_{Accout}$  have to satisfy:
%The three functions $F_{PID_{RP}}$, $F_{PID_{U}}$ and $F_{Accout}$ need to be carefully constructed, to protect the user's privacy and ensure each RP obtains the unchanged $Account$ among the user's multiple logins.
\begin{itemize}
  \item To prevent RP-based identity linkage, $F_{PID_{U}}$ needs to ensure that $PID_{U}$ are different and unlinkable among various RPs.
  \item To prevent IdP-based access tracing, $F_{PID_{RP}}$ needs to ensure that $PID_{RP}$ are different and unlinkable for multiple logins at an RP.
