...
%SSO的现状
%当前主流的SSO协议
Single Sign-On (SSO) systems enable users to access multiple services (called relying parties, RP) with the single authentication performed at the Identity Provider (IdP). To maintain each user's profile and provide individual services, each service provider needs to identify each user. It requires the users  to be authenticated repeatedly when using multiple online services. However, with the SSO system, a user only needs to be authenticated at the IdP which offers verifiable user's attributes,  instead of being identified at each RP. Once a user has been authenticated by the IdP, he/she enjoys the services of the RP after the user has granted its attributes access permission to the corresponding RP.
%with just the simple permission granting.
Moreover, SSO systems bring the convenience in the development and maintainance of RPs as they doesn't need to maintain their own authentication systems, which makes  SSO solutions  widely deployed and integrated. 
%实际数据
The survey on the top 100 websites demonstrates that 24 websites serve as the IdP while 63 websites integrate the SSO service. The leading internet companies such as Google, Facebook and Twitter all provide SSO services.
Various protocol standards, e.g., OAuth 2.0~\cite{rfc6749}, SAML~\cite{SAML} and OpenID Connect~\cite{OpenIDConnect}, are proposed to define the interchanges between the user, RP and IdP.
%第二段：
%引出要讨论的问题
%流行协议的现状
...
Security and privacy are important in SSO systems.
The security of SSO protocols and implementations are systematically analyzed~\cite{WangCW12}~\cite{FettKS16}~\cite{ZhouE14}~\cite{WangZLG16}.
SAML, OAuth and OIDC are formally analyzed based on the web infrastructure model.
The typical SSO solutions, e.g. Google, Facebook and Twitter, has been analyzed and the security has been improved significantly.
...
However, no existing SSO protocols is able to protect users from being tracked and profiled absolutely. 
Current widely adopted SSO protocols (e.g., OpenID Connect) protect the user from a possible correlation among RPs by using Pairwise Pseudonymous Identifier (PPID)~\cite{OpenIDConnect}. 
...
Some work, such as~\cite{ChenPCTKT14} has issued that for security consideration the identity proof offered by IdP should be bound with specific RP and user so that RP's identity has to be provided to IdP.
As a result IdP is always able to know which RP a user has logged in. 
