...
%SSO的现状
%当前主流的SSO协议
Single Sign-On (SSO) systems enable users access multiple services (called relying parties, RP) with the single authentication performed at the Identity Provider (IdP). To remember each user's interests and provide individual services, service providers need to identify each user. It requires that users need to be authenticated repeatedly when using multiple online services. However, with the help of SSO system, a user only needs to be authenticated by an IdP which can offer trustful user identities to RPs instead of being identified by each specific RP. Once a user has already been authenticated by the IdP, he/she can easily use the services of other RPs with just the simple permission granting.
Moreover, SSO systems are also convenient for RPs as they doesn't need to maintain their own authentication systems any more. This is why SSO solutions are widely deployed. 
%实际数据
