...
\begin{itemize}
    ...
    and requires the  consent from the user about the exposed attributes.
     ...
     \item \textbf{User}. Each user 
     initiates an SSO process by requiring to login at an RP,
     relays the identity proof request from RP to IdP,
     completes the authentication at the IdP with securely maintained credentials,
     checks the scope of attributes exposed to RP,
     and transmits the identity proof from IdP to RP correctly.
     Usually, these processes are handled at the user agent (e.g., browser),  a user-controlled software. % (e.g., the ), called user agent.
...
OpenID Connect, a typical SSO standard,  defines the process at each entity and the protocol flows between entities,
...
That is, IdP issues an access token for an RP after the IdP obtains a clear consent from the user;
...
For example, as the access token isn't required to be bound with the RP, the adversary may act as an RP to obtain the victim user's access token and use it to impersonate the victim user in another RP.
...
In the authorization code flow, the identity proof is the authorization code sent by the IdP.
 The authorization code is bound with the RP, as only the target RP who has the corresponding secret assigned by IdP in the RP registration, is able to obtain the user's attributes with this  code. % and the corresponding secret, which is obtained from IdP in the RP's registration. % which is performed in the similar way as the authentication in OAuth 2.0.
\begin{figure}[t]
  \centering
...
\item[3.] \textbf{Confidentiality}. The identity proof should only be obtained by the correct RP (and user), so that no one else may get the identity proof~\cite{ChenPCTKT14,FettKS16,WangZLG16}.
 Otherwise, the adversary may leverage the identity proof to log in as the victim user to the RP specified by the RP identifier in the proof. 
 ...
 transmission to ensure that the identity proof should only be generated valid for the requesting RP and sent to the bound RP's registered endpoint;   
