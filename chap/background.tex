...
The implicit flow of OIDC introduces a new token, known as {\em id token}, as the identity proof,
...
\noindent\textbf{Dynamic Registration.} OIDC provides a dynamic registration mechanism~\cite{DynamicRegistration} for the RPs to register new RPIDs dynamically. % shown in Figure~\ref{fig:OpenID}.
...
After each successful dynamic registration, the RP can obtain a new unique RPID from the IdP.


\subsection{Basic Requirements of a Secure SSO System}
\label{subsec:basicrequirements}

SSO allows users to leverage their existing accounts in the IdP to access services or resources at the RPs. Building such a system is considered challenging~\cite{SPRESSO}. Several design and implementation flaws of SSO systems have been discovered, which led to a plethora of attacks~\cite{SomorovskyMSKJ12,WangCW12,ArmandoCCCPS13,ZhouE14,WangZLLYLG15,WangZLG16,YangLLZH16,MainkaMS16,MohsenS16,MainkaMSW17,YangLCZ18,YangLS17,ShiWL19}. Based on the formal analysis of SAML, OAuth and OIDC~\cite{ArmandoCCCT08,FettKS16, FettKS17} and the study of existing attacks, we summarize a set of basic requirements that a secure SSO platform needs to meet.
...
\vspace{1mm}\noindent\textbf{Integrity.} Only the IdP can generate a valid identity proof. No entity should be able to modify or forge it~\cite{WangZLG16}. Otherwise, an adversary could modify user identifier in the proof to launch impersonation attack or identity injection attacks. In OIDC, for example, the IdP signs identity proofs with its private key to provide integrity~\cite{WangCW12, SomorovskyMSKJ12}.
