...
UPRESSO aims to address both RP-based identity linkage and IdP-based access tracing privacy problems, % at the same time,
 %preserve the users' privacy by hiding the users' traces from both IdPs and RPs in SSO systems,
with the basic requirements of SSO systems under consideration.
In this section, we adopt OIDC as an example to present the authentication flows %the necessary background information
and the basic requirements of SSO systems.
...
\begin{itemize}
    \item \textbf{IdP}. IdP maintains the user's attributes and credentials, performs the user authentication,
    issues the identity proof, binds it with the RP
    and sends the proof  to the correct RP through the user.
    IdP includes the user's identifier (e.g., PPID in OIDC), attributes and the RP's identity in the identity proof.
...
OIDC, a typical SSO standard,  defines both the process at each entity and the protocol flows between entities.
OIDC is  an extension of OAuth 2.0. %to extend OAuth for user authentication.
...
In OAuth, IdP issues an access token for an RP after the IdP obtains a clear consent from the user;
 the RP uses this access token to obtain the specified resources of the user from the resource holder.
However, plenty of RPs adopt OAuth 2.0 in the user authentication, which is not formally defined in the specifications~\cite{rfc6749,rfc6750}, and results in the impersonation attack~\cite{ChenPCTKT14, WangZLG16}.
For example, the access token isn't required to be bound with the RP, the adversary may act as an RP to obtain the access token and use it to impersonate the victim user at another RP.
...
% by binding the identity proof for authentication with the information of RP.
 OIDC provides three SSO  processes: authorization code flow, implicit flow and hybrid flow (i.e., a mix-up of the previous two flows).
In the authorization code flow, the identity proof is the authorization code sent by the IdP.
 The authorization code is bound with the RP, as only the target RP who has the corresponding secret can obtain the user's attributes. % with this  code. % and the corresponding secret, which is obtained from IdP in the RP's registration. % which is performed in the similar way as the authentication in OAuth 2.0.
...
The implicit flow of OIDC introduces a new token, named id token, which works as an identity proof.
...
In details, id token includes the user's PPID, the RP's identity (RPID), the issuer (i.e., IdP), issuing time, the validity period and other requested attributes.
...
An RP obtains a registration token from the IdP, when it has registered at the IdP for the first time. 
With this registration token, the  RP invokes the dynamic registration to
%After a successful initial registration, RP obtains a registration token from the IdP, and 
 updates its information (e.g., the endpoint). % by a dynamic registration process with the  registration token. %which enables the RP to integrate the service provided by IdP,
...
%Based on desiring function and the security analysis of SSO systems,
we conclude the basic requirements of SSO as follows:
%As our work is focused on the implicit flow of OIDC, the following requirements are illustrated based on the implicit flow of OIDC.
%The requirements are shown
\begin{itemize}
\item \textbf{Identification}. %Identification is the main feature of SSO system, which enables the RP to identify the user.
RP needs to identify the user during her multiple login flows,
as RP has to provide the continuous and personalized service based on the user's identity.
Therefore, RP should be able to derive a same identifer from multiple identity proofs for one user.
%the identity proof provided by the IdP must be linkable in the destination RP.
Currently, two types of identifiers are provided in the identity proof: 
(1) the user's unique identifier (e.g., the email address in BrowserID~\cite{BrowserID} and SPRESSO~\cite{SPRESSO}), which introduces the risk of the identity linkage;
and (2) an identifier (e.g., PPID in OIDC), which is different among RPs but remains the same for  one RP.
%That is, some of the existing SSO systems provide the pairwise identifier (e.g., PPID in OIDC) and
%others provide the unchanged identifier (e.g., email in SPRESSO) within the identity proof, all of which are unchanged for one user in one RP.

\item \textbf{Binding}. The identity proof should be bound with only one RP and accepted only by this RP, so that no other honest RP may accept this identity proof.
Otherwise, the adversary may impersonate the victim user,
for example, by pretending as an RP to collect the victim's identity proofs and using them at any other honest RPs~\cite{ChenPCTKT14, WangZLG16}.
...
\item[3.] \textbf{Confidentiality}. The identity proof should only be obtained by the correct RP (and user), no one else may get the identity proof~\cite{ChenPCTKT14,FettKS16,WangZLG16}.
 Otherwise, the adversary may misuse the identity proof to log in as the victim user on the RP specified by the RP identifier in the proof.
 To avoid the unauthorized leakage of identity proof,
 (1) the user and IdP should perform additional checks in its generation and
 transmission to ensure that the identity proof should only be generated valid for the request RP and sent to the binding RP's registered endpoint;
 %is generated for the correct RP (i.e., identifier) and sent to the exact RP (i.e., URL);
