...
\begin{comment}
A RP certificate (denoted as $Cert_{RP}$) is the self-verifying value required in Goal 1, which is  generated in the \textbf{RP initial registration} (phase 1). A transformation (denoted as $RPID_T$) of RP's original identifier (denoted as $RPID_O$) is generated in the \textbf{RP identifier transforming} (phase 2) for Goal 2. The global uniqueness of $RPID_T$ is checked in the \textbf{dynamic registration} (phase 3) for Goal 3. The unlinkable \textbf{PPID} is generated and transmitted to RP in the step 4 and 5 in Figure~\ref{fig:overview}, for Goal 4.
...
\begin{itemize}
  ...
  \item RP identifier transforming: Generating transformation (denoted as $RPID_T$) of RP's original identifier (denoted as $RPID_O$) (Step 2 in Figure~\ref{fig:overview});
  \item Dynamic registration: Checking the global uniqueness of $RPID_T$ (Step 3 in Figure~\ref{fig:overview});
  \item PPID:  Generating PPID at the IdP, and transferring PPID to the RP (Step 4 and 5 in in Figure~\ref{fig:overview});
...
The RP invokes initial registration to apply a valid $Cert_{RP}$ from IdP as provided in Figure~\ref{fig:registration}, which contains three steps:
\begin{itemize}
...
%1. r which is coprime to $P-1$来保证$RPID_O$是一个本原根，从而保证adversary无法从PPID中获取UID, ---
%2. $RPID_O$不能相同，保证RP无法进行identity linkage。 ---RP可以借助类似CT的方案来保证， 没有两个合法证书被颁发给同一个RPID_O
Here, we further explain the two requirements in the generation of $RPID_O$: 
\begin{itemize}
  \item $r$ should be coprime to $P-1$. This makes $RPID_O$ be another primitive root, and satisfies the requirement of Discrete Logarithm problem to prevent the RP  inferring $UID$ from $PPID$.
