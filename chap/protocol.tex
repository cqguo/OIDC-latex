...
\begin{itemize}
  ...
  \item Account: Calculating the user's account in the RP, Step 6 in Figure~\ref{fig:overview}, (Step 6, Goal 5).
\end{itemize}
...
In Recluse, the IdP needs to be initialized during the IdP setup, for generating the public parameters for the users and RPs; the RP has to perform the initial registration at the RP setup; and  the user triggers the Step 2-6 in Figure~\ref{fig:overview} for each login at a RP.
...
\begin{table}[tb]
    ...
    {$P$} & {A large prime.} \\
    \hline
    {$G_{P}$} & {The multiplicative group of $P$ elements.} \\
    \hline
    {$g$} & {A generation of $G_{P}$.} \\
    \hline
