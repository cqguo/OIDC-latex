...
\vspace{1mm}\noindent {\bf Goal 1:} {\em Generating an unforgeable and self-verifiable information for each RP to provide correct information for user-centric checking.}
...
\vspace{1mm}\noindent {\bf Goal 2:} {\em Generating a unique, session-specific and privacy-preserving RP identifier for the IdP to bind the identity proof to it.}
...
%to avoid one identity proof being accepted by two or more RPs. Then, IdP  \emph{binds the identity proof with the unique $PID_{RP}$}, and  can \emph{ never infer $ID_{RP}$ from $PID_{RP}$}, therefore achieving

\vspace{1mm}\noindent {\bf Goal 3:} {\em Generating a unique privacy-preserving user identifier based on the original user identifier and the privacy-preserving RP identifier in a way that a same user account can be derived from multiple privacy-preserving user identifiers for the same user at different RPs.}
...
%the \emph{RP derives the user's unique $Account$  from $PID_U$ with the  trapdoor}, ; while  \emph{RP fails to derive $ID_U$ from either $PID_U$ or $Account$}, and \emph{the $Accounts$ of a user are different among RPs}.
