...
We adopt SHA-256 for digest generation and  RSA-2048 for signature generation. %in  the $Cert_{RP}$, identity token and RP identifier refreshing result.
...
The SDK provides two functions to encapsulate the RP's processing: one for RP identifier transformation, $PID_{RP}$ registration, and identity token request generation; while the other for identity token verification and $Account$ calculation. %in Figure \ref{fig:process}
