...
The RP certificates ensures that the target RP has already registered itself at the IdP,
 which prevent the unauthorized RPs from accessing the IdP's services.
...
For example,
    $H(DN_{RP})$ \cite{irtf-cfrg-hash-to-curve-16} encodes an arbitrary input to a point on an elliptic curve $\mathbb{E}$ as $ID_{RP}$,
where $DN_{RP}$ is an RP's domain (i.e., the RP script's origin),
    and $H()$ is a collision-free hash function.
It does not reveal the discrete logarithm of the output point (i.e., $ID_{RP} = [r]G$ but $r$ is unknown).
Then, the RP script will send the endpoint to the IdP script in Step 2.2,
...
However, once the RP updates its domain, for instance, from \verb+https://www.theRPdomain.com/+ to \verb+https://www.RPdomain.com/+,
