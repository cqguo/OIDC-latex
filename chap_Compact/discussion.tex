...
To build quantum-secure SSO services with the \usso\ protocol,
    an IdP needs to generate a key pair of quantum-resistant public-key algorithms,
        for signing identity tokens and RP certificates.
Moreover, as discussed in Section \ref{sec:related},
    \usso\ leverages the OPRF design \cite{oprf-proved,voprf-proved} in identity transformations of privacy-preserving SSO
        but RP designation requires no collision exists in the blinded inputs of the evaluated pseudo-random function.
So we plan to analyze this property of existing quantum-secure ORPF protocols \cite{ideal-lattice-oprf,isogency-oprf} in the future,
 to find out whether they work for the quantum-secure identity transformations in \usso.
