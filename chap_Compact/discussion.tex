...
The current design of \usso\ does not support quantum security. In our future work, we will investigate alternative approaches for quantum-secure SSO services. First, we consider using quantum-resistant public-key algorithms, which are employed for the IdP to generate a key pair for signing identity tokens and RP certificates. Moreover, we will study existing quantum-secure ORPF protocols \cite{ideal-lattice-oprf,isogency-oprf} to see if they support RP designation. As discussed in Section \ref{sec:related}, such OPRF designs can be utilized to implement the \usso\ protocol.
