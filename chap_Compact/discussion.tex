...
In the authorization code flow of OIDC,
    the IdP does not directly send the identity token;
        instead,
            an access token is sent and forwarded to the RP,
...
The identity-transformation functions $\mathcal{F}_{PID_{U}}$, $\mathcal{F}_{PID_{RP}}$ and $\mathcal{F}_{Account}$
    can be integrated into the authorization code flow to generate identity tokens,
 so that the privacy threats are still prevented in the generation and verification of identity tokens.
However,
...
\vspace{1mm}
\noindent{\textbf{Identity Linkage through Cookies.}}
When a user logins multiple RPs on one device as different accounts,
    the cookies might be exploited by malicious RPs to correlate the user.
For instance,
     when the user has logined $RP_j$ with $Acct_j$,
     malicious $RP_j$ might try to redirect the user's $Acct_j$ to $RP_{j'}$ through a hidden iframe in the browser.
That is, as long as he has logined $RP_{j'}$ with $Acct_{j'}$,
     the user would be correlated.
This attack does not appear only in SSO systems, but also exists in all web applications with accounts.
Anyway,
    this attack can be easily detected through multiple methods,
