...
A targetOringin is specified as a protocol (e.g., \verb+https://+),
    a domain name (e.g., \verb+www.RP.com+), and an implicit or explicit port,
        and it requires the RP script's origin accurately matches the targetOringin.
It implies that only one RP is assumed to run on a domain.
...
\vspace{1mm}\noindent{\textbf{Alternative Ways to Bind $ID_{RP}$ and $Enpt_{RP}$.}}
