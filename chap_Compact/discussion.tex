...
In this paper, we propose the idea, that the privacy-protection problem of SSO can be converted into the identity-transformation challenge. 
This challenge is to design the three identity-transformation functions, $\mathcal{F}_{PID_{RP}}()$, $\mathcal{F}_{PID_U}(ID_U, PID_{RP}) = PID_{U}$, and $\mathcal{F}_{Acct}(PID_{U}, PID_{RP}) = Acct$. 
...
Anyway,
    it can be prevented through various methods,
