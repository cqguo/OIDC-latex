...
\noindent \textbf{Support for the authorization code flow.} In OIDC authorization code flow~\cite{OpenIDConnect},
 ...
 The identity-transformation algorithms, such as $\mathcal{F}_{PID_{U}}$, $\mathcal{F}_{PID_{RP}}$, and $\mathcal{F}_{Acct}$,
  can be integrated into this flow. %similar with the login flow introduced in Section \ref{implementations},
...
This function \cite{irtf-cfrg-hash-to-curve-16} provides collision resistance
 and does not reveal the discrete logarithm of the output,
  ensuring \emph{uniqueness} of $ID_{RP} = [r]G$ while keeping $r$ \emph{unknown}. %For example, using a hash function $Hs()$ to encode an RP's domain or the RP script's origin, e.g., verb+https://RP.com+)
...
The targetOrigin mechanism does not introduce any {\em additional} risk,
 although it does not check the URL path in $Enpt_{RP}$ %, e.g., \verb+/uploadTkn+
...
even if it restricts cookies to specific paths. This risk arises from the SOP design of browsers. %and is not related to the \usso~protocol.
