...
The identity-transformation functions $\mathcal{F}_{PID_{U}}$, $\mathcal{F}_{PID_{RP}}$ and $\mathcal{F}_{Acct}$
    can be integrated into the authorization code flow, similar with the login flow introduced in Section~\ref{implementations},
            while an authorization code is forwarded to the RP script by the IdP script
...
An authorization code is usually the index to retrieve the identity token from the IdP,
        and does not disclose any information about the authenticated user.


After receiving the authorization code, the RP uses it and another secret
         which is issued by the IdP during the initial registration,
     to retrieve the identity token from the IdP by itself \cite{OpenIDConnect}.
...
However,
    if these functions are implemented with browser extensions,
...
Therefore,
    a malicious RP could always access the other's resources within the user browser,
...
For example,
    $Hs()$ encodes an RP's domain (or the RP script's origin) to a point on the elliptic curve $\mathbb{E}$ as $ID_{RP}$,
where
