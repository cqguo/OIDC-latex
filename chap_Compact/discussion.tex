...
The identity-transformation functions, i.e., $\mathcal{F}_{PID_{RP}}()$, $\mathcal{F}_{PID_U}()$, and $\mathcal{F}_{Acct}()$,
...
In this case,
    ...
    before he visits an RP.
After some experiments while the IdP and RPs in the prototype system are unmodified,
    we find that at least 102 ms will be saved for each instance (i.e., about 208 ms in total for a login instance),
    compared with the version implemented with browser scripts.

\vspace{1mm}
\noindent{\textbf{Collusive Attacks by the IdP and RPs.}}
If the IdP is kept curious-but-honest and shares messages (i.e., 
$ID_U$, $PID_{RP}$, $PEnpt_U$, $H(t)$, and $PID_U$) in the login flow
        with some collusive RPs,
        (\emph{a}) UPPRESSO still provides secure SSO services,
    provided that the signed identity tokens are sent to the authenticated users only,
    and (\emph{b}) the users login activities at the other honest RPs,
