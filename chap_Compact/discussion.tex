...
We discuss some related issues in this section.
...
\vspace{1mm}\noindent{\textbf{OIDC authorization code flow support.}} The privacy-preserving functions $\mathcal{F}_{PID_{U}}$, $\mathcal{F}_{PID_{RP}}$ and $\mathcal{F}_{Account}$ can be integrated into OIDC authorization code flow directly, therefore  RP-based identity linkage and IdP-based login tracing are still prevented during the construction and parsing of identity token.
...
In UPPRESSO, IdP does not provide any distinctive information (such as $ID_U$) of each user to RP, which avoids RP-based identity linkage.
However, the cookie of each user may be exploited by RPs to correlate the same user.
For instance, while the user has logged in to $RP_j$ with $Acct_j$, RP may redirect the user's $Acct_j$ to $RP_{j'}$ through the hidden iframe.
That is, as long as the user has logged in to $RP_{j'}$ with $Acct_{j'}$,
 the user would be correlated.
Moreover, this attack is not only appeared in SSO systems, but also existed in all user-account systems.
However, this attack can be easily detected through multiple methods,
 such as checking the iframe in the script, observing redirection flow through browser network tool,
  and detecting the redirection based on the browser extension.
