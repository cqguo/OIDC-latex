...
Since browsers control access to web resources using the same-origin policy (SOP)~\cite{sop},
   an RP's resources in browsers could still be accessed by the other RP maliciously,
...
even if it restricts that only HTTP requests to specific paths carry its cookies.
 This risk arises from the SOP design of browsers. %and is not related to the \usso~protocol.
