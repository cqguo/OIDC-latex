...
Because $\mathbb{E}$ is a finite cyclic group,
    $ID_{RP} = [r]G$ is also a generator and the order of $ID_{RP}$ is $n$.
Therefore, given an RP,
    a unique account is inherently assigned to every user,
        because $Acct =  [ID_U]ID_{RP} = [u]ID_{RP}$ ($1 < u < n$).
So the capacity of accounts at any RP is also $n$.
...
Experiments show that at least 102 ms will be saved for each login instance (i.e., about 208 ms in total),
    compared with the version implemented with web scripts.

\vspace{1mm}
\noindent{\textbf{Collusive Attacks by the IdP and RPs.}}
If the IdP is kept curious-but-honest but shares messages in the login flow (i.e.,
