...
A targetOringin is specified as
    a domain (e.g., \verb+www.RP.com+) and the parts of protocol and port (if not presented, implied 80 for \verb+http+ protocol, and implied 443 for \verb+https+ protocol),
        and it requires the RP script's origin accurately matches the targetOringin.
        Although the path of the endpoint, such as \verb+/path/to/myfile.html+, is not checked by the targetOringin mechanism, which means only one RP is assumed to run on a domain, it would resulted in no additional risk.
For example, if two independent RPs run on one domain but with different endpoints to receive tokens
        (e.g., \verb+https://www.RP.com/RP1/uploadToken+ and \verb+https://www.RP.com/RP2/uploadToken+),
         they cannot be distinguished actually by web scripts and \verb+postMessage+.
         However, for a browser, due to the same-origin policy~\cite{sop}, the origin is also the essential parameter, that prevents the documents and scripts on one origin from accessing the resources on other origins.
         For example, while two services are deployed on the same origin and one of them is controlled by an adversary, the adversary's document and script in a user's browser can upload this user's cookie for the other service. 
        Although the honest service can restrict that only the request to specific path carries its cookie, however, the adversary's script can open the new window loading the honest service's path, and retrieve the cookie using the API \verb+w=window.open("http://www.example.com/honest")+
    and \verb+w.document.cookie+.
...
This binding may be finished in another way:
    $ID_{RP}$ is deterministically calculated based on the RP's website domain (e.g.,
    \verb+https://www.RP.com/+),
        when it is unambiguous and unique.
For example,
    $H(DN_{RP})$ \cite{irtf-cfrg-hash-to-curve-16} encodes an arbitrary input to a point on an elliptic curve. 
Here, $H()$ is a collision-free hash function and does not reveal the discrete logarithm of the output point. $DN_{RP}$ is an RP's domain (i.e., the RP script's origin).
The RP script will send its origin to the IdP script in Step 2.2,
 and then RP certificates are unnecessary.
    This elimination of RP certificates improves the downloading of IdP scripts,
        and on average xxx ms are saved.

Anyway, it still needs some methods to obtain $Enpt_{RP}$;
    for example, an implicit URL (\verb+https://www.RP.com/uploadToken+).

On the other hand, once the RP updates its website domain, for instance, from \verb+https://www.theRPdomain.com/+ to \verb+https://www.RPdomain.com/+,
...
Note that this migration of accounts requires extra cooperations explicitly by each user;
    otherwise,
        two collusive RPs could actually link a user's accounts across these RPs.
        Moreover, $Cert_{RP}$ ensures that the target has already registered itself at the IdP, which prevent the unauthorized RP joining in the UPPRESSO system.
