...
We then calculate the probability that at least two $PID_{RP}$s are equal in these $a$ ones. The probability is $1-\prod_{i=0}^{a-1}(1-i/q)$ which increases with $a$.
...
\vspace{1mm}\noindent{\textbf{Alternative Binding of $ID_{RP}$ and $Enpt_{RP}$.}}
xxxxx
...
Experiments show that at least 102 ms will be saved for each login instance (i.e., about 208 ms in total),
    compared with the version implemented with only browser scripts.
