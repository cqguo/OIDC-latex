...
For instance,
     ...
     malicious $RP_j$ might try to redirect the user's $Acct_j$ to another $RP_{j'}$ through a hidden iframe in the browser.
As long as he has logined to $RP_{j'}$ with $Acct_{j'}$,
     two accounts would be correlated.
This attack does not appear only in SSO systems, but also exists in all web applications with accounts.
Anyway,
    this attack can be easily detected through various methods,
