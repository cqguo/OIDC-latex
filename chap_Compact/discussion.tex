...
A targetOringin is specified as
    a domain (e.g., \verb+www.RP.com+) and the parts of protocol and port (if not presented, implied 80 for \verb+http+ protocol, and implied 443 for \verb+https+ protocol),
        ...
        However, the path of the endpoint, such as \verb+/path/to/myfile.html+, is not checked by the targetOringin mechanism.
So only one RP is assumed to run on a domain.
