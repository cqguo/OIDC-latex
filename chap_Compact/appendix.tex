...
\label{ape:model}
We formally analyze  the security properties of UPPRESSO as follows.
...
Finally, we finish the security proofs of UPPRESSO
        by analyzing the life cycle of identity tokens in this model.
...
Events are the basic communication elements in the model,
    each of which represents a message sent by a process to another process.
The self-triggering event represents the start of the procedure in the web system.
 For example, while a script is loaded by the browser, the browser would generate the self-triggering event to start the script.
\end{itemize}
...
It may contain constants such as ASCII strings and integers,
    sequence symbols such as $k$-ary sequences ($k \geq 0$), %$\langle \rangle$, $\langle . \rangle$, $\langle . ,. \rangle$,
...
\item \textbf{Equational theory}. Equational theory
 uses the symbol $\equiv$ to represent the congruence relation on terms,
  and $\not\equiv$ for the non-congruence relation.
 For example, while there are a message $m$, its signature $sig$, and the corresponding public key $PK$,
  the relation is described as $\mathtt{SigVerify}(m, sig, PK)\equiv \mathtt{TRUE}$.
...
\item \textbf{Retrieve attributes from a formatted term}.
 A formatted term is a term in the specific format, for instance, the HTTP request is the formatted data in the form \myss{\langle\mathtt{HTTPReq}}, \myss{nonce}, \myss{method}, \myss{host}, \myss{path}, \myss{parameters}, \myss{headers}, \myss{body\rangle}.
 We assume there is an HTTP request \myss{r :=}  \myss{\langle\mathtt{HTTPReq}},  \myss{n},  \myss{\mathtt{GET}},  \myss{example.com},  \myss{/path},  \myss{\myangle{}},  \myss{\myangle{}},  \myss{\myangle{}\rangle}, here we define the operation on $r$.
 That is, the elements in $r$ can be accessed in the form \myss{r.name}, such that \myss{r.method \equiv \mathtt{GET}},  \myss{r.path \equiv /path} and \myss{r.body \equiv \myangle{}}.
\item \textbf{Retrieve attributes from a dictionary term}.
A dictionary term is  a term in the form \myss{\myangle{\myangle{type, value}, \myangle{type, value}, \dotsc}}, for instance the \myss{body} in HTTP request is dictionary data. We assume there is a \myss{body := \myangle{\myangle{username, ``Alice''}, \myangle{password, ``123456''}}}, here we define the operation on the $body$. That is, we can access the elements in \myss{body} in the form \myss{body[type]},
 such that \myss{body[username] \equiv ``Alice''} and \myss{body[password] \equiv ``123456''}.
We can also add the new attributes to the dictionary, for example after we set \myss{body[age] := 18}, the \myss{body} are changed into\myss{ \myangle{\myangle{username, ``Alice''}, \myangle{password, ``123456''}, \myangle{age, 18}}}.
\end{itemize}
...
\vspace{1mm}
\noindent\textbf{Relation}. The relation represents the model of procedure, showing how the entity deal with the received message. For example, while the login server receive the login request from a visitor (the request is denoted as the event $e$),
 the input of the relation is $e$ and current state $\langle SessionList,UserList \rangle$.
 The server verifies the visitor's identity with the $UserList$ and add the login result into $SessionList$ (transitioning to $SessionList'$). Thus the output is the login response event $e'$ and the new state $\langle SessionList',UserList \rangle$.
...
A script process does not maintain any state itself,
 while the \verb+scriptstate+ is stored by the browser in each documents.
The \verb+scriptstate+ is transmitted to the script process as the part of input, when a script process is invoked to process an event.
...
$\mathcal{W}$ is the set of atomic processes, including IdP processes, RP processes and browser processes.
...
In the UPPRESSO system, the self-triggering events are triggered after the RP and IdP scripts are loaded. The self-triggering event for RP script is used to create the new window to download IdP script. The self-triggering event for IdP script is used to start the $PID_{RP}$ negotiation.
...
An HTTP request message is the formatted term,
\begin{multline*}
\ \ \ \ \ \ \ \ \langle \mathtt{HTTPReq}, nonce, method, host, path, \\
parameters, headers, body\rangle \ \ \ \ \ \ \ \
\end{multline*}
An HTTP response message is the formatted term,
\begin{equation*}
...
\begin{itemize}
 ...
 \item \myss{path} is the constant string representing the certain resource of the server.
\item \myss{parameters} contains the parameters carried by the url as the formatted term \myss{\myangle{\myangle{type, value}, \myangle{type, value}, \dotsc}}.
For example,  the HTTP request targeting to url \myss{http://www.example.com?type=confirm}  carries the \myss{parameters} as the formatted term \myss{\myangle{\myangle{type, confirm}}}.
 \item \myss{headers} is the header content of each HTTP messages as the term \myss{\myangle{\myangle{type, value}, \myangle{type, value}, \dotsc}}, such as \myss{\langle\myangle{Referer, http://www.example.com},} \myss{\myangle{Cookies, c}\rangle}.
\item \myss{body} is the content carried by HTTP \myss{\mathtt{POST}} request or HTTP response as the formatted term \myss{\myangle{\myangle{type, value}, \myangle{type, value}, \dotsc}}.
  \item \myss{status} is the HTTP status code defined by HTTP standard, such as 200, 302 and 404.
\end{itemize}

\vspace{1mm}\noindent\textbf{URL}.
URL is the formatted term,
$$\myangle{\mathtt{URL}, protocol,host,path,parameters}$$
\begin{itemize}
\item \myss{\mathtt{URL}} is the type.
\item \myss{protocol} is chosen in \myss{\{\mathtt{S}}, \myss{\mathtt{P}\}} as \myss{\mathtt{S}} stands for HTTPS and \myss{\mathtt{P}} stands for HTTP.
\item The \myss{host, path}, and \myss{parameters} are the same as in HTTP messages.
\end{itemize}

\vspace{1mm}\noindent\textbf{Origin}.
An Origin is the formatted term \myss{\myangle{host, protocol}} that can represent the server that a script is downloaded from, where \myss{host} and \myss{protocol} are the same as in URL.

\vspace{1mm}\noindent\textbf{POSTMESSAGE}.
PostMessage is used in the browser for transmitting messages between scripts. The postMessage package is defined as the formatted term $$\myangle{\mathtt{POSTMESSAGE}, target, content, origin}$$
\begin{itemize}
\item \myss{\mathtt{POSTMESSAGE}} is the type.
\item The \myss{target} is the constant nonce which stands for the receiver.
\item \myss{Content} is the message transmitted.
\item \myss{Origin} restricts the receiver.
\end{itemize}

\vspace{1mm}\noindent\textbf{XMLHTTPREQUEST}.
XMLHTTPRequest is the HTTP message processed  by scripts in the browser. That is, the XMLHTTPRequest  package can as the following formatted term can be converted into HTTP request message by the browser.
$$\myangle{\mathtt{XMLHTTPREQUEST}, URL, methods, body, nonce}$$

The XMLHTTPRequest  package as the following formatted term is converted from HTTP response message.
$$\myangle{\mathtt{XMLHTTPREQUEST}, body, nonce}$$
The $\mathtt{XMLHTTPREQUEST}$ is the type. Other attributes are defined similarly as them in HTTP message.
...
The state of IdP process is described as the formatted term,
\begin{multline*}
\langle Issuer, SK, SessionList, UserList, Validity,  TokenList \rangle
\end{multline*}
...
\begin{itemize}
\item \myss{Issuer} is the IdP's identity.
...
\begin{itemize}
\item \myss{\mathtt{CredentialVerify(credential)}} returns the authentication result of a user.
\item \myss{\mathtt{UIDOfUser(credential)}} returns the $ID_U$ of a user.
\item \myss{\mathtt{IsValid(PID_{RP})}} returns the result whether $PID_{RP}$ is the valid point on elliptic curve.
%\item \myss{\mathtt{ListOfPID()}} returns the set of $PID_{RP}$s of the registered RPs in the valid period.
%\item \myss{\mathtt{EnptOfRP(PID_{RP})}} returns the endpoint bound with this $PID_{RP}$ at IdP.
%\item \myss{\mathtt{ModPow(a, b, c)}} is the result of \myss{a^b \mod c}.
\item \myss{\mathtt{Multiply(P, a)}} returns the result of \myss{[a]P}, where $P$ is a point on the elliptic curve and $a$ is an integer.
\item \myss{\mathtt{CurrentTime()}} returns the current system time.
\end{itemize}

The relation of IdP process is shown as Relation~\ref{alg1} in Appendix~\ref{ape:alg}.
...
The state of RP process is the formatted term,
\begin{multline*}
\ \langle ID_{RP}, Enpt, IdP, Cert, SessionList, UserList \rangle \
 \end{multline*}

\begin{itemize}
\item \myss{ID_{RP}} is the  RP's identity assigned by IdP.
\item  \myss{Enpt} is the RP's address for uploading the identity token.
\item \myss{Cert} is the IdP-signed document containing \myss{ID_{RP}, Enpt} and other attributes.
\item \myss{IdP} is the term of the form \myss{\langle ScriptUrl},
    \myss{PK \rangle}, where \myss{ScriptUrl} is the site to download the IdP script,
     and \myss{PK} is the public key used to verify signed messages.
...
\begin{itemize}
 \item \myss{\mathtt{Inverse}(t)} is used to calculate the trapdoor in UPPRESSO system.
  \item \myss{\mathtt{Random}()} returns a fresh random number.
  \item \myss{\mathtt{AddUser}(Acct)} is used to add the new user with \myss{Acct} into RP's user list.
\end{itemize}
The relation of RP process is shown as Relation~\ref{alg2} in Appendix~\ref{ape:alg}.
...
In UPPRESSO system, the message transmitted through a browser is constructing and parsing by the script, so that we only focus on how the script process runs in the browser.
...
The opened windows, documents and scripts are parts of the state of a browser.
...
\vspace{1mm}
\noindent\textbf{Window}. A window is a formatted term,
$$\langle winID, documents, opener \rangle$$
It represents the certain browser window in the system.
\begin{itemize}
\item The $winID$ is the window reference to identify each windows.
...
\item The \myss{opener} represents the document from which this window is created,
    ...
    there is \myss{w.opener \equiv d.docID}.
\end{itemize}
...
\vspace{1mm}
\noindent\textbf{Document}. A document is the web page content in the browser window,
    in the formatted term
\begin{multline*}
  \ \ \ \langle docID, location, referer, script, scriptstate, \\
  scriptinputs, subwindows\rangle \ \ \
\end{multline*}
\begin{itemize}
\item \myss{docID} locates the document.
\item \myss{location} is the URL where the document is loaded.
\item \myss{referer} is same as the Referer header defined in HTTP standard.
\item \myss{script} is the script process downloaded from each servers.
\item \myss{scriptstate} is define by the script, different in each scripts, stored and updated in the document.
\item The \myss{scriptinputs} is the message transmitted into the script process.
\item The \myss{subwindows} is the set of \myss{winID} of windows created from this document.
\end{itemize}
...
The script process is the dependent process relying on the browser,
 which can be modelled as a relation \myss{R} mapping a message input and a message output.
 And finally the browser will conduct the command in the output message.
%A script can be modelled as the tuple containing the input, output and relation.
A script does not have state,
    the state is stored as input and updated as output.
...
\item \myss{docID} is the document's ID, representing the document containing this script.
\item \myss{scriptinputs} is the message transmitted to script. The \myss{scriptinputs} is defined as formatted term, for example, postMessage is one of the forms of \myss{scriptinputs}.
...
The relation of IdP script process $script\_idp$ is shown as Relation~\ref{alg3}, Appendix~\ref{ape:alg}.
 The new \myss{functions} in relation are defined as follows.
\begin{itemize}
 \item \myss{\mathtt{PARENTWINDOW}(tree,docID)}. The first parameter is the input relation tree defined before, and the second parameter is the ID of a document. It returns the current window's opener's $winID$ (null if it doesn't exist nor it is invisible to this document).
  \item \myss{\mathtt{CHOOSEINPUT}(inputs,pattern)}. The first parameter is a set of script inputs, and the second parameter is a pattern. The result returned by the function is the input in \myss{inputs} matching the \myss{pattern}.
%  \item \myss{\mathtt{RandomUrl}()} returns a newly generated host string.
    \item \myss{\mathtt{CredentialofUser}(user,secret)} returns the user's credential for authentication.
\end{itemize}
%Among the functions, only the generation of random url is implemented dependently by the script.
The retrieval of the parent window (the opener) is implemented by the browser, and it can be easily invoked in the script by using the browser-provided API.
The \myss{\mathtt{CHOOSEINPUT}} function is conducted by the browser and transparent to the script developer. 
For example, while the browser receives the HTTP response from the browser, it would decide witch callback function is invoked. 
Moreover, the function \myss{\mathtt{CredentialofUser}} needs the participation on the user, for example, the user inputs her password in the web page. 
...
\ \langle scriptstate, cookies, command \rangle \
\end{multline*}
The state of RP script process \myss{scriptstate} is the formatted term,
$$\langle IdPDomain, RPDomain, Parameters, phase, refXHR\rangle$$
The \myss{RPDomain} is the host string of the corresponding RP server, and other terms are defined in the same way as in IdP script process.
Other terms are same as them in IdP script


The relation of RP script process $script\_rp$ is shown as Relation~\ref{alg4}, Appendix~\ref{ape:alg}.

We define the function \myss{\mathtt{SUBWINDOW}(tree, docID)}, which takes the \myss{tree} defined above and the current document's \myss{docID} as the input. And it selects the $winID$ of the first window opened by this document as the output. However, if there is no opened windows, it returns  null. The retrieval of the sub window is similar with the retrieval of the parent window, the developer can simply achieve the sub window in the script by invoke the browser-provided API.

\subsection{Proofs of Security}
A secure SSO service ensures that only the legitimate user logins to an honest RP under his account.
...
We assume that in the UPPRESSO system, all network messages are transmitted using HTTPS,
 \verb+postMessage+  messages are protected by the browser,
 and the user browser is free of software vulnerabilities.
 % and then web attackers could never break the security of UPPRESSO.
 % 这句话我删去，说不清到底是指什么。否则，就是：已经假设“never break the security of UPPRESSO”，又来证明security。
Next,
...
\begin{lemma}
\textbf{Confidentiality}~In the UPPRESSO system, an identity token issued by the IdP,
 is accessible to only the authenticated user and the target RP, in addition to the IdP.
\label{lemma:confidentiality}
\end{lemma}
\begin{proof}
An identity token is sent to the authenticated user by the IdP and then forwarded to the target RP.
So we only need to prove that an attacker cannot retrieve an identity token from other non-collusive processes,
    if it is not the authenticated user or the target RP.
%\begin{itemize}
%\setlength\itemsep{-2pt}
%\item

The attacker cannot obtain the identity token from an honest RP server.
We check all the messages sent by the RP server at Line 4, 7, 18, 24, 29, 36, 46 in Relation~\ref{alg2}.
The RP server does not send any token to other processes.

%\item

The attacker cannot obtain the identity token from the RP script.
Messages sent by the RP script are classified into two classes. 1) The messages at Line 18, 36, 40 in Relation~\ref{alg4} are sent to the $RPDomain$, while the value of $RPDomain$ is set at Line 4, so that attackers cannot receive these messages. 2) The \verb+postMessage+ message at Line 26 only carries the content received from the RP server,
 and we have proved that the RP server does not send any token.
Therefore, attackers cannot receive the token from the RP script.

%\item

Attacker cannot obtain the identity token from the IdP server.
Among the messages at Line 4, 11, 15, 22, 25, 32, 37, 47 in Relation~\ref{alg1}, only the message at Line 47 carries the token.
This token is generated at Line 45, following the trace of the \myss{Content} at Line 43, the \myss{PID_U} at Line 41, the \myss{ID_U} at Line 40, the \myss{session} at Line 29, and finally the \myss{cookie} at Line 28. That is, the receiver of token must be the owner of the \myss{cookie} in which session that saves the parameter \myss{ID_U}. The \myss{ID_U} is set at Line 14 after verifying the credential and will never be modified.
As we assume that user credential cannot be known to attackers,
attackers cannot obtain the identity token from the IdP server.

%\item

The attacker cannot obtain the identity token from the IdP script.
As the proof provided above, only the IdP sends the token with the message at Line 47 in Relation~\ref{alg1},
and the IdP script only receives the token at Line 62 in Relation~\ref{alg3}.
According to the RP designation, a $PID_{RP}$ is uniquely mapping with an RP (i.e., the $ID_{RP}$).
And the $ID_{RP}$ is uniquely mapping with an $RPDomain$, which is guaranteed by the IdP's signature.
The receiver of token is restricted by the \myss{RPOrigin} at Line 64, while the value of $RPDomain$ is set at Line 63 according to the $Cert$.
Thus, an identity token can be only sent to the targeting RP, while the IdP script can verify the mapping between $PID_{RP}$ and $RPDomain$.


%Here we are going to prove that the token issued for user $u$'s login to RP $r$ (denoted as \myss{t(u,r)}) can only be sent to the targeting RP server through the IdP script. The receiver of \myss{t(u,r)} is restricted by the \myss{RPOrigin} at Line 64, while the value of $RPDomain$ is set at Line 63 according to the $Cert$.
%    The value of \myss{Cert} is set at 19 and the signature is verified at Line 16.
%    If the \myss{Cert} belong to \myss{r}, the attacker cannot obtain the \myss{t(u,r)}.
%    According to the RP designation, a $PID_{RP}$ is uniquely mapping with an RP (i.e., the $ID_{RP}$).
	
 %   Thus, while the 
   % Now we give the proof that the \myss{Cert} belongs to \myss{r}. 
    
    %Firstly we define the negotiated \myss{PID_{RP}} in \myss{t(u,r)} as \myss{p}.
    
%    That is the \myss{PID_{RP}} at Line 69 in Relation~\ref{alg2}  must equal to \myss{p} and the \myss{PID_{RP}} is verified at Line 44 with the \myss{RegistrationToken}.
%This verification cannot be bypassed due to the state check at Line 60.
%At the same validity period, the IdP script needs to send the registration request with same \myss{p}  and receive the successful $PID_{RP}$-registration result.
%As the honest IdP checks the uniqueness of \myss{PID_{RP}} at  Line 31 in Relation~\ref{alg1}.
%The \myss{r} and IdP script must share the same \myss{RegistrationToken}. As the \myss{RegistrationToken} contains \myss{\mathtt{Hash}(t)},
%    the IdP script and \myss{r} must share the same \myss{ID_{RP}}.
%    Therefore, the  \myss{Cert} saved as the IdP scriptstate parameter must belong to \myss{r}.
%Thus, the identity token would only be sent to the script in the corresponding RP's origin.

%\end{itemize}
Therefore, attackers cannot learn any valid identity tokens and this lemma is proved.
\end{proof}
...
\end{lemma}
\begin{proof}
%An RP server finally accepts an token at Line 77 in Relation~\ref{alg2}, after the verification of $PID_U$ at Line 73.
%If such a token is accepted, the $PID_{RP}$ must be equal to the stored one retrieved at Line 69 and verified at Line 44 in Relation~\ref{alg2}.
%The stored $PID_{RP}$ is included in the registration result verified at Line 43 in Relation~\ref{alg2}, received from the IdP server indirectly.
%Thus, the $PID_{RP}$ is only valid at the RP received the corresponding registration result.

%Moreover, this registration result is generated at Line 40 in Relation~\ref{alg1}
%     and protected by the signature generated at Line 41.
%According to the verification at 31 in Relation~\ref{alg1}, the $PID_{RP}$ is unique during the validity period,
%    so that no other registration results containing this $PID_{RP}$ exists.
%Moreover, according to Line 42, 44 (the $t$ used at Line 42 is set  at  Line 15, used for generating $PID_{RP}$ at Line 13),
%    the $PID_{RP}$ must be designated for this RP, as no other RP cannot generate the same $PID_{RP}$ with this $t$.
%So only one RP is able to achieve the corresponding registration result of an $PID_{RP}$.

%Based on the reasons above evidences, (1) the $PID_{RP}$ is only valid at the RP received the corresponding registration result, and (2) only one RP is able to achieve the corresponding registration result of an $PID_{RP}$.
%Thus,
%    the $PID_{RP}$ is only valid at one specific RP,
...
The $Acct$ derived from the identity token by an RP, is generated based on the $PID_U$ in identity proof and $t^{-1}$ assigned at Line 40 in Relation~\ref{alg2}.
According to RP designation, the $t$ is uniquely mapping the targeting RP.
%The integrity of $t^{-1}$ is protected by the IdP issued registration result, according to Line 41-44 in Relation~\ref{alg2}.
The integrity of $PID_{U}$ is guaranteed by the signature contained in the identity token. 
...
We can easily find that the IdP does not send the private key to any processes so that the attackers cannot obtain the private key. Then we only need to prove that the identity token is well signed and verified.
%all the proofs are well verified as below.
%\begin{itemize}
%\setlength\itemsep{-2pt}
...
%At Line 16 the \myss{Cert} is verified with the public key in the scriptstate,
%    where the key is considered initially honest and the key is not modified in Relation~\ref{alg3}.
%Therefore, \myss{Cert} cannot be forged or modified.
%\item \myss{RegistrationResult} is used in Relation~\ref{alg2} from Line 35 to 55, which is verified at Line 30.
%The IdP's public key is initially set in the RP and never modified.
%Therefore, \myss{RegistrationResult} cannot be forged or modified.
%\item 
\myss{Token} is used in Relation~\ref{alg2} from Line 34 to 39 after Line 28 where it is verified.  As proved before, the public key is honestly set and never modified. Therefore, \myss{Token} cannot be forged or modified.
%\end{itemize}
Moreover, an adversary may try to control the $ID_{RP}$ to generate the malicious $PID_{RP}$ and $PID_U$ to break the integrity indirectly, however, the proof of confidentiality has proved it's impossible.
...
\end{lemma}
\begin{proof}
The RP accepts the user's identity at Line 45 in Relation~\ref{alg2}.
And the identity is generated at Line 41, based on the $PID_U$ retrieved from the token and the trapdoor $t^{-1}$.
The $t^{-1}$ is generated at Line 14, set at Line 17, and never changed, as the multiplicative inverse of $t$.
According to Lemma~\ref{lemma:integrity}, only the IdP generates an acceptable identity token,
 so that the token must be issued at Line 45 in Relation~\ref{alg1}.
The IdP generates the $PID_U$ based on the $PID_{RP}$ and user's $ID_U$.
According to Lemma~\ref{lemma:RPdesignation},
     if the $PID_{RP}$ is accepted by an RP, the $PID_{RP}$ must be the generated between the RP and the user.
...
While the honest user has already negotiated the $PID_{RP}$ with the RP, the opener of the IdP script must be the RP script.
    As the $t$ generated at Line 7, Relation~\ref{alg3}, and $PID_{RP}$ generated at Line 21 in Relation~\ref{alg3} and Line 13 in Relation~\ref{alg2}.
The $t$ is only sent to the RP script at Line 8 in Relation~\ref{alg3}, and the RP server at Line 18 in Relation~\ref{alg4}.
The $PID_{RP}$ is only sent to the IdP server at Lines 39 and 53 in Relation~\ref{alg3}.
%Moreover, the $PID_{RP}$-registration result including the $PID_{RP}$ is sent to the IdP script at Line  35 in Relation~\ref{alg1}, the RP script at Line 40 in Relation~\ref{alg3}, RP server at Line 36 in Relation~\ref{alg4}.
An adversary can never know the $PID_{RP}$ negotiated between the honest user and RP.
While the honest user has not negotiated the $PID_{RP}$ with the RP yet, the \verb+session+[$PID_{RP}$] at Line 32, Relation~\ref{alg2} and other similar attributes must be empty, as the $Cookie$ used at Line 21 belongs to the honest user and the negotiation between Line 10-18 does not conducted by the user.
...
The IdP process only accepts the events, downloading scripts, accessing the login status of a cookie owner, authenticating with password, %registering the $PID_{RP}$, 
and requiring the identity token.
...
\begin{breakablealgorithm}
  ...
  \mystate{\myss{m' := \myangle{\mathtt{HTTPResp},n,200,\myangle{},\mathtt{Authenticated}}}}
  \mystop{b, a,m'}
  \ENDIF
...
The RP process only accepts the events, downloading scripts, redirecting to IdP server, negotiating the $PID_{RP}$, 
%uploading the $PID_{RP}$-registration result 
and identity token.
The detailed procedure of dealing with these events is shown as follows.
\begin{breakablealgorithm}
...
%  \mystop{b, a, m'}
 ...
 % \mystate{\myss{PIDValidity := session[PIDValidity]}}
  ...
  \myif{PID_{RP} \not\equiv Content.PID_{RP} \lor Time>Content.Validity}
  \mystate{\myss{m' := \myangle{\mathtt{HTTPResp}, n, 200, \myangle{}, \mathtt{Fail}}}}
  \mystop{b, a, m'}
  \ENDIF
...
(c) the HTTP response for transmitting 
%$PID_{RP}$-registration result and 
identity token.
The detailed procedure of dealing with these events is shown as follows.
 ...
 %     \myif{input \not\equiv \mathtt{null} \land input.Content[RegistrationResult].type \equiv OK}
  ...
  %    \mystate{\myss{s'.phase := expectTokenRquest}}
   ...
   %    \mystate{\myss{s'.Parameters[Enpt_{RP}] := Enpt_{RP}}}
      ...
      \mystate{\myss{input := \mathtt{CHOOSEINPUT}(scriptinputs,pattern) }}
      \myif{input \not\equiv \mathtt{null}}
      \myif{input.Body \equiv \mathtt{Unanthenticated}}
     ...
     \ mystate{\myss{s'.refXHR :=  \mathtt{Random}()}}
      ...
      \myelse{input.Body \equiv \mathtt{Authenticated}}
      \mystate{\myss{PID_{RP} := s'.Parameters[PID_{RP}]}}
     % \mystate{\myss{Enpt := s'.Parameters[Enpt]}}
     % \mystate{\myss{Nonce := s'.Parameters[Nonce]}}
      \mystate{\myss{Url := \langle \mathtt{URL}, \mathtt{S}, IdPDomain, /authorize,} \myss{\myangle{\myangle{PID_{RP}, PID_{RP}}} \rangle}}
      \mystate{\myss{s'.refXHR :=  \mathtt{Random}()}}
      \mystate{\myss{command : = \myangle{\mathtt{XMLHTTPREQUEST}, Url, \mathtt{GET},\myangle{}, s'.refXHR}}}
    ...
    %  \mystate{\myss{Enpt := s'.Parameters[Enpt]}}
     % \mystate{\myss{Nonce := s'.Parameters[Nonce]}}
      ...
      \mystate{\myss{RPOringin := \myangle{s'.Parameters[Cert].Content.Enpt, \mathtt{S}}}}
...
(b) the postMessage from other scripts for $PID_{RP}$ negotiation, 
%posting $PID_{RP}$-registration result 
and identity token;
(c) the HTTP response for downloading the RP $Cert$, 
%retrieving identity token request 
and confirming login result.
The detailed procedure of dealing with these events is shown as follows.
