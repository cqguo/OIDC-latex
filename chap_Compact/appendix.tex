...
\vspace{1mm}
\noindent\noindent\textbf{Operations over terms}. The operations are defined as follows.
\begin{itemize}
...
\item \myss{\mathtt{Multiply(P, a)}} returns the result of \myss{[a]P}, where $P$ is the point on elliptic curve and $a$ is the integer.
...
\begin{itemize}
\item \myss{ID_{RP}} is the iRP's identifier assigned by IdP.
...
\item \myss{IdP} is the term of the form \myss{\langle ScriptUrl}, 
    \myss{PK \rangle}, where \myss{ScriptUrl} is the site to download IdP script, 
...
In this section, we analyze how the browsers interact with other parties in UPPRESSO.
...
\vspace{1mm}
\noindent\textbf{Window}. A window is the formatted term,  
$$\langle ID_w, documents, opener \rangle$$
It represents the concrete browser window in the system.
\begin{itemize}
...
\item The \myss{subwindows} is the set of \myss{ID_w} of document-created windows.
\end{itemize}
...
\begin{multline*}
\langle tree, ID_d, scriptstate, stateinputs, cookies, ids, secret \rangle
\end{multline*}
\begin{itemize}
\item The \myss{tree} is the structure of the opened windows and documents, which are visible to this script.
\item \myss{ID_d} is the document's ID, representing the document containing this script. 
...
\begin{itemize}
 \item \myss{\mathtt{PARENTWINDOW}(tree,ID_d)}. The first parameter is the input relation tree defined before, and the second parameter is the ID of a document. It returns the current window's opener's $ID_w$ (null if it doesn't exist nor it is invisible to this document).
...
The relation of RP script process $script\_rp$ is shown as Relation~\ref{alg4}, Appendix~\ref{ape:alg}. We define the function \myss{\mathtt{SUBWINDOW}(tree, ID_d)}, which takes the \myss{tree} defined above and the current document's \myss{ID_d} as the input. And it selects the \myss{ID_w} of the first window opened by this document as the output. However, if there is no opened windows, it returns  null.
...
\begin{breakablealgorithm}
  ...
  \label{alg3}
  \begin{algorithmic}[1]
  \REQUIRE \myss{\myangle{tree, ID_d, scriptstate, scriptinputs, cookies, ids, secret}}
  \mystate{\myss{ s' := scriptstate}}
  \mystate{\myss{command := \myangle{}}}
  \mystate{\myss{target := \mathtt{PARENTWINDOW}(tree,ID_d)}}
