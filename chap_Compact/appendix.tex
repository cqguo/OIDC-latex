...
\item $\mathcal{S}$ is the set of \emph{script processes}. Besides the atomic processes in the web system,
    ...
    but cannot run independently without other atomic processes.
    These entities are called called script processes, e.g., JavaScript code within a browser.
\item $E^0$ is the set of \emph{events} acceptable to the processes in $\mathcal{W}$.
    An event is the basic communication elements in the model, representing atomic process A sending a message to atomic process B.
\end{itemize}
...
\noindent\textbf{Term}. Terms are the basic elements in Dolev-Yao style models,
     to describe the modelled system.
It contains constants such as ASCII strings and nonce, sequence symbols such as $n$-ary sequences $\langle \rangle$, $\langle . \rangle$, $\langle . ,. \rangle$, and function symbols that model cryptographic primitives such as $\mathtt{SigSign}$, $\mathtt{SigVerify}$ and $\mathtt{Hash}$.
...
So,
an HTTP GET request for the domain {\sf exa.com/path?para=1} with empty header and body is described as $\langle\mathtt{HTTPReq},n,\mathtt{GET},exa.com,/path,\langle \langle para, 1\rangle \rangle ,\langle \rangle,\langle \rangle \rangle$.
