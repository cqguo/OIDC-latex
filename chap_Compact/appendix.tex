...
We check all the messages sent by the RP server at Line 4, 7, 19, 25, 31, 36, 45, 55, 61, 66, 74, 84 in Relation~\ref{alg2}.
The RP server does not send any token to other processes.

%\item

The attacker cannot obtain the identity token from the RP script.
Messages sent by the RP script are classified into two classes. 1) The messages at Line 18, 36, 56 in Relation~\ref{alg4}  are sent to the RPDomain which is set at Line 4, so that attackers cannot receive these messages. 2) The messages at Line 26,  46 only carry the contents received from the RP server,
...
As the proof provided above, only the IdP sends the token with the message at Line 66 in Relation~\ref{alg1},
and the IdP script only receives the token at  Line 99 in Relation~\ref{alg3}.
Here we are going to prove that the token issued for user $u$'s login to RP $r$ (denoted as \myss{t(u,r)}) can only be  sent to the corresponding RP server through the IdP script. The receiver of \myss{t(u,r)} is restricted by the \myss{RPOrigin} at Line 100, which is set at Line 55.
    The host in the \myss{RPOrigin} is verified using the one included in \myss{Cert} at Line 51.
    If the \myss{Cert} belong to \myss{r}, the attacker cannot obtain the \myss{t(u,r)}.
    Now we give the proof that the \myss{Cert} belongs to \myss{r}. Firstly we define the negotiated \myss{PID_{RP}} in \myss{t(u,r)} as \myss{p}.
...
The $Acct$ derived from the identity token by an RP, is generated based on the $t^{-1}$ according to Line 79 in Relation~\ref{alg2}.
...
\end{lemma}
\begin{proof}
The RP accepts the user's identity at Line 83 in Relation~\ref{alg2}.
And the identity is generated at Line 79, based on the $PID_U$  retrieved from the token and the trapdoor $t^{-1}$.
...
According to Lemma~\ref{lemma:integrity}, only the IdP generates an acceptable identity token,
 so that the token must be issued at Line 64 in Relation~\ref{alg1}.
