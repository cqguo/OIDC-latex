...
\item $R^p$ is the \textbf{relation} between the input, state $s \in Z^p$ and an event $e$,
...
\item $s_0^p$ is the initial state.
\end{itemize}
...
\begin{itemize}
\item \textbf{Equational theory}. Equational theory %is defined as usual in Dolev-Yao style models, 这句话没有意义
 uses the symbol $\equiv$ to represent the congruence relation on terms.
 For example, $dec(enc(m, k), k)$ $\equiv$ $m$, where $k$ is a symmetric key.
\item \textbf{Pattern Matching}.  We define the term with the variable $*$ as the pattern, such as \myss{\myangle{a, b, *}}.
...
In this section, we introduce the model of honest UPPRESSO system, in the form of $(\mathcal{W}$, $\mathcal{S}$, $\mathtt{script}$, $E^0)$.
