...
\vspace{1mm}\noindent\textbf{URL}.
A URL is a term \myss{\myangle{\mathtt{URL}, protocol,host,path,parameters}}, where \myss{\mathtt{URL}} is the type, \myss{protocol} is chosen in \myss{\{\mathtt{S}}, \myss{\mathtt{P}\}} as \myss{\mathtt{S}} stands for HTTPS and \myss{\mathtt{P}} stands for HTTP. The \myss{host, path}, and \myss{parameters} are the same as in HTTP messages.
...
\vspace{1mm}\noindent\textbf{Patten Matching}.  We define the term with the variable $*$ as the pattern, such as \myss{\myangle{a, b, \star}}. 
The pattern matches any term which only replaces the $*$ with other terms. For instance,  \myss{\myangle{a, b, \star}} matches \myss{\myangle{a, b, c}}.
...
\item \myss{IdP} is the term of the form \myss{\myangle{ScriptUrl, p, q, PubKey}}, where \myss{ScriptUrl} is the site to download IdP script, \myss{p} and \myss{q} are large primes defined before, and \myss{PubKey} is the public key used to verify the IdP signed messages.
...
\mystate{\myss{m':=\myangle{\mathtt{HTTPResp},n,200, \myangle{}, \mathtt{RPScript}}}}
  ...
  \mystate{\myss{tT:= \mathtt{ExEU}(N_U, s'.IdP.q)}}
...
\end{itemize}
 ...
 \end{itemize}
 The relation of IdP scripting process $script\_idp$ is shown in Algorithm~\ref{alg3}.
...
The state of RP scripting process \myss{scriptstate} is a term in the form \myss{\myangle{IdPDomain, RPDomain, Parameters, q, refXHR}}. The \myss{RPDomain} is the host string of the corresponding RP server, and other terms are defined in the same way as in IdP scripting process.
...
The relation of RP scripting process $script\_rp$ is shown in Algorithm~\ref{alg4}.
\begin{breakablealgorithm}
  \caption{$script\_rp$}
  \label{alg4}
  \begin{algorithmic}[1]
