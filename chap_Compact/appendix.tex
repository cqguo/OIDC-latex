...
\end{lemma}
\begin{proof}
We only need to prove that an attacker cannot retrieve an identity token from other non-collusive processes,
    if it is not the authenticated user or the target RP.
\begin{itemize}
\setlength\itemsep{-2pt}
\item The attacker cannot obtain the identity token from an honest RP server.
We check all the messages sent by the RP server at Line 4, 7, 19, 25, 31, 36, 45, 55, 61, 66, 74, 84 in Relation~\ref{alg2}. It is easy to prove that the RP server does not send any token to other processes.
\item The attacker cannot obtain the identity token from the RP script.
...
\item Attacker cannot obtain the identity token from the IdP server.
...
\item The attacker cannot obtain the identity token from the IdP script.
