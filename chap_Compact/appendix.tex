...
An SSO system is secure,
    if and only if,
    in a honest user's all login instances to an honest RP,
...
\item \emph{\textbf{Integrity}}~An honest RP accepts only identity tokens issued by the IdP,
            and any forged or modified identity token will be detected by the RP.
...
Then, provided that the IdP is honest,
    confidentiality ensures that
            only the authenticated user and the target RP of an acceptable identity token are able to
                present this identity token in some instances, with adversaries sometimes.
                    
User identification enables the honest RP to identify the authenticated user in the verification of an identity token
    and a honest user only presents identity token accessible to himself,
        so that the derived accounts in a honest user's all login instances are identical
            and unique to the accounts derived in other honest users' login instances;
            otherwise, a token does not identify the authenticated user.
            ...
            (\emph{a}) such an identity token will not be accepted by any honest RP,
                ...
                and (\emph{b}) if such an identity token is presented by the malicious user and accepted by an honest RP which is designated in this token,
                        only this malicious user's account will be derived and then unique to others,
                            for user identification ensures only the authenticated is identified.
Therefore,
        in a honest user's some login instance to an honest RP,
