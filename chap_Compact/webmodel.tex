...
\noindent\textbf{Proof. }
While the $PID_{U_{adv}}$ is achieved by the adversary, if must be in the form $PID_{U_{adv}}=$ (a) $ID_{U_{adv}}PID_{RP_{adv}}$; (b) $ID_{U}PID_{RP_{adv}}$; or (c) $ID_{U_{adv}}PID_{RP}$. Due to the $PID_{RP}$ registration, the (a) and (b) cannot be accepted by honest RP. In (c) the $PID_{RP}$ must be created as $N_UID_{RP_{hon}}$, while the hash($N_U$) is included in registration result. Moreover, the trapdoor $T=N_U^{-1} \bmod n$ cannot be controlled be adversary. So, the $PID_{U_{adv}}$ must be transformed into $ID_{U_{adv}}ID_{RP_{hon}}$ (i.e., the $Account$). Therefore, the $Account$ cannot be controlled by the adversary.
...
In the security analysis, we consider web systems $\mathcal{UWS}$ defined in Section~\ref{subsec:webmodel}.
 In this model, we considers only
...
network attacker can control (malicious) browsers and RPs.
