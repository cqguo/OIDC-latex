...
The main overhead  in UPPRESSO is opening the new window and downloading the script from IdP, which needs about 104 ms. This overhead could be reduced by implicitly conducting this procedure when the user visits the RP website.
...
In the requesting, UPPRESSO requires that (1) the user downloads the RP script, opens the IdP window and downloads the IdP script, and (2) the user and RP performs 1 elliptic curve scalar multiplication for RP identifier transformation and complete $PID_{RP}$ registration at the IdP. The total proceeding time is 271 ms (104 ms downloading scripts and open new window ), where
