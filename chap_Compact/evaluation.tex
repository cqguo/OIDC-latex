...
The evaluation was performed on 3 machines,
one (3.4GHz CPU, 8GB RAM, Windows 10) as IdP,
one (3.1GHz CPU, 8GB RAM, Windows 10) as an RP,
and the last one (2.9GHz CPU, 8GB RAM, Windows 10) as a user.
The user agent is Chrome v75.0.3770.100.
These machines are connected in an isolated 1Gbps network.
...
They also adopt
RSA-2048 and SHA-256 as the signature algorithm and the hash function, respectively.
...
The RPs in all three schemes provide the same function, i.e.,   extracting the user's account from the identity token.
We have measured the time for a user's login at an RP and calculated the average values of $1,000$ measurements.


We divide a login instance into 3 phases according to the life-cycle of the identity token:
\textbf{\em Identity token requesting} (Steps 1.1-4.3 in Figure \ref{fig:process}), the RP (and user) constructing and transmitting the request to IdP;
\textbf{\em Identity token generation} (Steps 4.4-4.6 in Figure \ref{fig:process}), the IdP generating identity token (no user authentication);
and \textbf{\em Identity token acceptance} (Steps 4.5-5.2 in Figure \ref{fig:process}), the RP server receives, verifies and parse the identity token relayed from the IdP.
...
The overall SSO login times are  113 ms, 308 ms, and 310 ms for MITREid Connect, SPRESSO, and UPPRESSO, respectively. The details are as follows.
