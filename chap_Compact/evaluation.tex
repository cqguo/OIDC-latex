...
For better analysis, we divide a login into 4 phases according to the lifecycle of identity proof:
 \textbf{Identity proof requesting} (Steps 1.1-3.2 in Figure~\ref{fig:process}), the RP (and user) constructing and transmitting the request to IdP;
 \textbf{Identity proof generation} (Steps 3.3 and 3.4 in Figure~\ref{fig:process}), the IdP generating identity proof (no user authentication);
 \textbf{Identity proof extraction} (Steps 3.4 and 3.5 in Figure~\ref{fig:process}), the RP server extracts the identity proof from the IdP;
 and \textbf{Identity proof verification} (Steps 4.1 and 4.2 in Figure~\ref{fig:process}), the RP verifying and parsing the identity proof.
...
In the requesting, UPPRESSO requires both the user and RP to perform 2  modular exponentiations for RP identifier transformation
and complete $PID_{RP}$ registration at the IdP, which totally need 144 ms;
...
In the identity proof extraction, UPPRESSO only needs 14 ms where the Chrome extension relays the identity proof to the RP server directly.
