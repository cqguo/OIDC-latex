...
%UPPRESSO is compared with MITREid Connect and SPRESSO.
...
The IdP and RP servers are deployed on Alibaba Cloud Elastic Compute Service,
      each of which run Window 10 with 8 vCPUs and 32 GB memory.
The FWD server of SPRESSO runs Ubuntu 20.04.4 with 16 vCPUs and 16 GB memory,
    also on Alibaba Cloud.
We evaluate the schemes in two scenarios,
    when a user browser runs locally (on a PC with Core i7-8700 CPU and 32 GB memory) to remotely visit the IdP and RP,
        and when a browser runs on another virtual machine (with 8 vCPUs and 32 GB memory) also on Alibaba Cloud.
In the latter scenario, all entities are deployed in the same virtual private cloud and connected with each other in one vSwitch,
    which minimizes the influence of network delays.
Note that, in any scenario, the IdP never directly connects to the RP.
...
{\em Identity-token generation} (Step 3 in Figure \ref{fig:process}),
    for the IdP to generate an identity token, while the user authentication and  the authorization of user attributes are not included;
...
We measured the time of a login flow
...
or
 (\emph{b}) 
312 ms, 421 ms, and 510 ms, respectively, when the user browser runs locally to remotely visit the IdP and RP on Alibaba Cloud.

In the part of identity-token requesting, the user browser loads the RP's webpage and starts the login request. The RP of MITREid Connect constructs the identity-token request immediately.
%MITREid Connect only needs 10 ms but
 %   UPPRESSO requires 271 ms.
Compared with MITREid Connect, the main overhead of UPPRESSO is to open the new browser window and download the scripts.
%, which needs about 104 ms.
This overhead may be mitigated %by silently conducting these operations when the user visits the RP website, or 
...
UPPRESSO simply retrieves an identity token from the IdP.
On the contrary, in MITREid Connect when a user retrieves the identity token from IdP, the token must be carried within an URL following the fragment identifier \verb+#+ instead of \verb+?+, due to some security considerations \cite{de2014oauth}.
...
SPRESSO takes more time to generate an identity token,
