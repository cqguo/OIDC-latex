...
For better analysis, we divide a login into 4 phases according to the lifecycle of identity proof:
 ...
 \textbf{Identity proof generation} (Steps 3.3 and 3.4 in Figure~\ref{fig:process}), IdP generating identity proof (no user authentication);
 \textbf{Identity proof extraction} (Steps 3.4 and 3.5 in Figure~\ref{fig:process}), RP server extracts the identity proof from the IdP;
...
In the requesting, UPPRESSO requires the user and RP to perform 2 and 2  modular exponentiations respectively for RP identifier transformation
...
In the evaluation,  forwarder and IdP are deployed in one machine, which doesn't introduce performance degradation based on the observation. % as  FWD and IdP work sequently for one login.
