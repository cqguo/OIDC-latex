...
In the identity tokens of SPRESSO, $ID_{RP}$ is xxxxxx.
...
We then implemented the RPs based on Spring Boot for both UPPRESSO and MITREid Connect, by integrating the corresponding SDK respectively.
The RPs in three schemes provide the same function, i.e.,
     extract the user's account from the verified identity token.
...
\textbf{\em the preparation and identity token requesting} (Steps 1-3 in Figure \ref{fig:process}),
 the RP (and user) constructing and transmitting the request to IdP;
\textbf{\em Identity token generation} (Step 4 in Figure \ref{fig:process}),
    the IdP generating identity token (no user authentication);
and \textbf{\em Identity token acceptance} (Step 5 in Figure \ref{fig:process}),
...
SPRESSO needs 19 ms for the RP to obtain IdP's public key and encrypt its domain (???why???),
...
MITREid Connect requires the IdP to send the identity token to the RP's web page which then sends the token to the RP
 through a JavaScript function, and needs 71 ms (???why???).
SPRESSO needs the longest time (210 ms) due to the complicated processing at the user's browser (???why???).
