...
\noindent {\bf Experiment setup.} We conducted our comparisons in two scenarios: (\emph{a}) a browser, Chrome 104.0.5112.81, ran on a virtual machine with 8 vCPUs and 32 GB memory on Alibaba Cloud, and (\emph{b}) a browser running locally on a PC with Core i7-8700 CPU and 32 GB memory remotely accessed the servers. In the cloud scenario, all entities were deployed in the same virtual private cloud and connected to one vSwitch, which minimized the impact of network delays. In both scenarios, the IdP server never directly communicated with the RPs.
...
In contrast, \usso~incurs overhead\footnote{This overhead could be reduced %by silently conducting these operations when the user visits the RP website, or
