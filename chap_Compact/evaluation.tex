...
For better analysis, we divide a login into 4 phases according to the lifecycle of identity proof:
 ...
 \textbf{Identity proof generation} (Steps 4.4 and 4.5 in Figure~\ref{fig:process}), the IdP generating identity proof (no user authentication);
 \textbf{Identity proof extraction} (Steps 4.5 and 4.6 in Figure~\ref{fig:process}), the RP server extracts the identity proof from the IdP;
...
The overall processing times are  113 ms, 308 ms and 492 ms for MITREid Connect, SPRESSO and UPPRESSO, respectively.
The part in UPPRESSO takes the longest time is opening the new window and downloading the script which totally cost about 255 ms. The time cost can be cut down as the browser can implicitly conduct this procedure while user visit the RP website. 
...
In the requesting, UPPRESSO requires firstly user downloads the RP script, opens the IdP window and downloads the IdP script, then both the user and RP to perform 1  modular exponentiations for RP identifier transformation
and complete $PID_{RP}$ registration at the IdP, which totally need 371 ms (255 ms downloading scripts and open new window ), where
...
In the identity proof extraction, UPPRESSO only needs  about 25 ms where the scripts relay the identity proof to the RP server.
MITREid Connect requires the IdP to send the identity proof to the RP's web page which then sends the proof to the RP server through a JavaScript function, and needs 57 ms.
