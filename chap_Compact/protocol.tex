...
In the original SSO protocols,
    an RP is initially registered at the IdP with an endpoint to receive identity tokens (e.g., redirection URI in OIDC),
        and this endpoint is set in the identity-token response by the IdP, to instruct the user to forward the identity token.
Obviously,
...
\vspace{1mm}
\noindent\textbf{SSO Login.} An SSO login instance is typically launched through a user agent (or browser),
