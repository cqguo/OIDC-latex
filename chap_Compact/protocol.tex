...
%    \hline
    ...
    \hline
    {$PID_{RP}$} & {$PID_{RP} = {ID_{RP}}^{N_{U}} \bmod p$, an RP's pseudo-identifier.} & {One-time variable} \\
    \hline
 ...
 %   {$N_{RP}$} & {An RP-generated nonce for $PID_{RP}$.} & {One-time variable} \\
  %  \hline
   % {$Y_{RP}$} & {$Y_{RP} = {ID_{RP}}^{N_{RP}} \bmod p$, the public value for $N_{RP}$.} & {One-time variable} \\
    %\hline
    {$T$} & {$T=N_U^{-1} \bmod q$, the trapdoor to derive $Account$.} & {One-time variable} \\
    \hline
    \end{tabular}
    \label{tbl:notations}
\end{table}
...
As $q$ is a prime number and $1< N_U < q$, $q$ is coprime to $N_U$. Also, there always exists a $T$ that satisfies $T N_U = 1 \bmod q$. Moreover, from Equations~\ref{equ:PIDRP}, \ref{equ:PIDU} and \ref{equ:Account}, we have $Account = {ID_{RP}}^{ID_U} \bmod p$, derived as below.
\begin{multline}\label{equ:AccountNotChanged}
   Account =  {PID_{U}}^{T} = {({PID_{RP}}^{ID_U})}^{{N_U^{-1} \bmod q}}\\
