...
%\vspace{1mm}
\noindent {\bf The User Account Transformation Function.} We define the trapdoor $T_{RP}$ of each login session as $T_{RP} = N_U^{-1} \bmod n$. As $n$ is a prime number and $1< N_U < n$, $n$ is coprime to $N_U$. So, there always exists a $T_{RP}$ that satisfies $T_{RP} N_U = 1 \bmod n$. Using this trapdoor, the RP can easily derive a unique account (denoted as $A$ in the equation) for the user in each login session as:%with the function $\mathcal{F}_{PID_{U} \mapsto Account}$
\begin{equation}
   \mathcal{F}_{PID_{U} \mapsto Account}: A = {T_{RP}}{PID_U}
   \label{equ:Account}
\end{equation}
...
\item[2.2] The RP verifies $N_{U} \in (1,n)$, calculates $PID_{RP}$ and derives the trapdoor $T_{RP}={N_U}^{-1} \bmod n$. To acknowledge the negotiation of $PID_{RP}$, the RP replies with $Cert_{RP}$, which is transmitted from the RP script to the IdP script through postMessage.
...
\item [5.2] The RP extracts $PID_U$, calculates $Accout = T_{RP}{PID_U}$, and allows the user to log in.
\end{itemize}
...
\item[2.2] The RP  verifies $N_{U} \neq 0 \bmod q$, calculates $PID_{RP}$ with $N_U$, derives the trapdoor $T_{RP}={(N_U N_{RP})}^{-1} \bmod q$; and  acknowledges the negotiation by responding with $Cert_{RP}$. The $Cert_{RP}$ is transmitted from RP script to IdP script through postMessage.
