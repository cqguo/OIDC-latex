...
%    \begin{tabular}{|c|c|c|}
    ...
    \hline
    {$n$} & {the order of the base point $G$.} & {Long-term} \\    \hline
   ...
   {$ID_{RP}$} & {An RP's unique identity,  a point in cyclic subgroup of $E(\mathbb{F}_q)$ generated base on  $P$.} & {Long-term} \\
    ...
    \hline
    {$PID_{RP}$} & {$PID_{RP} = {N_{U}}{ID_{RP}} $, an RP's pseudo-identity.} & {Ephemeral} \\
    \hline
    {$PID_U$} & {$PID_U = {ID_U}{PID_{RP}}$, a user's pseudo-identity.} & {Ephemeral} \\
    \hline
    {$N_U$} & {A user-generated nonce for $PID_{RP}$.} & {Ephemeral} \\
    \hline
    {$T$} & {The trapdoor to derive $Account$: $T=N_U^{-1} \bmod n$.} & {Ephemeral} \\
    \hline
    \end{tabular}
    \label{tbl:notations}
\end{table}

We construct three functions, $\mathcal{F}_{PID_{RP}}$, $\mathcal{F}_{PID_{U}}$ and $\mathcal{F}_{Acct}$, based on the NIST elliptic curve $P$-256,
...
\vspace{1mm}
\noindent {\bf $ID_{RP}$-$PID_{RP}$ Transformation.} In each login session, the user assists the RP to convert $ID_{RP}$ into a pseudo-identity $PID_{RP}$. In particular, the user selects a random number $N_{U}$ ($1 < N_{U}<n$) and calculates $PID_{RP}$ as:
...
%Using $PID_{RP}$ and $\mathcal{F}_{ID_{U} \mapsto PID_{U}}$, the IdP generates a corresponding $PID_U$ and encloses it in the identity token to allow the user to log in to that RP.
...
The RP verifies the identity token, derives the user's unique $Account$, and allows her to log in.
\vspace{-\topsep}
\begin{itemize}
%\setlength{\itemsep}{0pt plus 1pt}
...
\vspace{-\topsep}
\item [5.2] The RP extracts $PID_U$, calculates $Acct = T \cdot {PID_U}$, and allows the user to log in.
\end{itemize}
\vspace{-\topsep}
