...
\item The RP sends a registration request to the IdP,
    including the RP's endpoint (e.g., URL) to receive identity proofs.
...
The design of UPPRESSO can be easily integrated in the traditional SSO systems, %to  prevent the IdP-based login tracing and RP-based identity linkage.
...
The SSO login flow of UPPRESSO is composed of four phases, namely,
        RP identifier transformation, $PID_{RP}$ registration, identity proof generation, and $Account$ calculation.
RP identifier transformation is conducted between a user and the RP, which is specific to UPPRESSO,
...
In UPPRESSO,
    this process is launched by any authenticated user who provides an RP identifer,
      the registration response includes a signature by the IdP,
        and the registration will become invalid after a validity period.
These differences (i.e., the RP identifier not provided by the IdP, the registration response signed by the IdP,
    and the automatically-expired registration) bring very small modifications.
...
$ID_U$ is transformed to $PID_U$ by the IdP, and this modification is actually allowed in OIDC;
    for example, PPID allows various ways to calculate $PID_U$ from $ID_U$.
