...
\begin{itemize}
\item[1.1,2] The user visits the RP's script site and obtains the script.
\item[1.3,4] The script opens a new window in the browser visiting the login path at RP server.
\item[1.5,6] The visit to RP's login path is redirected to IdP's script.
...
\item[3.3,4] The IdP script forwards the registration result to the RP server through RP script.
...
\begin{itemize}
\item[4.1,2] The RP uses $PID_{RP}$ and $Endpoint_{RP}$ to construct an identity proof request for a set of user's attributes, and the request is forwarded to IdP script through RP script.
...
\item[4.7,8] The IdP script forwards the identity proof to RP script with the origin $Endpoint_{RP}$ and RP script sends it to the server.
