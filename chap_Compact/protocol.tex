...
%    \begin{tabular}{|c|c|c|}
    ...
    {$ID_U$} & {$ID_U = u$, $u \in [1, n)$; the user's unique identity at the IdP, unknown to RPs.} \\ \hline
   {$ID_{RP_j}$} & {$ID_{RP} = [r]G$, the $j$-th RP's unique identity; $r \in [1, n)$, unknown to the RP.} \\ \hline
    {$t$} & {A user-generated random integer in a login instance; $t \in [1, n)$, kept secret to the IdP.} \\ \hline
...
After receiving $t$ from and extracting $PID_{RP}$ from the token,
    an RP checks that $PID_{RP} = [t]ID_{RP}$,
        because the correct account is derived only when this equation holds;
        otherwise, a user $U$ might send $t' \neq t$ to login as another account,
            while the calculated account $Acct^* =  [t'^{-1}]PID_{U} = [t'^{-1}t][ID_U]ID_{RP}$
                cannot be purposefully manipulated
        due to the ECDLP.
See Section \ref{sec:discussion} for more discussions about this checking by RPs.
...
This leakage risk may be mitigated %by silently conducting these operations when the user visits the RP website, or
...
The IdP %checks that the received $PID_{RP}$ is a point on $\mathbb{E}$,
...
The IdP rejects an identity-token request, if the received $PID_{RP}$ is not a point on $\mathbb{E}$.
