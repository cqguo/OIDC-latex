...
\begin{multline}
  \ \ \ \ \ \ \ \ \  Acct = \mathcal{F}_{Acct}(PID_{U}, PID_{RP}) 
   \\= [t^{-1} \bmod n]PID_{U} \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 
   \label{equ:Account}
\end{multline}
...
In UPPRESSO such user-agent functions are implemented by web scripts within browsers.
...
After using the extracted $ID_{RP}$ to calculate $PID_{RP}$
...
\begin{figure*}[bt]
  \centering
  \includegraphics[height=0.584\textheight]{fig/process-js.pdf}
...
\item[1.2]
The RP script opens a window in the browser to visit the login path at the RP, which is then \emph{redirected} to the IdP.
\item[1.3]
