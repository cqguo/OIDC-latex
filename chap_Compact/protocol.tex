...
%    {$T$} & {The trapdoor to derive $Account$: $T=N_U^{-1} \bmod n$.} \\ \hline
    ...
    {$PEnpt_{U,j}^i$} & {A user-generated ephemeral random ``pseudo-endpoint'', in the user's $i$-th login instance to the $j$-th RP.} \\ \hline
    \end{tabular}
...
In existing SSO protocols,
    an RP initially registers its endpoint at the IdP,
        and then in each login instance, the IdP will set this endpoint correctly in the identity-token response.
...
\item[3.2] The IdP checks the list of unexpired $PID_{RP}$
    to verify the received $PID_{RP}$ is unique among them.
Then, it signs the response $[PID_{RP}, H(t), Validity]_{SK}$,
...
\begin{itemize}
\vspace{-\topsep}
\item[4.1] The RP constructs an identity-token request with $PID_{RP}$ and $Enpt_{RP}$,
    which is then forwarded to the IdP script through the RP script.
\vspace{-\topsep}
\item[4.2] The IdP authenticates the user if he has not been authenticated yet.
\vspace{-\topsep}
\item[4.3] The user checks the scope of the requested attributes.
The IdP script verifies $PID_{RP}$ and $Enpt_{RP}$ in the request;
    that is, $PID_{RP}$ is the one registered in the previous step,
...
\vspace{-\topsep}
\item [4.4] The IdP verifies if the pair of $PID_{RP}$ and $PEnpt_U$ is in the list of unexpired $PID_{RP}$,
    and calculates $PID_U = [ID_U]{PID_{RP}}$ for the authenticated user.
The IdP signs an identity token $[PID_{RP}, PID_U, Iss, Validity, Attr]_{SK}$,
 where $Iss$ is the IdP's identity, $Validity$ is the validity period, and $Attr$ contains the requested attributes.
\vspace{-\topsep}
\item[4.6] The IdP sends the identity token to $PEnpt_{U}$.
The IdP script forwards this token to the RP script,
    which then sends it to the RP.
\vspace{-\topsep}
\end{itemize}
...
%\setlength{\itemsep}{0pt plus 1pt}
\vspace{-\topsep}
\item[5.1] The RP verifies the identity token, including the IdP's signature and its validity period.
It checks $PID_{RP}$ in the token is consistent with the one negotiated in the previous step.
\vspace{-\topsep}
\item [5.2] The RP extracts $PID_U$, calculates $Acct = [t^{-1}]{PID_U}$, and allows the user to login as $Acct$.
\vspace{-\topsep}
\end{itemize}
...
We explain the compatibility with OIDC as below,
    and this compatibility helps us to analyze and implement UPPRESSO.



It consists of five steps, namely script downloading, RP identity transformation, $PID_{RP}$ registration, identity token generation, and $Acct$ calculation

    
Five steps of the SSO login flow in UPPRESSO,
