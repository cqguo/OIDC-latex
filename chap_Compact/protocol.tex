...
%    {$n$} & {the order of the base point $G$.} \\ \hline
   ...
   {$ID_{RP_j}$} & {$ID_{RP} = [r]G$, $1 \leq r < n$; the $j$-th RP's unique identity.} \\ \hline
    {$t$} & {The user-generated random integer in a login instance, $1 \leq t < n$.} \\ \hline
...
Note that $r$ is kept unknown to RPs; otherwise, two collusive RPs with $ID_{RP_j} = [r]G$ and $ID_{RP_{j'}} = [r']G$ could check whether $[r']Acct_j = [r]Acct_{j'}$ or not,
...
When the IdP script sends messages,
 the receiver's origin is set as a parameter,
  ...
  so that only the script downloaded from this targetOrigin is a legal receiver.
