...
%    {$n$} & {the order of the base point $G$.} \\ \hline
   ...
   {$ID_{RP_j}$} & {$ID_{RP} = [r]G$, $1 < r < n$; the $j$-th RP's unique identity at the IdP.} \\ \hline
    {$t$} & {A user-generated random number in each login instance, $1 < t < n$.} \\ \hline
...
%    {$T$} & {The trapdoor to derive $Account$: $T=N_U^{-1} \bmod n$.} \\ \hline
    ...
    {$PEnpt_{U,j}^i$} & {A user-generated ``pseudo-endpoint'', in the user's $i$-th login instance to the $j$-th RP.} \\ \hline
    \end{tabular}
...
These designs enable UPPRESSO to provide SSO services for web users with standard browsers,
    while more efficient but less portable implementations with browser extensions and/or plug-ins are discussed in Section \ref{sec:discussion}.
...
Two scripts, one downloaded from the IdP and the other from the target RP,
    work together (with the standard  browser functions) as a user agent.
...
The RP script maintains the sessions with the RP.
The IdP script cannot be eliminated or replaced, because an HTTP request launched by the RP script and sent to the IdP
...
The browser downloads the scripts from the IdP and visited RP.
\vspace{-\topsep}
\begin{itemize}
\item[1.1] The user visits the RP's website to download the RP script.
\vspace{-\topsep}
...
The user and RP cooperate to generate $PID_{RP} = [t]{ID_{RP}}$. To hide the RP's endpoint from the IdP,
 the user needs to generate a random endpoint to replace the real RP endpoint.
\vspace{-\topsep}
\begin{itemize}
%\setlength{\itemsep}{0pt plus 1pt}
\item[2.1] The IdP script chooses a random number $t$ ($1 < t <n$) and sends it to the RP script through postMessage.
Then, the RP script sends $t$ to the RP server.
\vspace{-\topsep}
\item[2.2] The RP receives $t$ and calculates $PID_{RP}$.
To acknowledge the negotiation of $PID_{RP}$, the RP replies with $Cert_{RP}$, which is transmitted from the RP script to the IdP script through postMessage.
\vspace{-\topsep}
