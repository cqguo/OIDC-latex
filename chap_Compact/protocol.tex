...
We design three identity-transformation functions,
    ...
    over an elliptic curve $\mathbb{E}$,
     where $G$ is a base point (or generator) of this elliptic curve and the order of $G$ is a prime number denoted as $n$.        %the NIST elliptic curve $P$-256,
...
In the negotiation of $PID_{RP}$,
    the user also sends the trapdoor $t$ to the target RP.
...
Besides,
    if a user visits the IdP and RPs through a local client software,
...
\begin{figure*}[!t]
  \centering
  \includegraphics[height=0.8\textheight,keepaspectratio]{fig/process-js.pdf}
...
To be noticed is that, after a window is opened, the script in this window may change. For example, in this figure, a window is opened at step 1.2, and the script in this window belongs to the RP. However, after the redirection at step 1.3, it visits the IdP server and downloads the script, so that the script inside this window changes into the IdP script. The main difference is that, the IdP script is only trusted by IdP server and allowed to communicate with IdP server. And so does RP script.
...
It then creates a random endpoint $PEnpt_{U}$ for this login instance,
    as the RP endpoint required by IdP.
\vspace{-\topsep}
\end{itemize}
...
\vspace{-\topsep}
\item[3.3] The IdP script forwards the $RegToken$ to the RP through the RP script.
\vspace{-\topsep}
\item[3.4] The RP verifies the IdP's signature, and accepts the $RegToken$
...
\begin{itemize}
\vspace{-\topsep}
\item[4.1] On receiving an identity-token request,
    the IdP authenticates the user if he has not been authenticated yet.
\vspace{-\topsep}
\item[4.2] The user checks the scope of the requested attributes.
...
\vspace{-\topsep}
\item [4.3] The IdP checks whether the received pair of $PID_{RP}$ and $PEnpt_U$ is in the list of unexpired $PID_{RP}$ or not,
...
\vspace{-\topsep}
\item [5.3] The RP returns the result back to user, and allows the user to login as $Acct$.
\vspace{-\topsep}
\end{itemize}
