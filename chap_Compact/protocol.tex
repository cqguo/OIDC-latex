...
We design three identity-transformation functions,
    ...
    over an elliptic curve $\mathbb{E}$ where $G$ is the base point of this curve and $n$ is the order of $G$.        %the NIST elliptic curve $P$-256,
...
First of all,
    ...
    and $ID_{RP}$ is unique point on $\mathbb{E}$.
When a user is registering,
    ...
    when an RP is initially registering at the IdP,
            a random number $r$ $(1 < r < n)$ is generated and $ID_{RP} = [r]G$ is assigned to this RP.
...
\vspace{1mm}
\noindent {\bf $ID_{RP}$-$PID_{RP}$ Transformation.} In each login instance,
    the user selects a random number $t$ ($1 < t <n$) and calculates $PID_{RP}$ as below.
...
\vspace{1mm}
\noindent {\bf ${ID_U}$-$PID_U$ Transformation.}
...
On receiving an identity-token request with $ID_U$ and $PID_{RP}$,
    the IdP calculates $PID_{U}$ as below.
\begin{equation}
...
\vspace{1mm}
\noindent {\bf $PID_U$-$Acct$ Transformation.}
In the negotiation of $PID_{RP}$,
    the user also sends the random number $t$ to the RP.
...
Given a user, the accounts at different are inherently unique.
Moreover,
    due to the elliptic curve discrete logarithm problem (ECDLP),
it is computationally infeasible (\emph{a}) for the IdP to derive $ID_{RP}$ from $PID_{RP}$,
    or (\emph{b}) for the RP to derive $ID_U$ from either $PID_U$ or $Acct$.
...
\subsubsection{The Designs for Web Applications}
With the identity-transformation functions,
 UPRESSO potentially satisfies all the requirements of security and privacy. %desirable for a secure and privacy-preserving SSO service.
But we still need another design, \emph{RP certificate},
     to ensure the confidentiality of identity tokens,
    especially when it is designed to provides SSO services through browsers.
In the original SSO protocols,
    an RP is initially registered at the IdP with an endpoint to receive identity tokens (e.g., redirection URI in OIDC).
This endpoint is then set in the identity-token response by the IdP, to instruct the browser to forward the identity token,
    for a browser does not pre-configure the endpoint for each RP.
Obviously,
    an incorrect RP endpoint leaks identity tokens.

In the UPPRESSO protocols,
    because the IdP is not aware of the visited RP,
        the user has to deal with RP endpoints by himself.
An RP certificate is a message signed by the IdP, binding the RP's identity and its endpoint.
This message is sent by the RP in the login instance,
    so that the user is able to forward the tokens to this verified endpoint.

++++++
special
Design for browser!

Script downloading and postMessage are necessary
...
++++++
Because something need to be done, so we need script.
In the OIDC, nothing to do;
only an redirection is enough [this is a standard browser function].

Why two scripts? [Due to the same original policy], different script to different server.

Why postMessage? in user agent
How to remember the IdP's public key?
Why need "PIDRPDynamic Registration." This is for OIDC compatibility.




We present the detailed protocols as below,
    Table \ref{tbl:notations-protocol} lists the notations used in these protocols.
...
%    \begin{tabular}{|c|c|c|}
    ...
    {$\mathbb{E}$} & {An elliptic curve, where the ECDLP is computationally infeasible.} \\ \hline
    {$G$, $n$}&{$G$ is a base point of $\mathbb{E}$, and the order of $G$ is $n$.} \\ \hline
%    {$q$} & {A large prime, the size of the underlying field.} \\ \hline
%    {$n$} & {the order of the base point $G$.} \\ \hline
    {$ID_U$} & {$ID_U = u$, $1 < u < n$; a user's unique identity at the IdP.} \\ \hline
   {$ID_{RP}$} & {$ID_{RP} = [r]G$, $1 < r < n$; an RP's unique identity at the IdP.} \\ \hline
    {$t$} & {A user-generated random number for each login instance, $1 < t < n$.} \\ \hline
    {$PID_U$} & {$PID_U = [{ID_U}]{PID_{RP}} = [utr]G$; a user's pseudo-identity, in some login instance.} \\ \hline
    {$PID_{RP}$} & {$PID_{RP} = [t]{ID_{RP}} = [tr]G$; an RP's pseudo-identity, in some login instance.} \\ \hline
     {$Acct$} & {$Acct = [t^{-1}]PID_{U} = [ID_U]ID_{RP} = [ur]G$; a user's identity at the RP.} \\ \hline
    {$SK$, $PK$} & {The IdP's key pair, a private key and a public key, to sign and verify identity tokens and RP certificates.} \\ \hline
    {$Cert_{RP}$} & {An RP certificate, a message signed by the IdP, binding the RP's identity and endpoint.} \\ \hline
...
\noindent\textbf{RP Initial Registration.} Each RP launches
    an initial registration process once to finish configurations.
In particular, an RP registers itself at the IdP to obtain its unique identifier $ID_{RP}$
 and the corresponding RP certificate $Cert_{RP}$ as follows:
\begin{enumerate}
\item
The RP sends a registration request to the IdP, including the RP endpoint (e.g., URL) to receive identity tokens,
    and other optional information.
\item
The IdP generates a unique random number $r$,
    and $ID_{RP} = [r]G$ is assigned to this RP.
The IdP then signs $Cert_{RP} = [ID_{RP}, Endpoint_{RP}, *]_{SK}$,
...
UPPRESSO adopts a similar user registration process as the ones in other SSO systems.
Each user registers once at the IdP to set up a unique user identifier $ID_U$ and the corresponding user credential.
...
It consists of five phases, namely script downloading, RP identity transformation, $PID_{RP}$ registration, identity token generation, and $Acct$ calculation, as shown in Figure \ref{fig:process}.


++++++

\noindent 1. {\em Script Downloading.}
The script downloading phase is for the user's browser
 to download the scripts from the IdP and RPs.
The browser and two scripts work together as the user agent.
\vspace{-\topsep}
\begin{itemize}
...
The user and RP cooperate to generate $PID_{RP} = {N_{U}}{ID_{RP}}$. To hide the RP's endpoint from the IdP,
 the user needs to generate a random endpoint to replace the real RP endpoint.
\vspace{-\topsep}
\begin{itemize}
...
Do you mean the public key is in the IdP script?
\vspace{-\topsep}
\item[2.3] The IdP script verifies $Cert_{RP}$, extracts $ID_{RP}$ from $Cert_{RP}$ and calculates $PID_{RP}={N_{U}}{ID_{RP}}$ and $nonce=H(N_U)$. It also creates a one-time endpoint for the RP. If $Cert_{RP}$ is invalid, the user halts the negotiation.
\end{itemize}
\vspace{-\topsep}
...
\noindent{\em 3. $\mathbf{PID_{RP}}$ Dynamic Registration.}
...
\begin{itemize}
%\setlength{\itemsep}{0pt plus 1pt}
\item[3.1] The IdP script sends the $PID_{RP}$ registration request $[PID_{RP}, H(N_U), Endpoint_U]$ to the IdP.
\vspace{-\topsep}
\item[3.2] The IdP checks the unexpired $PID_{RP}$s to verify if the received $PID_{RP}$ is unique. Then, it signs the response as $[PID_{RP}, H(N_U), validity]_{SK}$, where $validity$ denotes when the $PID_{RP}$ will expire.
\vspace{-\topsep}
\item[3.3] The IdP script forwards the registration result to the RP server through the RP script.
\vspace{-\topsep}
\item[3.4] The RP verifies the IdP's signature and accepts the result only if $PID_{RP}$ and $H(N_U)$ match those in the negotiation and $PID_{RP}$ is not expired.
\end{itemize}
\vspace{-\topsep}
$H(N_U)$ is a nonce to distinguish different login sessions because there is a very small chance that the same $PID_{RP}$ is generated for two RPs even using two different pairs of $ID_{RP}$s and $N_U$s. This nonce avoids a registration result being acceptable to two RPs.

\noindent{\em 4. Identity Token Generation.}
In this phase, the IdP calculates $PID_U = {ID_U}{PID_{RP}}$ and signs the identity token. % The processes are as follows.
\vspace{-\topsep}
\begin{itemize}
%\setlength{\itemsep}{0pt plus 1pt}
\item[4.1] The RP constructs an identity token request containing its $PID_{RP}$ and $Endpoint_{RP}$, which is then forwarded to the IdP script through the RP script.
\vspace{-\topsep}
\item[4.2] The IdP authenticates the user if she has not been authenticated yet.
\vspace{-\topsep}
\item[4.3] First, the user checks the scope of the requested attributes, while the IdP script verifies that $PID_{RP}$ and $Endpoint_{RP}$ in the request are valid. Then, the IdP script replaces the RP's endpoint with the newly registered one-time $Endpoint_U$ and sends the modified identity token request to the IdP server.
\vspace{-\topsep}
...
\vspace{-\topsep}
\item[4.5] The IdP constructs and signs the identity token $[PID_{RP}, PID_U, Iss, ValTime, Attr]_{SK}$, where $Iss$ is the identifier of the IdP, $ValTime$ is the validity period, and $Attr$ contains the requested attributes.
\vspace{-\topsep}
\item[4.6] The IdP sends the identity token to the one-time endpoint. The IdP script forwards the identity token to the RP script that holds the origin $Endpoint_{RP}$. Finally, the RP script sends it to the RP server.
\end{itemize}
\vspace{-\topsep}
In this phase, if any check fails, the process will be halted. For example, the user halts the process if $PID_{RP}$ in the identity token request is inconsistent with  the negotiated one. The IdP rejects the identity token request, if the pair of $PID_{RP}$ and $Endpoint_U$ has not been registered.

\noindent{\em 5. $Acct$ Calculation.}
The RP verifies the identity token, derives the user's unique $Account$, and allows her to login.
\vspace{-\topsep}
\begin{itemize}
%\setlength{\itemsep}{0pt plus 1pt}
\item[5.1] The RP verifies the identity token, including the signature, validity period, and if $PID_{RP}$ is consistent with the negotiated one. If any fails, the RP rejects this login.
\vspace{-\topsep}
\item [5.2] The RP extracts $PID_U$, calculates $Acct = T \cdot {PID_U}$, and allows the user to login.
\end{itemize}
\vspace{-\topsep}
...
In this phase, the user login continues and the IdP signs the identity token. % The processes are as follows.
\begin{itemize}
...
\item[4.2] The IdP authenticates the user if she has not been authenticated yet.
 \item[4.3] The user first confirms the scope of the requested attributes. IdP script verifies the $PID_{RP}$ with the negotiated one and $Endpoint_{RP} \in Cert_{RP}$, replaces the endpoint with the registered one-time $Endpoint_U$ and then sends the modified identity token request to the IdP server.
...
\subsubsection{Compatibility with OIDC}
\label{subsec:compatible}
UPPRESSO does not introduce any new role (or component) nor change the security assumptions for each role. It follows a similar logic flow as OIDC in SSO login and only requires small modifications to perform identifier transformation.
...
%The {\em identity token generation} and {\em $Account$ calculation} phases adopt the same steps and functions as the implicit protocol flow of OIDC, while using a few different parameters. First, in identity token generation, $PID_U$ transformed from $ID_U$ is used to replace $ID_U$, which is directly supported by OIDC, similar as in the PPID approaches that also convert $ID_U$ into $PID_U$. The calculation of $Account$ from $PID_U$
%can be viewed as a customized step by the RP to derive its user account after the implicit protocol flow of OIDC ends.

%So,the identity token generation and $Account$ calculation phases of UPPRESSO can be viewed as a particular but compatible implementation of the implicit protocol flow of OIDC. It is worth noting that the identity token generation and $Account$ calculation phases of
