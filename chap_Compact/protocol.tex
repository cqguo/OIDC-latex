...
%    {$T$} & {The trapdoor to derive $Account$: $T=N_U^{-1} \bmod n$.} \\ \hline
    ...
    {$PEnpt_{U,j}^i$} & {A user-generated ephemeral ``pseudo-endpoint'', in the user's $i$-th login instance to the $j$-th RP.} \\ \hline
    \end{tabular}
...
because the it could provide the trusted base for cryptographic calculations, such as generating $PID_{RP}$, and verifying $Cert_{RP}$.
...
The RP script opens a window in the browser to visit the login path at the RP, which is redirected to the IdP.
\vspace{-\topsep}\item[1.3]
The redirection to the IdP opens another window and downloads the IdP script.
...
\vspace{-\topsep}\item[2.3] The IdP script verifies $Cert_{RP}$, extracts $ID_{RP}$ from $Cert_{RP}$ and calculates $PID_{RP}=[t]{ID_{RP}}$.
It also creates an ephemeral endpoint $PEnpt_{U}$ for this login instance.
\vspace{-\topsep}
\end{itemize}
...
%\setlength{\itemsep}{0pt plus 1pt}
\vspace{-\topsep}
\item[3.1] The IdP script sends the $PID_{RP}$ registration request $[PID_{RP}, PEnpt_U]$ to the IdP,
    along with $H(t)$ as nonce where $H()$ is a one-way hash function.
++++++
\vspace{-\topsep}
...
\vspace{-\topsep}
\item[4.3] First, the user checks the scope of the requested attributes, while the IdP script verifies that $PID_{RP}$ and $Endpoint_{RP}$ in the request are valid. Then, the IdP script replaces the RP's endpoint with the newly registered one-time $Endpoint_U$ and sends the modified identity token request to the IdP.
\vspace{-\topsep}
...
\vspace{-\topsep}
\item[4.6] The IdP sends the identity token to the one-time endpoint. The IdP script forwards the identity token to the RP script that holds the origin $Endpoint_{RP}$. Finally, the RP script sends it to the RP.
\end{itemize}
\vspace{-\topsep}
...
If $Cert_{RP}$ is invalid, the user halts the negotiation.
