...
%    \begin{tabular}{|c|c|c|}
   ...
   {$ID_{RP_j}$} & {$ID_{RP} = [r]G$, is the $j$-th RP's unique identity, publicly known; $r \in [1, n)$, \textcolor{blue}{is known to only the IdP.}} \\ \hline
...
The user and the RP need to calculate $PID_{RP}$ independently; or, more exactly,
 one computes $PID_{RP} = [t]ID_{RP}$, and the other checks if $PID_{RP}$ is equal to $[t]ID_{RP}$ or not.
...
\begin{figure*}[htb]
  \centering
  \includegraphics[height=0.585\textheight]{fig/process-js.pdf}
...
Or, the RP rejects an identity token in Step 4.2,
    if the signature is invalid or $PID_{RP}$ enclosed in it is not equal to $\neq [t]ID_{RP}$.
