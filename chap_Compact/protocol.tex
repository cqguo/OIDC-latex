...
With the identity-transformation functions,
 UPRESSO potentially satisfies all the requirements of security and privacy
    as an SSO service. %desirable for a secure and privacy-preserving SSO service.
But we still another design, \emph{RP certificate},
     to ensure .
In the original SSO protocols,
    an RP is initially registered at the IdP with an endpoint to receive identity tokens, 
        and this endpoint is set in an identity-token response by the IdP, to instruct the user to forward the identity token.
Obviously,
    an incorrect RP endpoint breaks the confidentiality of identity tokens.
...
RP certificates are designed to 
    
    the target RP's endpoint (i.e., redirection URI in OIDC) is pre-registered at the IdP,
        and is a parameter in the identity-token response.
    
    verified parameter return from the IdP.
To ensure 
    in privacy-preserving SSO, since the IdP knows nothing about the visited RP,
    the user has to obtain this critical parameter by itself.
We 
...
%    {$n$} & {the order of the base point $G$.} \\ \hline
   ...
   {$ID_{RP}$} & {$ID_{RP} = [r]G$, $1 < r < n$; an RP's unique identity at the IdP.} \\ \hline
    ...
    {$SK$, $PK$} & {The IdP's key pair, a private key and a public key, to sign/verify identity tokens and RP certificates.} \\ \hline
    {$Cert_{RP}$} & {An RP certificate, containing the RP's identity and endpoint.} \\ \hline
...
%\end{itemize}
\textcolor[rgb]{1.00,0.00,0.00}{Note that, in UPRESSO, $ID_{RP}$ must be generated by the IdP but cannot be chosen by the RP.}
why cannot? How about $ID_U$?
% with $r$ unknown to the RP.
...
%\vspace{1mm}
\noindent\textbf{SSO Login.} An SSO login procedure is launched when a user requests to login an RP.
...
It consists of five phases, namely scripts downloading, RP identifier transformation, $PID_{RP}$ registration, identity token generation, and $Account$ calculation, as shown in Figure \ref{fig:process}.
