...
The RP should compute $[t]ID_{RP}$ by itself and verify if it equals the received $PID_{RP}$. Otherwise, a malicious user with $ID_{U'} = u'$ could impersonate a victim user by using $[t'u'^{-1}]Acct$ as $PID_{RP}$ to request an identity token from the IdP, where the victim user with $ID_{U} = u$ holds $Acct$ at that RP. The IdP would calculate $PID_{U'} = [u'][t'u'^{-1}]Acct = [t']Acct$, with which the RP would authenticate the malicious user under $Acct$. 
...
If we ask the RP to select $t$ and calculate $PID_{RP}$, the user needs to compute $[t]ID_{RP}$ by herself and check if $PID_{RP}=[t]ID_{RP}$ holds. Otherwise, a malicious user could impersonate the victim user at an honest RP by colluding with a malicious $RP'$. That is, when receiving $PID_{RP}=[t']ID_{RP}$ from the RP, the malicious user with $ID_{U'}$ forwards it to $RP'$, which sends it to the victim user with $ID_{U}$ in another login instance. If the victim user does not check the received $PID_{RP}$, she would request an identity token that binds $PID_U$ and $PID_{RP}$ from the IdP and presents the token to the malicious RP. This token would allow the malicious user to log in to the honest RP under her account.
...
The RP script is responsible for preparing $ID_{RP}$ and $Enpt_{RP}$ for the IdP script using the \emph{RP certificate} issued by the IdP during RP registration. %It binds the RP's identity and endpoint. %(i.e., $ID_{RP}$ and $Enpt_{RP}$).
...
After receiving an identity token from the IdP, the IdP script needs to ensure that the RP script will forward the token to $Enpt_{RP}$ %, which is bound with $ID_{RP}$ 
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item[2.1] The IdP script chooses a random number $t$ in $\mathbb{Z}_n$ and sends it to the RP script through \verb+postMessage+ within the browser.
...
\textcolor{blue}{The IdP may reject an identity-token request Step 3.3 if the received $PID_{RP}$ is not a point on $\mathbb{E}$, and the RP may reject an identity token in Step 4.2 if the signature is invalid or the enclosed $PID_{RP}$ is not equal to $[t]ID_{RP}$.}



\subsection{Compatibility with OIDC}
\label{subsec:compatible}
Both UPPRESSO and OIDC work with COTS browsers. Among the four steps of the login flow in UPPRESSO,
