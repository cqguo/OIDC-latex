...
We further propose the following designs specific for web applications,
...
Because the UPPRESSO IdP is not aware of the visited RP and cannot set the RP endpoints,
        \emph{RP certificates} are designed to instructs the user agents (or browsers) about RP endpoints.
An RP certificate is a document signed by the IdP, binding the RP's identity and its endpoint.
...
\begin{figure*}[!t]
  \centering
  \includegraphics[height=0.805\textheight]{fig/process-js.pdf}
...
\noindent\textbf{RP Initial Registration.} Each RP launches
    an initial registration operation to finish configurations.
...
The user registers an ephemeral $PID_{RP}$ at the IdP.
...
The operations in the $PID_{RP}$ registration are almost identical to those in the RP Dynamic Registration of OIDC \cite{DynamicRegistration},
    ...
    the OIDC IdP assigns the RP's identity  while this (pseudo-)identity in UPPRESSO is generated by the registered entity.
