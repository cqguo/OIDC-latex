...
Two scripts, one downloaded from the IdP and the other from the target RP,
    work together (with the standard  browser functions) as a user agent of UPPRESSO.
...
\vspace{-\topsep}
\item [4.4] The IdP checks whether the received pair of $PID_{RP}$ and $PEnpt_U$ is in the list of unexpired $PID_{RP}$ or not,
...
\vspace{-\topsep}
\item[4.5] The IdP sends the identity token to $PEnpt_{U}$.
\vspace{-\topsep}
\end{itemize}
...
\vspace{-\topsep}
\item [5.3] The RP extracts $PID_U$, calculates $Acct = [t^{-1}]{PID_U}$, and allows the user to login as $Acct$.
\vspace{-\topsep}
\end{itemize}
...
The operations in the steps of identity token generation and $Acct$ calculation, 
    ...
    while (\emph{a}) the calculation of $PID_U$ is viewed as a method to generate PPIDs by the IdP
        and (\emph{b}) the calculation of $Acct$ is viewed as a mapping 
    the RP's operations are actually identical to 
