...
\item[3.2] The IdP checks the list of unexpired $PID_{RP}$
    to verify that the received $PID_{RP}$ is unique.
Then, it signs the response as $[PID_{RP}, H(N_U), validity]_{SK}$, where $validity$ denotes when the $PID_{RP}$ will expire.
\vspace{-\topsep}
