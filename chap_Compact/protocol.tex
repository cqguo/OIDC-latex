...
It is impossible for the RP to derive $ID_U$ from either $PID_U$ or $Acct$ due to the elliptic curve discrete logarithm problem (ECDLP),
    and for the IdP to learn $ID_{RP}$ from $PID_{RP}$ because $t$ is random in $\mathbb{Z}_n$
     and $PID_{RP}$ is indistinguishable from a random variable on $\mathbb{E}$ from the IdP's view.
...
After receiving $t$ from and extracting $PID_{RP}$ from the token,
    an RP checks that $PID_{RP} = [t]ID_{RP}$,
        ...
        otherwise, a user $U$ might send $t' \neq t$ to login as another user's account,
...
and interacts with users for the authorization of user attributes,
...
\item [3.3]
The IdP script obtains the user's authorization to enclose each attribute,
