...
%    \begin{tabular}{|c|c|c|}
    ...
    {$ID_U$} & {$ID_U = u \in [1, n)$ is a user's unique identity at the IdP, \newc{which is known only to this user and the IdP.}} \\ \hline
   {$ID_{RP_j}$} & {$ID_{RP} = [r]G$ is the $j$-th RP's unique identity, which is publicly known; $r \in [1, n)$ \newc{is known only to the IdP but not the RP.}} \\ \hline
    {$t$} & {$t \in [1, n)$ is a user-selected random integer in each login; \newc{$t$ is shared with the target RP and kept unknown to the IdP.
    }} \\ \hline
...
On receiving an identity-token request for $PID_{RP}$ from a user with $ID_U$, the IdP calculates $PID_{U}$ as below.
\begin{equation}
...
\newc
Meanwhile, a user's identity $u$ is kept secret from the RPs. Otherwise, colluding RPs could calculate $[u]ID_{RP_j}$ for the known $u$s and link them.
...
\begin{figure*}[htb]
  \centering
  \includegraphics[height=0.559\textheight]{fig/process-js.pdf}
...
\item[4.2] The RP verifies the signature and the validity period of the identity token 
%\newc
