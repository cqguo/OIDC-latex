...
In addition to the identity-transformation functions,
    we propose the following designs specific for web applications,
        facilitating a user to visit the services through browsers.
...
In existing SSO protocols,
    an RP initially registers its endpoint at the IdP,
        and then in each login instance, the IdP will set this endpoint correctly in the identity-token response according to the RP identity.
This instructs the user agent (or browser) to forward it correctly;
    otherwise, the confidentiality of identity tokens might be broken.

Because the IdP of UPPRESSO is not aware of the visited RP and then cannot set the RP endpoints,
        \emph{RP certificates} are designed to instructs the browsers about RP endpoints.
...
Secondly,
    browser scripts are needed to implement the functions by users,
        including the generation of $t$ and $PID_{RP}$,
            ...
            for these are not standard functions of a browser.
Two scripts, one downloaded from the IdP and the other from the target RP,
    work together in the browser as a user agent.
The IdP script communicates with the IdP,
    the RP script does with the RP,
    while two scripts communicates with each other through the \verb+postMessage+ HTML5 API.
The RP script is necessary to keep the sessions with the RP,
    and the IdP script cannot be eliminated or replaced because an HTTP request launched by the RP script and sent to the IdP
            will automatically carry the \verb+Referer+ HTTP header, which discloses the RP's domain.

Finally,
    the IdP's public key is also downloaded in the IdP script to verify RP certificates, which are sent from the RP.
So the user agent does not need configure anything locally,
    as it does in OIDC.

The above designs enable UPPRESSO to provide SSO services for users with standard browsers,
    more efficient but less portable implementations with browser extensions and/or plug-ins is discussed in Section \ref{sec:discussion}.
...
do not
The designs and implementations of UPPRESSO
