...
%    \begin{tabular}{|c|c|c|}
    ...
    {$E$}&{An elliptic curve defined over a finite field $\mathbb{F}_q$ (e.g., P-256).} \\ \hline
    {$ID_U$} & {$ID_U = u$, $1 < u < n$} \\ \hline
   {$ID_{RP}$} & {$ID_{RP} = [r]G$, $1 < r < n$} \\ \hline
    {$q$} & {A large prime, the size of the underlying field.} \\ \hline
    {G}&{A point in $E(\mathbb{F}_q)$,  known as the base point.} \\ \hline
    {$n$} & {the order of the base point $G$.} \\ \hline
    {$SK$, $PK$} & {The IdP's key pair, a private key and a public key.} \\ \hline
    {$Cert_{RP}$} & {An RP certificate, containing the RP's identity and endpoint.} \\ \hline
     {$Acct$} & {A user's identifier at an RP: $A=  {ID_U}{ID_{RP}} $ (denoted as $A$ in equations).} \\ \hline
    {$PID_{RP}$} & {$PID_{RP} = {N_{U}}{ID_{RP}} $, an RP's pseudo-identity.} \\ \hline
    {$PID_U$} & {$PID_U = {ID_U}{PID_{RP}}$, a user's pseudo-identity.} \\ \hline
    {$N_U$} & {A user-generated nonce for $PID_{RP}$.} \\ \hline
...
When a user is registering at the IdP,
...
\noindent {\bf $ID_{RP}$-$PID_{RP}$ Transformation.} In each login instance,
    the user selects a random number $N_{U}$ ($1 < N_{U}<n$) and calculates $PID_{RP}$ as:
    
    $\mathcal{F}_{PID_{RP}}(ID_{RP}) = $, 
...
Moreover, the nonce $N_{U}$ ensures that: (\emph{a}) $PID_{RP}$ is a one-time pseudo-identity in each login;
%that is valid only for the identity token generated in this login;
...
To avoid using the user identity $ID_U$ in identity tokens, the IdP can convert $ID_U$ into a pseudo-identity for the user as follows:
\begin{equation}
\setlength{\abovedisplayskip}{3pt}
\setlength{\belowdisplayskip}{3pt}
...
%\vspace{0.5mm}
\noindent {\bf $PID_U$-$Acct$ Transformation.} We define the trapdoor $T$ in each login session as $T = N_U^{-1} \bmod n$.
%As $n$ is a prime number and $1< N_U < n$, $n$ is coprime to $N_U$. So, there always exists a $T$ that satisfies $T N_U = 1 \bmod n$.
With this trapdoor, the RP derives a unique account (denoted as $A$ in the equation) for the user as:%with the function $\mathcal{F}_{PID_{U} \mapsto Account}$
\begin{equation}
\setlength{\abovedisplayskip}{3pt}
\setlength{\belowdisplayskip}{3pt}
...
In particular, the IdP generates key pair ($SK$, $PK$) to sign identity tokens and RP certificates.
