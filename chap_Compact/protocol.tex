...
If the RP does not validate the received $PID_{RP}$, it would use the received $PID_{U'}$ to calculate an account as $Acct = [t'^{-1}]PID_{U'}=[u]ID_{RP}$, which is the account of a potential victim user with $ID_{U} = u$.
The malicious user could repeat the above attacks with random $u \in \mathbb{Z}_n$,
     until the account of some target victim user is calculated by the RP.\footnote{In this attack case,
     if it does not derive an account of some honest user, the RP will consider the token holder as a newly-registered user.}
