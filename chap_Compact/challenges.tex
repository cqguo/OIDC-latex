...
First of all,
    UPRESSO adopts the communication pattern of OpenID Connect,
...
In order to prevent \emph{IdP-based visit tracing},
    the IdP of UPRESSO is aware of $PID_{RP}$ but not $ID_{RP}$,
        and it is computationally impossible for the IdP to derive (any information about) $ID_{RP}$ from $PID_{RP}$ and $PID_{U}$;
to prevent \emph{RP-based identity linkage},
    an RP accepts identity proofs including $PID_{U}$ but not $ID_{U}$,
     to derive $Account$ distinct among RPs,
        and it is computationally impossible for the RP to derive (any information about) $ID_{U}$ from $PID_{U}$, $PID_{RP}$ and $Account$.

Finally,
    we attempt to achieve \emph{user identification} and \emph{receiver designation},
...
UPRESSO introduces trapdoor user identification,
    while an RP holds a trapdoor to derive the identical $Accout$ from dynamic $PID_{U}$s in identity proofs.
Intuitively,
this trapdoor also works in the generations of $PID_RP$ and $PID_U$
...
The framework of identity transformation in Figure \ref{fig:IDCorrelation}
    is applicable to explain the approaches of existing privacy-preserving SSO solutions,
