...
\label{sec:challenge}
In 
this section, we analyze the challenges to design secure privacy-preserving SSO systems,
     and provide an overview of the identifier-transformation approach of UPPRESSO.
...
Or,
     ...
     if confidentiality is not well ensured.
...
As mentioned in Section \ref{sec:intro},
    the user privacy is leaked through IdP-based login tracing or RP-based identity linkage.
...
Since integrity and confidentiality are ensured by digital signatures of the IdP and TLS communications in the system \cite{OpenIDConnect},
    we focus only \emph{user identification} and \emph{RP designation} in the remainder.

The pseudo-identifiers of users and RPs (denoted as $PID_U$ and $PID_{RP}$, respectively)
    are introduced,
     as we consider the two privacy threats.
To prevent IdP-based login tracing, % while ensure RP designation,
    the IdP shall be aware of $PID_{RP}$ at most
        and then $ID_{RP}$ does not appear in identity proofs;
    otherwise, if $ID_{RP}$ is disclosed to the IdP,
     it enable IdP-based login tracing because the user is authenticated by the IdP and $ID_U$ is automatically disclosed.
...
Finally,
    only the pseudo-identifiers of users and RPs (i.e., at most $PID_U$ and $PID_{RP}$,
     but not $ID_U$ or $ID_{RP}$) are bound in identity proofs.

When we consider both the requirements of security and privacy,
it brings the identity dilemma as follows:
    when the IdP is unaware of the visited RP,
        it (\emph{a}) binds $PID_{RP}$ in each identity proof that enables an RP to verify the specified receiver,
        to \emph{ensure RP designation},
        and (\emph{b}) generates $PID_U$s in the identity proofs for a user in multiple logins at an RP,
         which enable the RP to derive the identical user identifier
         (i.e., $Account$), to \emph{ensure user identification}.
Moreover,
    for a certain user,
    (\emph{c}) to \emph{prevent IdP-based visit tracing}, different $PID_{RP}$s appear in the multiple logins at an RP, 
        while
        it is computationally infeasible for the IdP to derive $ID_{RP}$ from $PID_{RP}$ and $PID_{U}$,
    and (\emph{d})  to \emph{prevent RP-based identity linkage}, $PID_U$s and also the derived $Account$s are different when the user visits different RPs,
    while 
    it is computationally infeasible for an RP to derive $ID_{U}$ from $PID_{U}$, $PID_{RP}$ and $Account$.
...
One-time $PID_{RP}$ and $PID_U$ are generated and bound in the identity proof,
    while $PID_{RP}$ enables the verification of RP designation and $PID_{U}$ enables the derivation of identical $Account$.
It is worthy noting that, when user privacy is not well considered,
    this relationship is simplified.
For example,
in commonly-deployed OIDC systems \cite{OpenIDConnect},
...
%In order to prevent IdP-based login tracing,
...
%Again, if IdP-based login tracing is not a threat,
...
As shown in Figure \ref{fig:IDCorrelation},
to design secure privacy-preserving SSO solutions
    (i.e., solve the identity dilemma),
  we need to find out qualified identifier-transformation functions to compute $PID_U$, $PID_{RP}$ and $Account$,
denoted as $\mathcal{F}_{ID_{U} \mapsto PID_U}$,   $\mathcal{F}_{ID_{RP} \mapsto PID_{RP}}$,
  and $\mathcal{F}_{PID_{U} \mapsto Account}$, respectively.

%We present UPPRESSO, a secure and privacy-preserving SSO system,
%which prevents the IdP-based login tracing and RP-based identity linkage under the prerequisites that  the basic requirements of SSO systems are satisfied.
...
%In order to prevent \emph{IdP-based login tracing},
...
%        by the comprehensive design of identifier transformations
...
Intuitively,
this trapdoor also plays a part in the generations of $PID_{RP}$ and $PID_U$
