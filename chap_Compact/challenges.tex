...
The IdP knows $ID_U$ after authenticating the user.
One-time $PID_{RP}$ are generated for identity proof,
as well as  $PID_U$.
After verifying $PID_{RP}$, the RP accepts the identity proof,
    and compute $Account$ from $PID_U$.
This can also for no privacy requirement,
    $ID_U$ is assigned to $PID_U$ and $ID_{RP}$ is assigned to $PID_{RP}$,
        so that $ID_U$ and $ID_{RP}$ are bound in identity proofs.
