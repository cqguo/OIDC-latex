...
An honest RP verifies the RP's (pseudo-)identity in the identity token before accepting it,
 and authorizes the token holder to log in as the specified user. This prevents malicious RPs from replaying received identity tokens to gain unauthorized access to other honest RPs as some victim user.
...
The requirements for secure SSO services, i.e., the RP designation, user identification, integrity, and confidentiality of identity tokens, have been extensively studied in the literature \cite{ArmandoCCCT08, SPRESSO, FettKS14, FettKS16, FettKS17}.
...
We aim to develop a privacy-preserving SSO system that ensures security properties
 while prevents both IdP-based login tracing and RP-based identity linkage.
...
%        which are independent of each other for different RPs and in multiple logins, respectively,
...
To ensure RP designation, $PID_{RP}$ should be \emph{uniquely} associated to the target RP.
For user identification, the \emph{ephemeral} $PID_{U}^i$ in each login should enable the RP to derive the user's \emph{permanent} account  (i.e., $Acct$) at this RP.
...
Thus, in a user's multiple logins to the same RP, she should generate independent $PID_{RP}^i$\footnote{The IdP should not be able to link multiple logins to a given RP, while the RP's identity is unknown to the IdP.} % the IdP-based login tracing is still effective, to correlate a user's multiple logins.
...
Finally, we propose identity transformations as below:
