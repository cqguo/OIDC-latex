...
\vspace{0.75mm}
\noindent\textsc{Proof.} 
For a given $PID_{RP}$, the IdP calculates $PID_U = [ID_U]PID_{RP}$ following Eq.~\ref{equ:PIDU}. Therefore, $PID_U$ is associated with $ID_U=u$ only but not with any other user with $ID_{U'}$. 
Meanwhile, $PID_U$ can only be calculated by the IdP and the user, since no one else knows or intercepts $u$, according to the \dy~model.
...
\vspace{1mm}
\noindent{\textsc{Theorem 4. (Identity Token Confidentiality)}} {\em A valid identity token $T$ can only be accessed by the RP that is designated by $PID_{RP}$ in $T$, besides the requesting user and the issuing IdP.}
