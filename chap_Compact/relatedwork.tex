...
\vspace{1mm}
\noindent\textbf{RP-Verified Credential.}
A user of EL PASSO \cite{ZhangKSZR21}
    keeps a secret on his device.
After authenticating the user,
    the trusted IdP issues a certified long-term credential binding the secret,
...
These RP-verified credentials protect user privacy well \cite{ZhangKSZR21,IsaakidisHD16}, %and enable the visits to multiple RPs,
...
In EL PASSO, UnlimitID or FIDO,
    a user needs to notify each RP one by one when a credential or FIDO device is lost or compromised,
because the user is authenticated actually by the RP with this user-maintained credentials.\footnote{EL PASSO
 calls itself an SSO scheme \cite{ZhangKSZR21},
    but it follows a login flow different from commonly-used SSO protocols such as OIDC and OAuth 2.0.}
On the contrary,
    in the commonly-used SSO systems (and also UPPRESSO),
