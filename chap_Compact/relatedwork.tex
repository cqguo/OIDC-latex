...
\usso\ leverages three properties of OPRFs \cite{oprf-proved,voprf-proved}, %which are also utilized by other OPRF-based designs~\cite{oprf-proved, voprf-proved, oprf-bitcoin-wallet, privacypass, trusttoken},
 to prove (parts of) its security and privacy. % in Theorems 4, 6, and 7.
...
It is satisfied (see Lemma 1) only if no collision exists in RPs' pseudo-identities, which are actually the blinded inputs of the evaluated pseudo-random function.
...
For example, we plan to analyze this property of the quantum-secure ORPF protocols \cite{ideal-lattice-oprf,isogency-oprf} in the future,
 to find out whether they work compatibly with the identity transformations in \usso.
...
PESTO \cite{pesto} combines distributed partially-oblivious PRFs and distribution signatures,
    and applies proactive security to build SSO services;
