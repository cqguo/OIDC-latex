...
PrivacyPass and TrustToken directly adopted the OPRF design for their token generation, i.e., the token server obliviously calculates a pseudo-random output for any user input,
...
\usso\ leverages three properties of OPRFs \cite{oprf-proved,voprf-proved}, %which are also utilized by other OPRF-based designs~\cite{oprf-proved, voprf-proved, oprf-bitcoin-wallet, privacypass, trusttoken},
 to ensure (parts of) its security and privacy. % in Theorems 4, 6, and 7.
...
and the \emph{randomness} property to prevent colluding RPs from linking the users' permanent accounts these across RPs.
 %when user identities (i.e., the key secret $k$ of pseudo-random functions) are unknown.
Last but not least, \usso\ requires an additional property in SSO services, called \emph{RP designation}. It is satisfied (see Lemma 1) only if no collision exists in RPs' pseudo-identities, which are the blinded inputs of the pseudo-random function. 
