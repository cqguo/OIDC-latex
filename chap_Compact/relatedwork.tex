...
PrivacyPass and TrustToken \cite{privacypass,trusttoken} designed anonymous tokens to grant anonymous access to applications to authorized users. They adopted the oblivious pseudo-random function (OPRF) protocol \cite{oprf-proved,voprf-proved,oprf-bitcoin-wallet} to generate anonymous tokens: a user generates a random $e_i$ for each unsigned token $T_i$ and blinds $T_i$ into $[e_i]T_i$.
Once the user is authenticated, a token server signs $[e_i]T_i$ with a private key $k$ as $[k e_i]T_i$. Then, the user converts $[ke_i]T_i$ to $[k]T_i$ and redeems the token ($T_i, T_i^k$) to access resources.
\usso~applies a similar cryptographic skill to design identity transformations: a user transforms $ID_{RP}$ to $PID_{RP} = [t_i]ID_{RP}$ using a random $t_i$. Then, the IdP calculates $PID_U = [u]PID_{RP}$ %= [ut]ID_{RP}$ 
from $PID_{RP}$ using a random $u$.
Finally, the visited RP calculates $Acct = [t_i^{-1}]PID_{U}$ from $PID_{U}$ using the shared $t_i$. If we directly compare the two approaches, the IdP-untraceability property in \usso, i.e., $[t_i]ID_{RP}$ and $ID_{RP}$ cannot be linked by an IdP, roughly corresponds to 
the unlinkability between token signing and redemption in PrivacyPass and TrustToken, i.e., ($[e_i]T_i, [ke_i]T_i$) and  ($T_i, [k]T_i$) cannot be linked by a server.

However, PrivacyPass and TrustToken directly adopted the OPRF design for their token generation, i.e., the token server calculates a pseudo-random number for a given user input, so that the anonymous tokens are completely indistinguishable. When they are used to realize SSO, the RPs cannot identify each user. In contrast, \usso~leverages the cryptographic technique in a greater depth than anonymous tokens to provide a non-anonymous but privacy-preserving SSO service. First, it extends the original two-party protocol to work for three parties in SSO. It lets the user and the visited RP share the random number $t$, which is used for protecting $ID_{RP}$, and thus enables the RP to derive the user's permanent account at this RP. This extension is unlikely to occur in OPFRs that consider $k$ as the private key of the server, but it is reasonable in \usso~as it uses random numbers as identities and trapdoors to design oblivious pseudo-random functions for identity transformations. 
...
Second, we leverage properties of oblivious random functions to prove an additional property for SSO, called RP unlinkability, which is not considered or supported by anonymous tokens. It ensures that colluding RPs cannot link any two logins even if they share the knowledge about the pseudo-identities and permanent accounts of the users who initiate these logins.
...
\usso~leverages three properties of oblivious pseudo-random functions, which are also utilized by other OPRF-based designs~\cite{oprf-proved, voprf-proved, oprf-bitcoin-wallet, privacypass, trusttoken}, to prove its security. %and privacy in Theorems 4, 6, and 7. 
It uses the \emph{obliviousness} property to prevent the IdP from learning any information about $ID_{RP}$ when receiving a token request for $[t_i]ID_{RP}$, the \emph{deterministicness} property of \emph{pseudo-random functions} to enable the RP to derive the permanent account for a given $t$, and the \emph{randomness} property to prevent colluding RPs from linking the users' permanent accounts when $u$ or $k$ are unknown. Last but not least, \usso~requires an additional property, called \emph{RP designation}, and proves in Lemma 1 that no collision exists in RPs' pseudo-identities, which are the input of the pseudo-random function. This property is not required in other OPRF-based approaches and therefore may not be supported by all OPRF protocols. So, an OPRF protocol cannot be directly applied to implement \usso~unless this additional property is proved.
...
Other cryptographic primitives such as zero-knowledge proofs and distributed key generation are used to prove users' statements about their credentials without revealing the credential content (e.g., ZKlaims \cite{zklaim}) or generate ring-signature private keys as untraceable pseudonyms~\cite{crypto-book}. Two-party threshold cryptography is also used~\cite{tandem} to generate one-time key-share tokens to protect key-usage patterns.
