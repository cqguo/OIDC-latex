...
ZKlaims \cite{zklaim} allow users to prove statements on credentials issued by a trusted third party 
    by zero-knowledge techniques,
        so that the credential contents are not revealed to the verifiers.
PrivacyPass \cite{privacypass} allows users to receive a significant amount of anonymous tokens.
 These tokens are then used to access resources on content delivery networks,
    so that the user does not interact with challenges such as CAPTCHAs.
Crypto-Book \cite{crypto-book} coordinates several key servers to generate a user private key for ring signatures, and the user picks up his private key through a list of email addresses (i.e., an anonymity set), including his own email address.
...
Two-party threshold-cryptographic schemes are implemented with a central server,
    to improve the security of a user's private key \cite{mRSA,ss-rsa}:
    to sign or decrypt a message, the user needs to obtain a token from the server.
To preserve the privacy of key-usage patterns,
