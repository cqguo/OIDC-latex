...
This unusual application requires different understandings of the variables in OPRFs and the (pseudo-)identities in SSO services.
...
Moreover, \usso\ actually leverages the randomness property of OPRFs
 to provide \emph{RP unlinkability}, while this property is not discussed in PrivacyPass/TrustToken.
...
Although \usso\ employs the same mathematical algorithms in its identity transformations as the OPRF protocol \cite{oprf-proved,voprf-proved},
 we prove more properties of these algorithms %which are also utilized by other OPRF-based designs \cite{oprf-proved, voprf-proved, oprf-bitcoin-wallet, privacypass, trusttoken},
 to ensure security and privacy. % in Theorems 4, 6, and 7.
\usso\ uses the \emph{obliviousness} property to prevent the IdP from learning any information about $ID_{RP}$ when receiving a token request for $[t_i]ID_{RP}$, the \emph{deterministicness} property of \emph{pseudo-random} functions to enable the RP to derive a permanent account for any $t$,
