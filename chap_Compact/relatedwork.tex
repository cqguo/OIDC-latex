...
The exponent $k$ in PrivacyPass/TrustToken is held only by the server as a private key, and the random number $e$ is only known to a user. In contrast, the scalar $u$ in \usso~denotes a user identity known to the user and the IdP,
 while the random number $t$ is an ephemeral trapdoor shared between the user and the RP.
...
The sharing of random numbers enables the derivation of permanent accounts,
 while the additional unlinkability across colluding RPs is supported. Their knowledge about two different users $u$ and $u'$, i.e., ($ID_{RP}, t, [u]ID_{RP}$) and ($ID_{RP'}, t', [u']ID_{RP'}$), obtained from different login instances, are indistinguishable.
...
\oldc
Cryptographic primitives are used to protect user privacy.
