...
in greater depth, resulting in additional privacy protection. The exponent $k$ in PrivacyPass/TrustToken is held only by the server as a private key, and the random number $e$ is only known to a user. In contrast, the scalar $u$ in \usso~denotes a user identity known to the user and the IdP, while the random number $t$ is an ephemeral secret shared between the user and the RP.  
...
This property requires designing the transformations under the ECDDH assumption (see Section~\ref{sec-:analysis}), which is not considered or supported in existing anonymous tokens \cite{privacypass,trusttoken} or OPRFs \cite{oprf-proved,voprf-proved}.
