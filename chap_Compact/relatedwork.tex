...
However, none of them ensures the identity tokens are sent to the designated RP only \cite{ChenPCTKT14,WangZLLYLG15},
    because a WebView or the system browser cannot authenticate the RP Apps and the IdP App may be repackaged.
The SSO protocols are modified to work for mobile Apps, but these modifications are not well understood by developers \cite{ChenPCTKT14,YangLS17}.
