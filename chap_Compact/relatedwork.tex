...
Fett et al. \cite{FettKS16, FettKS17} formally analyze OAuth 2.0 and OIDC using an expressive Dolev-Yao style model \cite{FettKS14},
    and presented two attacks, i.e., the attacks of 307 redirection and IdP mix-up.
...
SAML-based SSO is also formally analyzed \cite{ArmandoCCCT08},
    and the RP identity is found not to be bound correctly in the identity tokens of its variant designed by Google.
...
Various vulnerabilities were found in SSO implementations,
    which could be exploited to launch impersonation and identity injection attacks
...
In fact,
    the integrity of identity tokens was found to be vulnerable in lots of SSO systems  \cite{SomorovskyMSKJ12,WangCW12,WangZLG16,MainkaMS16, MainkaMSW17}
...
Vulnerabilities also break the RP designation of tokens,
    such as incorrect binding at the IdP \cite{YangLCZ18,WangZLG16} and insufficient verification by RPs \cite{MainkaMS16,MainkaMSW17,YangLCZ18}.
...
Wang et al. \cite{ExplicatingSDK} also present a tool to detect the vulnerable applications
    constructed on top of authentication/authorization SDKs,
...
In mobile SSO applications,
    the IdP App, IdP-provided SDK (e.g., an encapsulated WebView) or system browser
         are responsible for redirecting identity tokens from the IdP App to RP Apps.
...
The SSO protocols needed to be modified to work for mobile Apps, but these modifications are not well understood by RP developers \cite{ChenPCTKT14,YangLS17}.
Vulnerabilities were found in lots of Android applications,
    to break the confidentiality \cite{TowardsShehabM14,ChenPCTKT14,WangZLLYLG15,YangLS17,ShiWL19}, integrity \cite{ChenPCTKT14,YangLS17}, and RP designation \cite{ChenPCTKT14,ShiWL19} of SSO identity tokens.
