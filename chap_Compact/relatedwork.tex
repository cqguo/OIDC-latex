...
In SSO systems, once a user account at the IdP is compromised,
    the adversaries will control all his accounts at the RPs.
An extension to OIDC, named single sign-off \cite{GhasemisharifRC18}, is proposed and then
        the IdP helps the victim user 
 to revoke all his active identity tokens and logout from these RPs.
...
\vspace{1mm}
\noindent\textbf{Certified Anonymous Credential.}
EL PASSO \cite{ZhangKSZR21} 
    requires a user to keep a permanent secret on his device.
After authenticating the user,
    the trusted third party (TTP) issues a certified long-term anonymous credential, also kept on the user's device.
Once attempting to login to any RP,
    the user proves that he is the owner of this credential to the RP without exposing the secret;
even if a credential is used for multiple RPs,
    this anonymous credential \cite{anon-credential} prevents collusive RPs from linking these login instances.
UnlimitID \cite{IsaakidisHD16} presents similar designs.
TTP-certified anonymous credentials protect user privacy in the SSO scenarios \cite{ZhangKSZR21,IsaakidisHD16},
    but the user has to  by himself management user attributes and the accounts at different RPs.
FIDO \cite{fidouaf} allows a user to generate a key pair on her FIDO-certified device
    and register the public key at an RP, so that the user logins to this RP with the corresponding private key.
FIDO supports multiple key pairs for different RPs on one device,
        and the RP-based linkage is also considered and prevented in the registration flow.

In EL PASSO, UnlimitID or FIDO,
    the most inconvenient operations for a user,
        is to notify each RP when a credential or FIDO device is lost or compromised,
because a user is authenticated by the RP with certified anonymous credentials.
On the contrary,
    in the SSO systems (and also UPPRESSO)
        the user only needs to renew his credential at the IdP if it is stolen,
    as authentication happens between a user and the IdP (and an RP only verifies tokens).