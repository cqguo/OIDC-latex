...
Last but not least, \usso\ requires an additional property in SSO services, called \emph{RP designation},
 and it is ensured only if no collision exists in RPs' pseudo-identities (see Theorem 1 and Lemma 1), which are actually the blinded inputs of the evaluated pseudo-random function.
This property is not required in other OPRF-based approaches and therefore may not be supported by all OPRF protocols.
Thus, an OPRF protocol cannot always be ready to implement identity transformations in \usso, unless no collision exists in the blinded inputs of the pseudo-random function.
