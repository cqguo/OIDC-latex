...
\vspace{1mm}
\noindent\textbf{RP-Verified Anonymous Credentials.}
A user of EL PASSO \cite{ZhangKSZR21}
    keeps a permanent secret on his device.
After authenticating the user,
    the trusted IdP issues a certified long-term credential, also kept on the user's device.
...
even if a credential is verified by multiple RPs,
...
These RP-verified anonymous credentials protect user privacy well \cite{ZhangKSZR21,IsaakidisHD16}, %and enable the visits to multiple RPs,
    but the user has to by himself manage user attributes at different RPs.
