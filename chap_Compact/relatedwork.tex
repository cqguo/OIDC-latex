...
\usso\ applies a similar cryptographic skill to design identity transformations. A user selects a random number $t_i$ to transform $ID_{RP}$ to $PID_{RP} = [t_i]ID_{RP}$, which prevents an IdP from learning about $ID_{RP}$.
...
It ensures that colluding RPs cannot link any two logins, %i.e., ($ID_{RP}, t, [u]ID_{RP}$) and ($ID_{RP'}, t', [u']ID_{RP'}$),
 by sharing their knowledge about the pseudo-identities and permanent accounts related to the logins.
...
PESTO \cite{pesto} combines distributed partially-oblivious PRFs and distribution signatures,
    to build proactive-secure SSO services,
    and proposes to sign a commitment of an identity token (but not the token itself) for better user privacy.
