...
\vspace{1mm}
\noindent\textbf{RP-Verified User Credential.}
...
even if such a credential is verified by multiple RPs,
        user-maintained pseudonyms and anonymous credentials \cite{anon-credential} prevents collusive RPs from linking the login instances.
...
PseudoID \cite{PseudoID} introduces a token service in addition to the IdP,
    to  blindly sign a pseudonym credential binding the user's secret.
Then, the user proves to the RP that he owns this secret to login. 
These RP-verified credentials protect user privacy well \cite{ZhangKSZR21,IsaakidisHD16,PseudoID} %and enable the visits to multiple RPs,
        and the two kinds of privacy threats are prevented,
...
because the user is authenticated by the RP with his user-maintained credentials
            to prove that he owns the secret.
On the contrary,
    in the commonly-used SSO systems such as OIDC \cite{OpenIDConnect}, OAuth 2.0 \cite{rfc6749} and also UPPRESSO,
...
Although EL PASSO
 calls itself an SSO scheme \cite{ZhangKSZR21}
    ...
    the authentication steps between the user and an RP do not exist in the common SSO protocols.
