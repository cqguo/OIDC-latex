...
ZKlaims \cite{zklaim} allow users to prove statements on the credentials issued by a trusted third party 
    by zero-knowledge proofs,
        so that the credential contents are not revealed to the verifiers.
PrivacyPass \cite{privacypass} allows a user to receive a significant amount of anonymous tokens.
...
Crypto-Book \cite{crypto-book} coordinates several key servers to generate a user private key for ring signatures, and the user picks up his private key through a list of email addresses (i.e., an anonymity set).
