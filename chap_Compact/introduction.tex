...
An RP delegates user identification and authentication to the IdP, which issues an \emph{identity token} (such as an id token in OIDC or an identity assertion in SAML) for a user to visit the RP. %after authenticating this user.
...
(\emph{b}) \emph{user authentication only once to a trusted IdP}, which eliminates the need for authentication between a user and an RP and requires maintaining only the credential for the IdP, 
