...
Another privacy risk arises from the fact that RPs learn the user's identity from the identity tokens they receive. If the same user identity is enclosed in tokens for visits to different RPs~\cite{maler2008venn, Google, FirefoxAccount}, colluding RPs could link these login instances across different RPs %and track the user's activities
...
We present an Untraceable and Unlinkable Privacy-PREserving Single Sign-On (UPPRESSO) protocol for privacy-preserving SSO. It involves the use of {\em identity transformations} in the SSO login flow. %to generate ephemeral identifiers that cannot be linked to the permanent identifiers of the user and the RP. 
