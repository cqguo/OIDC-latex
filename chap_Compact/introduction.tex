...
\begin{table}[tb]
    \caption{Identifiers of user \& RP in privacy-preserving SSO.}
    \centering
    \begin{tabular}{|c|c|c|}
    \hline
    {Solutions} & {Identifier of visiting RP at IdP} & {User's identifier at RP} \\
    \hline
    {PPID} & {$ID_{RP}$} & {$PID_U=PPID$} \\
    \hline
    {SPRESSO} & {$PID_{RP}$} & {$ID_U$} \\
    \hline
    {BrowserID} & {$PID_{RP}=Null$} & {$ID_U$} \\
    \hline
    {UPRESSO} & {$PID_{RP}$} & {$PID_U$} \\
    \hline
    \end{tabular}
    \label{tbl:compare}
\end{table}
