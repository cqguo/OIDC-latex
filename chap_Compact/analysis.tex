...
As described in the threat model in Section \ref{subsec:threatmodel}, a malicious user, by colluding with malicious RPs, attempts to (\emph{a}) impersonate an honest user to log into an honest RP; (\emph{b}) tamper with the login session of an honest user to an honest RP such that the honest user is authenticated as another user. 
...
honest RP as an honest user; and {\bf (B)} the attacker should not be able to authenticate an honest user to an honest RP on behalf of another user (identity injection). For proof, we demonstrate that both properties are fulfilled for every $\mathcal{U\!W\!S}^{auth}$. 

To prove property (A), we first show that the attacker cannot manipulate the login session of any honest user to any honest RP, or steal any useful information from such login to impersonate the honest user at an honest RP. In particular, we show two properties of $\mathcal{U\!W\!S}^{auth}$ that a network attacker cannot alter (A1) encrypted communications between an honest user and an honest IdP over HTTPS; and (A2) encrypted communications between an honest IdP and an honest RP over HTTPS. The former means that $PID_{RP}$ in the identity token request cannot be intercepted or forged, while the latter indicates that a valid identity token cannot be intercepted or forged (i.e., {\em ID token confidentiality and integrity}). We adopted the general web system properties shown in the full version of \cite{FettKS14} and followed the approaches in \cite{BrowserID,SPRESSO}, which are based on a Dolev-Yao-style model, to prove the these two properties. The full proof is provide in Appendix~\ref{appendix-security}.

Meanwhile, when an honest user $U$ logs in to a malicious RP, it could share $U$'s identity token with a malicious user $U'$. If the malicious RP is able to find a $t'$ satisfying $[t']ID_{RP'}=[t]ID_{RP}$, the malicious user could log in to the honest $RP'$ on behalf of $U$ by using $t'$ and $U$'s identity token. However, as proved in Lemma 1 and Theorem 3,  the malicious RP knowing $t$ and $[t]ID_{RP}$ cannot find such a $t'$ and therefore $PID_{RP}$ in the identity token uniquely designates the RP associated with the login session with $t$ (i.e., {\em RP Designation}).
...
To prove property (B), we first show that the attacker cannot manipulate the login session of an honest user to an honest RP. We show a property of $\mathcal{U\!W\!S}^{auth}$ that a network attacker cannot tamper with $ID_{RP}$ that is extracted from the RP's certificate. Then, we show that the attacker cannot manipulate $PID_{RP}$ in the identity token request based on the property (A1) nor the identity token issued by the IdP to the honest user and the target RP based on the property (A2).

Meanwhile, we show that the identity token with pseudo-identity of the user uniquely identifies a user account at the designated RP, as proved in Theorem 2 (i.e., {\em User Identification}).  \hfill $\square$
...
\vspace{2mm}
Next, we prove that \usso\ provides the ``user identification'' and ``RP designation'' properties that are used in Theorem 1. First, let us consider an identity token $TK$ binding $PID_{RP}$ and $PID_U$, which is generated by the IdP upon a request from an authenticated user with $ID_U$.
