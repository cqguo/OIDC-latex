...
From each login request, an RP learns an ephemeral identifier $PID_{U}$ and a permanent identifier $Acct = [ID_U]ID_{RP}$ of the user, but it cannot derive $ID_U$ from $Acct$ due to the ECDLP problem.
Therefore, the colluding adversaries share information about the logins to malicious RPs and/or initiated by malicious users,
...
When the  $c$ RPs collude with $v$ malicious users,
 the logins initiated by these malicious users are picked out and linked together as:
...
\noindent where $L^m_{i, j}=([r_j]G, t_{i,j}, [u_ir_j]G)$ for $1 \le i \le v$ and $1 \le j \le c$,
 and $L^m_{i,j} \in \mathbb{L}$.
Any other login in $\mathbb{L}$ but not linked in $\mathfrak{L}^m$ is initiated by an honest user to some malicious RP.
...
Next, we prove that the colluding adversaries cannot decide if $u'$ is in $\mathbf{u}_w$ or not (i.e., $u'$ is randomly selected from the universal user set). This indicates that the colluding adversaries cannot link $L'$ to another login or even any subset of $\mathbb{L}''$.

We first define an RP-based linkage game $\mathcal{G}_r$ between an adversary and a challenger, which describes this RP-based login linkage privacy threat: the adversary receives $\mathfrak{L}^m$, $L'$, and $\mathbb{L}''$ from the challenger and outputs $s$, where $s = 1$ if it decides $u'$ is in $\mathbf{u}_w$ %$\{{U''_1}, {U''_2}, \cdots, {U''_w}\}$
...
(3) The challenger chooses random numbers in $\mathbb{Z}_n$ to construct $\{u''_k\}$ and $\{t''_k\}$ for $1 \leq k \leq w$,
 from which it assembles $\mathbb{L}'' = \{L''_{k}\} = \{(Q_2, t''_k, [u''_k]Q_2) = ([y]G, t''_k, [u''_ky]G)\}$.
In this process, the challenger ensures that $[u''_k]G \neq Q_1$ (i.e., $u''_k \neq x$) and $u''_k \neq u_i$ for any $1 \le i \le v$.
Then, it randomly chooses $d \in [1, w]$ and replaces $L''_{d}$ with $(Q_2, t''_d, Q_3) = ([y]G, t''_d, [z]G)$. Now, the challenger constructs $\mathbb{L}'' = \{L_{k;1\leq k \leq w}\}$, representing the logins from $w$ users, i.e., $\mathbf{u}_w=\{u''_1, u''_2, \cdots, u''_{d-1}, z/y, u''_{d+1}, \cdots, u''_w\}$. (4) When the adversary receives $\mathfrak{L}^m$, $L'$, and $\mathbb{L}''$, it returns $s$ as the output of $\mathcal{D}^*_r$.
...
If the adversary has non-negligible advantage in $\mathcal{G}_r$, then $\mathbf{Adv}^*_{A}={\mathbf{Adv}}_{A}$ is non-negligible regardless of the security parameter $\lambda$. This violates the ECDDH assumption. Therefore, the adversary has no advantage in $\mathcal{G}_r$ and cannot decide whether $L'$ is initiated by a user in $\mathbf{u}_w$ or in the universal user set.
