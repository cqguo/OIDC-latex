...
At the beginning, let us see the data exposed to an RP.
...
Finally, the effective date collected by the RP during the SSO login flow is $\langle ID_{RP}, t, PID_U\rangle$,
...
Here, we can define the game, the adversary owns the same ability as collusive RPs.
...
The $s$ is set 1, when adversary guess that $ID_U = ID_U'$, otherwise 0.
...
The challenger receives the input and set $ID_{RP_j}=X$, $ID_{RP_{j'}}=Y$, $PID_{U,j}=[t_j]Z$, and $PID_{U',j'}= [t_{j'}] N$.
The challenger input $<X, t_j, [t_j]Z>$ $<Y, t_{j'}, [t_{j'}] N>$ to the game. the adversary returns $r$.
At the end, $D$ returns the $r$ from the adversary as the result.

Now, we let ($P$, $aP$, $bP$, $abP$) and  ($P$, $aP$, $bP$, $cP$) be two inputs.
Thus there is
\begin{multline*}
\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ Pr[D(P,aP,bP,abP)=1]=\\ Pr[A(P, t_j, [t_j][b]P, aP, t_{j'},[t_{j'}][ab]P)=1]=Pr_1\\\\
Pr[D(P,aP,bP,cP)=1]=\ \ \ \ \ \ \ \ \ \
\\ Pr[A(P, t_j, [t_j][b]P, aP, t_{j'},[t_{j'}][c/a] [a]P)=1]=Pr_2
\end{multline*}
in the first equation $ID_{U} $ and $ ID_{U'}$ all equal to $b$, while in the second equation $ID_{U} =b$ does not equal to $ID_{U'} = c/a$.
Due to DDH, we have
\begin{equation}
Pr_1-Pr_2=\sigma(n)
\end{equation}
