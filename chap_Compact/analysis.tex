...
\item \textbf{RP designation. }IdP issues the identity token for specific RP(s), so that only the target RP would accept the identity token from user. Otherwise, when a malicious receives a valid identity token from an honest user, it can login other RPs as this user with this identity token.
