...
\vspace{0.75mm}
\noindent\textsc{Proof.}
We define an RP-based login-linkage game $\mathcal{G}_r$ between an adversary and a challenger,
...
Upon receiving an input, the challenger chooses
...
According to the construction of $\mathfrak{L}$, $L'$, and $\mathbb{L}''$, 
 $x$ is embedded as $ID_{U'}$ in $L'$ visiting $RP_{j'}$,
 and $z/y$ is embedded as $ID_{U''_d}$ into $\mathbb{L}''$ visiting $[y]G$ with $\{u''_1, u''_2, \cdots, u''_{d-1}, u''_{d+1}, \cdots, u''_w\}$, where $u''_k \neq x$.
Meanwhile, $[r_{j'}]G$ and $[y]G$ two different malicious RPs, 
    and $x$ is not in $\{u''_1, u''_2, \cdots, u''_{d-1}, u''_{d+1}, \cdots, u''_w\}$.
