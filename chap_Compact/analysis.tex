...
Moreover, adversaries cannot retrieve $PID_{RP}$ from a user's login to any honest RP,
    as the user and the RP would not send $PID_{RP}$ and $t$ to any adversary party.
Therefore, even the adversary lead the honest to send the adversary's token to an honest RP, the RP would not accept this token anyway.
...
The details about Dolev-Yao style web model and security proof of UPPRESSO is  shown in the Appendix.
