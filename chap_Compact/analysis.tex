...
An adversary might try allure an honest user to login under the adversary's account,
    by injecting his identity token into the user's communications with the RP.
Such identity injection attacks is impossible in UPPRESSO as follows.
...
Even when $PID_{RP}$ has been negotiated,
    the adversary cannot eavesdrop $PID_{RP}$ because the communications between RP script and IdP script are protected by browser, and the communications between browser and servers are protected by TLS.
