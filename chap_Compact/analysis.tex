...
\newc In each login instance, the IdP receives an identity-token request from a user (Step 3.1 in Section~\ref{implementations}),
 which reveals only the user's identity $ID_U$ and the RP's ephemeral pseudo-identity $PID_{RP}$.
...
\noindent{\textsc{Theorem 6. (Privacy against the IdP)}} {\em In UPPRESSO the IdP cannot distinguish $PID_{RP} = [t]ID_{RP}$
    from random variables, where $t$ is random in $\mathbb{Z}_n$.} %and unknown to the IdP.}
...
enclosing $PID_{RP}$ and $PID_U$. Meanwhile, the RP knows its own identity $ID_{RP}$ and the user's account $Acct$.
...
Since $PID_{RP}$ and $PID_U$ can be deduced from $ID_{RP}$ and $Acct$ with $t$, respectively,
 an RP gains unique knowledge about a user when she logins, denoted as $L=(ID_{RP}, t, Acct)$.
...
Malicious RPs, colluding with malicious users, attempt to link login instances including accounts
        of honest users across these malicious RPs.
We prove that UPPRESSO prevents RP-based identity linkage, by showing that 
    given a login instance $L'$ by an honest user $U'$ to one of the $c+1$ malicious RP,
    the adversaries cannot distinguish a login instance by $U'$ to other $c$ malicious RP,
        from the login instances by any honest users to other $c$ malicious RPs.

The login instances to $c$ malicious RPs by $v$ colluding users
    are $\mathfrak{L}=\left \{ \begin{matrix}
...
and the login instance to another RP is denoted as $L' = (ID_{RP'}, t', [ID_{U'}]ID_{RP'})$.
...
%
First,
 consider a login instance $L'$ by an honest user $U'$ to one of the $c+1$ malicious RPs, without loss of generality, denoted as $RP_{c+1}$, and any $RP_j$ ($j \in [1,c]$) of the other $c$ malicious RPs.
According to Lemma 2, $RP_j$ and $RP_{c+1}$ cannot decide whether a login instance to $RP_{c+1}$,
        is initiated by some users visiting $RP_j$ or not.
Thus,
    for each  $RP_j$,
    $RP_j$ and $RP_{c+1}$ cannot distinguish a login instance by an honest user $U'$ to one of the two RP,
        from the login instances by any honest users to the other RP.

Therefore, the $c+1$ malicious RPs cannot distinguish a login instance by an honest user $U'$ to $RP_{c+1}$,
        from the login instances by any honest users to other $c$ malicious RPs.
Therefore,  
    $c+1$ colluding RPs cannot distinguish a login instance by an honest user $U'$ to one of the $c+1$ RP,
        from the login instances by any honest users to the other $c$ RPs.

Next, when colluding with $v$ malicious users,
    according to Lemma 3, $RP$ and $RP'$ cannot decide whether a login instance to $RP'$ denoted as $L' = (ID_{RP'}, t', [ID_{U'}]ID_{RP'})$,
        belong to one of the $v$ users visiting to $RP$ or not (i.e., $ID_{U'} \in \{u_i\}$ or $\mathbb{Z}_n$).

Furthermore, we need to prove that,
    even when more RPs are colluding with some malicious users,
    the collusion does not bring any benefits to the adversaries in the RP-based identity linkage:
given a matrix of login instances to $c$ RPs by $v$ users,
        where these login instances are linked across the $c$ RPs with the help from the $v$ colluding users,
    the adversaries still cannot distinguish a login instance to another RP that is initiated by some of the $v$ users
                    from login instances by a user randomly chosen in the system.
Therefore,
        malicious RPs cannot collude with malicious users cannot link login instances across the RPs,
            and RP-based identity linkage is prevented in UPPRESSO.
...
\noindent\textsc{Lemma 2.} {\em  For two RPs, $RP'$ and $RP$,
