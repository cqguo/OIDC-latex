...
When $c+1$ malicious RPs and $v$ malicious users collude in \usso, given a login instance $\mathcal{L'}$ initiated by an honest user $U'$ to a malicious RP, the adversaries cannot distinguish a login instance by $U'$ from the ones initiated by any other honest users to any of the other $c$ malicious RPs.}
