...
Therefore, the colluding adversaries share information about the logins to malicious RPs {\bf and/or initiated by malicious users,} % not clear.
{\bf attempting to link multiple logins initiated by some honest user to multiple malicious RPs.} % repeated, said in the first sentence
...
\end{matrix}\right\}\]
\noindent where $L^m_{i, j}=([r_j]G, t_{i,j}, [u_ir_j]G)$ for $1 \le i \le v$ and $1 \le j \le c$, and $L^m_{i,j} \in \mathbb{L}$. Any login in $\mathbb{L}$ but not linked in $\mathfrak{L}^m$ is initiated by an honest user to one of the $c$ malicious RPs.
...
Consider any subset $\mathbb{L}''$ of $w$ logins visiting $RP_b$ by unknown honest users, we denote the corresponding honest users who initiate these logins as $\mathbf{u}_w=\{{u''_1}, {u''_2}, \cdots, {u''_w}\}$.
Next, we prove that the colluding adversaries cannot decide if $u'$ is in $\mathbf{u}_w$ or not (i.e., $u'$ is randomly selected from the universal user set). This indicates that the colluding adversaries cannot link $L'$ to another login in $\mathbb{L}''$ or even link $L'$ to  any subset of $\mathbb{L}''$.
...
Then, it randomly chooses $d \in [1, w]$ and replaces $L''_{d}$ with $(Q_2, t''_d, Q_3) = ([y]G, t''_d, [z]G)$. Finally, it constructs $\mathbb{L}'' = \{L_{k;1\leq k \leq w}\}$, representing the logins from $w$ users, i.e., $\mathbf{u}_w=\{u''_1, u''_2, \cdots, u''_{d-1}, z/y, u''_{d+1}, \cdots, u''_w\}$. (4) When the adversary receives $\mathfrak{L}^m$, $L'$, and $\mathbb{L}''$, it returns $s$ as the output of $\mathcal{D}^*_r$.
