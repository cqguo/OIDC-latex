...
\vspace{0.75mm}
\noindent{\textsc{Proof.} Without loss of generality, let us consider an arbitrary login instance in which a user with $ID_{U'}=u'$ logs in to the $c$th malicious RP. The RP obtains $L'=(ID_{RP_c}, t', Acct'_c)$, where $ID_{RP_c}=[r_c]G$ and $Acct'_c=[u'r_c]G$. If this user previously logged in to at least one colluding RP (e.g., $RP_x$), she should be in $\mathbb{U}_c$, and her login information should be in $\mathbb{V}_x$. Since $x$ is unknown, in RP-based linkage, $RP_c$ attempts to associate $L'$ with any record $L^*$ in $\mathbb{V}_j$s, where $L^*=([r_k]G, t_{u'k}, [u'r_k]G)$ and $k \in [1,c]$. 
However, according to Lemma 2, $([r_c]G, t', [u'r_c]G)$ is indistinguishable from $([r_k]G, t_{u'k}, [u'r_k]G)$. Therefore, $RP_c$ cannot tell if the user with $ID_{U'}$ is in the set $\mathbb{U}_c$ nor link it with any other user logins. 

Besides, when a malicious user ($ID_{U''}=u''$) logs in to $RP_c$ while not revealing his identity, $RP_c$ cannot tell if he is in the malicious user group $\{u_1,...,u_v\}$, according to Lemma 3.
