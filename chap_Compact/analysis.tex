...
We develop a Dolev-Yao style model \cite{BrowserID, SPRESSO, FettKS16, FettKS17} for \usso, referred to as the \dyu~model, to formalize the login flow of \usso. Dolev-Yao style models abstract cryptographic concepts into an algebra of symbolic messages to discover structural flaws using simple formal logic. % which has been used in the formal analysis of SSO protocols such as OAuth 2.0 \cite{FettKS16} and OIDC \cite{FettKS17}.

The \dyu~model abstracts the entities in a web system, such as web servers and browsers, as \emph{atomic processes}, %which communicate with each other through events. % such as HTTPS requests and responses.
...
Applying the DYU model, we confirm the following conclusions:
 ...
 (\emph{b}) pseudo-identities and other elements in verified identity tokens cannot be manipulated by any malicious entity;
    (\emph{c}) the IdP's public key set in the IdP script cannot be replaced by any malicious process, for an honest browser;
 (\emph{d}) the IdP does not receive anything about $t$ shared between the user and the RP;
 (\emph{e}) the RPs does not receive anything about $u$ shared between the user and the IdP;
...
It is computationally easy for any party who knows $ID_{RP}$ and $t$ to validate the $PID_{RP}$ in an identity token. A valid $PID_{RP}$ always specifies an RP in the system. %$PID_{RP}$ sent by a user in her identity-token request is calculated as $PID_{RP} = [t]ID_{RP}$, where $ID_{RP}$ is the target RP's identity and $t$ is a random number selected by the user and shared with this RP.
...
\vspace{3mm}
\noindent{\textsc{Theorem 4. (Identity Token Confidentiality)}} {\em An identity token can only be accessed by its designated RP, besides the requesting user and the IdP.}
...
Following the login flow of \usso, when the confidentiality and integrity of identity tokens are satisfied, no adversary could obtain a valid identity token that is issued to an honest user for accessing an honest RP. %and accepted by an honest RP.
...
\vspace{3mm}
\noindent{\textsc{Theorem 6. (Privacy against the IdP)}} {\em In \usso, the IdP cannot distinguish $PID_{RP} = [t]ID_{RP}$ from any random variable on $\mathbb{E}$, where $t$ is random in $\mathbb{Z}_n$.} %and unknown to the IdP.}
...
Then, we consider the third adversarial scenario where $c+1$ malicious RPs collude with $v$ malicious users attempting to link logins of honest users across these malicious RPs.
That is, given an arbitrary login $\mathcal{L'}$ initiated by an honest user $U'$ to one of the $c+1$ malicious RPs, the colluding adversaries aim to link $\mathcal{L'}$ to $U'$'s other logins to the remaining $c$ malicious RPs. To show that \usso~prevents such linkage, we will prove in Theorem 7 that the colluding adversaries cannot distinguish the logins initiated by $U'$ from the ones initiated by any other honest users, requesting to log into any of the remaining $c$ malicious RPs. As a result, given $\mathcal{L'}$, the adversaries have no clue to link it to one login that has been observed by a colluding RP instead of another.
...
Let us define the knowledge that an RP obtains from each login instance. In \usso, an RP receives a random number $t$ and an identity token enclosing $PID_{RP}$ and $PID_U$ in each login instance. Meanwhile, it knows its own identity $ID_{RP}$ and derives the user's account $Acct$. Since $PID_{RP}$ and $PID_U$ can be deduced from $ID_{RP}$ and $Acct$ with $t$, respectively, we ignore the duplicate elements and define an RP's knowledge about a login instance $\mathcal{L}$ as $L=(ID_{RP}, t, Acct)=(ID_{RP}, t, [ID_{U}]ID_{RP})$. %Note that a login instance $L$ includes the user's account at the RP.
When $v$ malicious users collude with malicious RPs, they could notify the RPs about their own login instances. As a result, the colluding RPs could further associate their knowledge to form a shared view: %, denoted by
...
% L_{v,1},&L_{v,2},&\cdots,&L_{v,c}
% \end{matrix}\right\}$
\noindent where $L_{i,j} = (ID_{RP_j}, t_{i,j}, [ID_{U_i}]ID_{RP_j})$ is about the login initiated by $U_i$ ($1 \le i \le v$) to $RP_j$ ($1 \le j \le c$). %We denote the login initiated by $U'$ to the $c+1$th RP as $L' = (ID_{RP'}, t', [ID_{U'}]ID_{RP'})$.
...
When $c+1$ malicious RPs and $v$ malicious users collude in \usso, given a login instance $\mathcal{L'}$ initiated by an honest user $U'$ to a malicious RP, the adversaries cannot distinguish a login instance initiated by $U'$ from the ones initiated by any other honest users to any of the remaining $c$ malicious RPs.}
...
\vspace{0.75mm}
\noindent\textsc{Proof.}
Consider an arbitrary malicious RP, denoted as $RP'$, among $c+1$ colluding RPs. When an honest user $U'$ initiates a login $\mathcal{L'}$ to $RP'$, it obtains $L' = (ID_{RP'}, t', [ID_{U'}]ID_{RP'})$. Suppose one of the remaining malicious RP, denoted as $RP_j$ colludes with $RP'$ and shares its knowledge about all the $w$ logins it has observed, denoted as $\mathbb{L} = \{{L_i}\}$, where $L_i = ([r]G, t_i, [u_i r]G)$ and $1 \leq i \leq w$. $\mathbb{L}$ is about the logins that may potentially be initiated by any honest or malicious users. 

First, let us assume the malicious users have not colluded with the malicious RPs by not revealing additional information about their own $PID_U$s. Therefore, $RP'$ does not know some logins in $w$ are initiated by the same malicious users. According to Lemma 2, $RP'$ and $RP_j$ cannot distinguish if any of the logins among $w$ is initiated by $U'$ or by another honest user in the system, given $L'$ and $\mathbb{L}$.


%Let us denote $c$ malicious RPs as $RP_j$ ($1 \leq j \leq c$) and the $(c+1)$th RP as $RP'$.  

///We first consider a malicious RP denoted as $RP'$ that receives a login $L'$ by an honest user $U'$, the other $c$ malicious RPs denoted as $RP_j$ ($1 \leq j \leq c$), and no malicious user.
According to Lemma 2, a pair of $RP_j$ and $RP'$ cannot decide whether a login initiated by $U'$ to $RP'$ is initiated by one of the users visiting $RP_j$ or not.

After $RP'$ colludes with each $RP_j$ ($1 \leq j \leq c$),  we find that, given $L'$ initiated by $U'$ to $RP'$, the adversaries cannot distinguish a login by $U'$ to any $RP_j$, from the logins by any users to $RP_j$ ($1 \leq j \leq c$).
Therefore, given a login $L'$ by $U'$ to a malicious RP, the adversaries cannot distinguish a login by $U'$ to any of the other $c$ malicious RPs, from the logins by any users to the $c$ malicious RPs.

Next, when colluding with $v$ malicious users, according to Lemma 3, $RP_j$ ($1 \leq j \leq c$) and $RP'$ cannot decide whether a login by $U'$ to $RP'$ is initiated by one of the $v$ malicious users visiting $RP_j$ ($1 \leq j \leq c$) or not, unless $U'$ is malicious and reveal his identity to $RP'$.
So, the collusion with malicious users does not bring benefits for the adversaries, to link an honest user's logins across colluding RPs. So UPPRESSO prevents RP-based identity linkage, even when malicious RPs are colluding with malicious users.
...
\noindent\textsc{Lemma 2.} {\em Given the logins to $RP$ by $v$ users denoted as $\mathbb{L} = \{{L_i}\}$, where $L_i = ([r]G, t_i, [u_i r]G)$ and $1 \leq i \leq v$,
    two colluding RPs, $RP$ and $RP'$, cannot decide whether a login to $RP'$ denoted as $L' = (ID_{RP'}, t', [ID_{U'}]ID_{RP'})$, is initiated by one of the $v$ users or not, i.e., $ID_{U'} \in \{u_i\}$ ($1 \leq i \leq v$) or $ID_{U'} \in \mathbb{Z}_n$.}



\vspace{0.75mm}
\noindent{\textsc{Proof.} We define a game $\mathcal{G}_r$ between an adversary and a challenger,
    to describe the RP-based identity linkage:
 the adversary receives $\mathbb{L} = \{{L_i}\}$ and $L' = (ID_{RP'}, t', [ID_{U'}]ID_{RP'})$ from the challenger and outputs the result $s = 1$ if it decides $u' \in \{u_i\}$ or $s = 0$ if $u'$ is randomly chosen in $\mathbb{Z}_n$.
...
If the probability of distinguishing a login initiated by a user $U'$ chosen from \{${U_1}, {U_2}, \cdots, {U_v}$\} or randomly from the universal user set is non-negligible,
...
We define a game $\mathcal{G}_r$ between an adversary and a challenger to describe the linkage:
...
The probability of solving the ECDDH problem using $\mathcal{D}^*_r$ is defined as $\epsilon_{r}(k)$, and
% = |{\rm Pr}^*_1 - {\rm Pr}^*_2|$, where:
%\begin{align*}
%&{\rm Pr}^*_1 =  {\rm Pr}\{\mathcal{D}^*(G, [x]G, [y]G, [xy]G)=1\} \\
%&{\rm Pr}^*_2 =  {\rm Pr}\{\mathcal{D}^*(G, [x]G, [y]G, [z]G)=1\}
%\end{align*}
$\epsilon_{r}(k)$ becomes negligible when the security parameter $k$ is sufficiently large.
...
Upon receiving ($G, Q_1=[x]G, Q_2=[y]G, Q_3=[z]G$), the challenger first chooses $\{u_1, u_2, \cdots, u_v\}$, $\{r_1, r_2, \cdots, r_c\}$, $\{t_{1, 1}, t_{1, 2}, \cdots, t_{v, c}\}$, and $t'$ randomly in $\mathbb{Z}_n$ and assigns $L_{i, j} = ([r_j]G, t_{i, j}, [u_ir_j]G)$. Then, it randomly chooses $d \in [1, v]$ to replace $[u_d r_j]G$ with $[r_j]Q_1=[xr_j]G$ for $1\leq j \leq c$,
 to form $\mathfrak{L}=\left \{ \begin{matrix}
L_{1,1},&L_{1,2},&\cdots,&L_{1,c}\\
L_{2,1},& L_{2,2},&\cdots,&L_{2,c}\\
\cdots,&\cdots,&L_{i,j},&\cdots\\
...
%
Finally, it assigns $L' = (Q_2, t', Q_3) = ([y]G, t', [z/y][y]G)$. The challenger sends $\mathfrak{L}$ and $L'$ to the adversary, which outputs $s$. According to the above construction of $\mathfrak{L}$ and $L'$,
...
The probability $\epsilon_{r}(k)$ of solving the ECDDH problem using $\mathcal{D}^*_r$ is deduced as:
\begin{equation*}
\epsilon_{r}(k) = |{\rm Pr}_1-{\rm Pr}_2|={\mathbf{Adv}}_{A}
\end{equation*}

The ECDDH assumption means that in $\mathcal{G}_r$ the adversary does not have advantages, i.e., cannot distinguish a user chosen from \{${U_1}, {U_2}, \cdots, {U_v}$\} or from the universal user set.
