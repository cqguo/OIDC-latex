...
Thus, the probability of finding $t$ and $t'$ satisfying that $[t]ID_{RP_j} = [t']ID_{RP_{j'}}$ in UPPRESSO is negligible,
    and then adversaries cannot break RP designation. $\square$
...
\vspace{1mm}
\noindent\textbf{Integrity.}~~An honest RP accepts only identity tokens
 binding its pseudo-identity $PID_{RP}$ and the authenticated user's pseudo-identity $PID_U$,
 and also binding $ID_{RP}$ and $Acct=[ID_U]ID_{RP}$.

\vspace{0.5mm}
\noindent\emph{Proof.}
The identity token binds $PID_{RP} = [t]ID_{RP}$ and $PID_U = [ID_U]PID_{RP}$,
...
\vspace{1mm}
We also analyzed the properties of UPPRESSO, %especially confidentiality and integrity,
...
So the prevention against the IdP-based login tracing
    is expressed as below.

\vspace{1mm}
\noindent\textbf{Privacy against the IdP.}~~The IdP
 cannot infer any information about $ID_{RP}$ or link any pair of $PID_{RP}^i$ and $PID_{RP}^{i'}$
  ($i \neq i'$),
    from an honest user's all identity token requests for $PID_{RP}^i$ ($i = 1, 2, \cdots$).

\vspace{0.5mm}
\noindent\emph{Proof.}
Because (\emph{a}) $ID_{RP} = [r]G$ is also a generator of order $n$,
        where $G$ is a generator of finite cyclic group $\mathbb{E}$,
    and (\emph{b}) $t$ is kept secret to the IdP,
the IdP cannot any information about $ID_{RP}$ from $PID_{RP} = [t]ID_{RP}$ due to the ECDLP.
Next, because $t$ is a random number from $\mathbb{Z}_n$,
from the IdP's view,
$PID_{RP}$
is \emph{indistinguishable} from a variable randomly chosen from $\mathbb{E}$.
Thus, 
    it cannot distinguish $[t]ID_{RP_j} = [tr]G$ from any other $[t']ID_{RP_{j'}} = [t'r']G$.
So the IdP-based login tracing is computationally impossible. $\square$
...
So the prevention against the RP-based identity linkage in UPPRESSO is expressed
    as below.

\vspace{1mm}
\noindent\textbf{Prevention against colluding RPs.}~~After
 collecting the information of login instances by $v$ users,
$c$ colluding RPs cannot decide whether a login instance to another RP is launched by one of these $v$ users or not,
    where
...
So the RP-based identity linkage is computationally impossible. $\square$
