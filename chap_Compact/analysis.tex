...
The adversary receive the input $\langle ID_{RP_j}$, $t_j$, $[ID_U][t_j]{ID_{RP_j}}\rangle$,
    $\langle ID_{RP_{j'}}$, $t_{j'}$, $[ID_{U'}] [t_{j'}] {ID_{RP_{j'}}}\rangle$, from the challenger, and returns the result $r$.
The $r$ is set 1, when adversary guess that $ID_U$ equals $ID_U'$, otherwise 0.
...
We define $Pr_1$ is the probability, while the adversary returns $r=1$ as $ID_U$ equals to $ID_{U'}$.
 And $Pr_2$ is the probability, while the adversary returns $r=1$ but $ID_U$ does not equal to $ID_{U'}$.
...
Thus there is
\begin{multline*}
\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ Pr[D(P,aP,bP,abP)=1]=\\ Pr[A(P, t_j, b \cdot{N_{U1}P}, aP, N_{U2},b\cdot{N_{U2} aP})=1]=Pr_1\\
Pr[D(P,aP,bP,cP)=1]=\ \ \ \ \ \ \ \ \ \
