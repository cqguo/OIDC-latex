...
This negligible probability means the identity token designates \emph{only} the target RP.

Let $\mathbb{E}$ be an elliptic curve over a finite field $\mathbb{F}_q$,
    $G$ be a point on $\mathbb{E}$ of order $n$,
        and $Q = [x]G$ where $x$ is an integer number randomly  chosen from $\mathbb{Z}_n$.
Given $G$ and $Q$,
    the probability that a probabilistic polynomial time (PPT) algorithm $\mathcal{D}$ calculates $x$ (i.e., to solve the ECDLP) is negligible.
...
Note that $m$ is a finite integer number, and $m \ll 2^k$ with the increasing $k$.
...
We then design a PPT algorithm $\mathcal{D}^*_c$ based on this game, shown in Figure~\ref{fig:ecdlp_algorithm}, to solve the ECDLP as below.
The input of $\mathcal{D}^*_c$ is in the form of ($G$, $Q$).
On receiving the input, the challenger of $\mathcal{G}_c$ randomly chooses $r_1, r_2, \cdots, r_m \in \mathbb{Z}_n$,
...
Then,
as $m \ll 2^k$, ${\rm Pr}\{\mathcal{D}^*_c(G, [x]G)=x\} = \frac{1}{m}{\rm Pr}_s$ also
becomes non-negligible with the increasing security parameter $k$.
This violates the ECDLP.
Therefore, the probability of finding $t$ and $t'$ satisfying that $[t]ID_{RP_j} = [t']ID_{RP_{j'}}$ in UPPRESSO is negligible,
...
Finally, we also formally analyze the security properties of UPPRESSO, %especially confidentiality and integrity, 
     based on an Dolev-Yao style model \cite{SPRESSO}.
...
The Dolev-Yao style model abstracts the entities in a web system,
    ...
    as atomic processes.
    , which communicate with each other through events.% such as HTTPS request and response.
It also defines script processes to formulate client-side scripts, i.e.,  JavaScript code.
The script is dependently invoked by the browser to process the server-defined logic, such as verifying $Certificate_{RP}$.
...
The UPPRESSO system contains an IdP process,
    ...
    The processes communicate with each other through events such as HTTPS request and response.
We consider all RP processes and browser processes are honest,
 while model an RP or a browser controlled by an adversary as attacker atomic processes.
 Inside the honest browsers, the honest IdP's script, honest RPs' scripts and attackers' scripts are deployed. Although the scripts coexist in the same browser, they are strictly separated. In a browser, the scripts can communicated with each other through postMessage mechanism. To clearly indicate the action of postMessage communication, we define it as the transmitting-to-itself event of the browser (which is not defined in SPRESSO).
...
We locate the generation of an identity token in UPPRESSO, and trace to the places
    where $PID_U$, $PID_{RP}$ and other parameters enclosed in this token are calculated and transmitted,
...
Because  (\emph{a}) $t$ is a number randomly chosen from $\mathbb{Z}_n$ by the user and kept secret to the IdP
...
is negligible,
 where $x$, $y$ and $z$ are integer numbers randomly and independently chosen from $\mathbb{Z}_n$.
Let  ${\rm Pr}\{\}$ denote the probability and
 we define
\begin{align*}
&{\rm Pr}_1 =  {\rm Pr}\{\mathcal{D}(G, [x]G, [y]G, [xy]G)=1\} \\
&{\rm Pr}_2 =  {\rm Pr}\{\mathcal{D}(G, [x]G, [y]G, [z]G)=1\} \\
& \epsilon_{r}(k) = |{\rm Pr}_1 - {\rm Pr}_2|
\end{align*}
Then, $\epsilon_{r}(k)$ becomes negligible with the increasing security parameter $k$.
...
After filtering out the redundant information (i.e., $PID_{RP}= [t]{ID_{RP}}$ and $Acct = [t^{-1}]PID_{U}$),
    the RP actually receives $(ID_{RP}, t, Acct)$ in an UPPRESSO login instance, where $ID_{RP} = [r]G$ and $Acct = [ID_U]{ID_{RP}} = [ur]G$.
...
when $c$ collusive RPs collect the information of login instances by $v$ users,
    they cannot determine whether a login instance to another RP belongs to one of these $v$ users or not.
...
we define the RP-based identity linkage game $\mathcal{G}_r$  as follows:
...
The ECDDH assumption indicates that in $\mathcal{G}_r$ the adversary does not have advantages,
