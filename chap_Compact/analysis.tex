...
Let us consider an \emph{honest} user $U'$ who initiates a login $\mathcal{L'}$ to  one malicious RP.
All the colluding entities, including this RP,
 aim to associate $\mathcal{L'}$ with any other logins by $U'$ to any of the other $c$ colluding RPs.
...
To show that \usso~prevents such linkage, we prove in Theorem 7 that the adversaries cannot {\em distinguish} logins initiated by $U'$ to one of the $c$ colluding RPs from the ones initiated by any other honest users to these $c$ RPs. %a login initiated by $U'$ to some of the $c$ malicious RPs from the ones initiated by any other honest users to the $c$ malicious RPs.
As these logins are \emph{indistinguishable} from each other, given $\mathcal{L'}$ and a shared view of all the logins to the $c$ colluding RPs, the adversaries have no clue to link $\mathcal{L'}$ to one login instead of another.
They even cannot link $\mathcal{L'}$ to one subset of the shared logins instead of another subset.
...
Because $PID_{RP}$ and $PID_U$ can be deduced from $ID_{RP}$ and $Acct$ with $t$, respectively, we ignore duplicated elements and define an RP's knowledge about a login $\mathcal{L}$ as $L=(ID_{RP}, t, Acct)=(ID_{RP}, t, [ID_{U}]ID_{RP})$. %Note that a login $L$ includes the user's account at the RP.
...
\vspace{3mm}
\noindent \textsc{Theorem 7. (Privacy against Colluding RPs)} {Given a login initiated by an honest user to a malicious RP, this RP colluding with other $c$ malicious RPs and $v$ malicious users, cannot distinguish if a login to any of the $c$ malicious RPs is initiated by this user or another honest user.}
...
\vspace{0.75mm}
\noindent\textsc{Proof.}
When a malicious RP, denoted as $RP'$, receives and verifies an identity token from an honest user, denoted as $U'$, it obtains $L' = (ID_{RP'}, t', [ID_{U'}]ID_{RP'})$. Suppose another malicious RP, denoted as $RP_j$, colludes with $RP'$ and shares the knowledge of its logins, i.e., $\mathbb{L} = \{{L_{i}}\}$ and $1 \leq i \leq w$. We denote the users who initiate these $w$ logins as $\mathcal{U}_j=\mathcal{U}_{j,H}\cup \mathcal{U}_{j,M}$, where $\mathcal{U}_{j,H}$ and $\mathcal{U}_{j,M}$ represent the set of honest and malicious users, respectively.
...
Let us first assume that malicious users do not collude with malicious RPs. So, the colluding RPs cannot associate the logins initiated by the same malicious user. %so they do not reveal their accounts. As a result, even if $RP_j$ and $RP'$ collude, they are not told if some of the logins are initiated by the same malicious user.
According to Lemma 2, given $L'$ and any arbitrary subset of $\mathbb{L}$, $RP'$ cannot decide if $U'$ is one of the users in the corresponding subset of $\mathcal{U}_j$.
...
This indicates any login in $\mathbb{L}$ is \emph{indistinguishable} from other logins in $\mathbb{L}$, to be associated with a given $\mathcal{L}'$.
...
The advantage $\mathbf{Adv}^*_{A}=|{\rm Pr}^*_1 - {\rm Pr}^*_2|$ of solving the ECDDH problem using $\mathcal{D}^*_r$ is deduced as:
\begin{align*}
...
If the adversary has advantages in $\mathcal{G}_r$ to decide whether $L'$ is initiated by $U'$ chosen from \{${U_1}, {U_2}, \cdots, {U_w}$\} or randomly from the universal user set,
    then $\mathbf{Adv}^*_{A}={\mathbf{Adv}}_{A}$ is \emph{non-negligible} regardless of $\lambda$.
...
Thus, according to the ECDDH assumption,
    the advantage of using $\mathcal{D}^*_r$ to decide whether $z$ equals to $xy$ or not,
    i.e., $\mathbf{Adv}^*_{A} = \epsilon_{r}(\lambda)$, becomes negligible when the security parameter $\lambda$ is sufficiently large.

%Thus, $|{\rm Pr}^*_1 - {\rm Pr}^*_2| = \epsilon_{r}(\lambda)$ becomes negligible when $\lambda$ is sufficiently large, where
%\begin{align*}
%&{\rm Pr}^*_1 =  {\rm Pr}\{\mathcal{D}^*(G, [x]G, [y]G, [xy]G)=1\} \\
%&{\rm Pr}^*_2 =  {\rm Pr}\{\mathcal{D}^*(G, [x]G, [y]G, [z]G)=1\}
%\end{align*}
...
The advantage $\mathbf{Adv}^*_{A}$ of solving the ECDDH problem using $\mathcal{D}^*_r$ is deduced as:
\begin{equation*}
\mathbf{Adv}^*_{A} = |{\rm Pr}_1-{\rm Pr}_2|={\mathbf{Adv}}_{A}
\end{equation*}
