...
It is worth noting that these guarantees are proved against different types of adversaries.
...
\newc
For the analysis of security (w.r.t. authentication), we consider the formal model of \usso\ web systems (denoted as $\mathcal{U\!W\!S}^{Auth}$), which is defined in Appendix \ref{appendix-model}. It contains an IdP, a finite set of RPs, a finite set of users (i.e., browsers), and an attacker. The attacker subsumes all web attackers, who could corrupt a number of users and RPs.

Based on the threat model in Section \ref{subsec:threatmodel},
 the adversary against security (w.r.t. authentication) \cite{FettKS14,BrowserID,SPRESSO}
 considers
 a malicious user, by colluding with malicious RPs, attempts to (\emph{a}) impersonate an honest user to log into an honest RP (i.e., \emph{impersonation}), and (\emph{b}) intrude the login of an honest user to an honest RP so that the honest user is authenticated as another user (i.e., \emph{identity injection}). 
...
\noindent\textbf{\textsc{Proof.}} Every SSO system should satisfy two fundamental properties that are deduced from \cite{FettKS14} and summarized in \cite{BrowserID,SPRESSO}: {\bf (A)} the attacker should not be able to access services of an
honest RP as an honest user; and {\bf (B)} the attacker should not be able to authenticate an honest user to an honest RP on behalf of another user.
For the proofs, we demonstrate that both properties are fulfilled for every $\mathcal{U\!W\!S}^{Auth}$.

To prove Property {\bf (A)}, we first show that the attacker cannot obtain any useful information from any login of an honest user to an honest RP, which allows him to impersonate an honest user at some honest RP.
In particular, we show two properties of $\mathcal{U\!W\!S}^{Auth}$, i.e., a network attacker cannot break secure communications over HTTPS between {\bf (C1)} the honest IdP and an honest user (proved in Lemmas 2 and 3 in Appendix \ref{appendix-security}); and {\bf (C2)} the honest IdP and an honest RP (proved in Lemmas 4, 5, and 6 in Appendix \ref{appendix-security}). The former indicates that $PID_{RP}$ in the identity-token request of the honest user cannot be leaked to the attacker and the latter indicates that a valid identity token of the honest user cannot be leaked to the attacker (i.e., {\em Token Confidentiality}). We adopted the general web system properties shown in the full version of \cite{FettKS14} and followed the approaches in \cite{BrowserID,SPRESSO}, which are based on a Dolev-Yao-style model, to prove Properties {\bf (C1)} and {\bf (C2)}. A detailed proof is provide in Appendix \ref{appendix-security}.

Meanwhile, we prove that Property {\bf (A)} is still fulfilled in the following two adversarial scenarios. First, when a malicious user initiates a login to an honest RP, where he selects a random $t$ to share with this RP and then manipulates $PID_{RP}$ to be $[t']ID_{RP}$ instead of $[t]ID_{RP}$ when requesting for an identity token, we prove in Theorem \ref{thm-user-id} that, by showing the identity token with a manipulated $PID_{RP}$ to this honest RP, the malicious user cannot be recognized under the account of any existing honest user at this RP (i.e., {\em User Identification}). 

Finally, when a malicious $RP$ shares with a malicious user $U$ the identity token $TK$ that it received from an honest user $U'$. With $TK$, if the malicious $RP$ is able to find $t'$ satisfying $[t']ID_{RP'}=[t]ID_{RP}$ for any honest $RP'$, it could share $TK$ and $t'$ with the malicious user, enabling him to log into $RP'$ on behalf of $U$. We prove in Lemma \ref{lemma-rp} and Theorem \ref{thm-rp-designation} that a malicious RP receiving $t$ and $[t]ID_{RP}$ from $U$'s login cannot find such $t'$, and $PID_{RP}$ in $TK$ is uniquely associated with $t$ for the login to the designated RP (i.e., {\em RP Designation}).
...
To prove property {\bf (B)}, we first prove in Theorem \ref{thm-user-id}: when an honest user logs into an honest RP, the identity token $TK$ that binds $PID_U$ and $PID_{RP}$ uniquely identifies a user account for this user at the designated RP (i.e., {\em User Identification}). Therefore, an honest user will not be authenticated by an honest RP as any other users in the system.
...
In particular, we show a property of $\mathcal{U\!W\!S}^{Auth}$, i.e., a network attacker cannot tamper with $ID_{RP}$ extracted from the RP's certificate. 
Then, we show that the attacker cannot manipulate $PID_{RP}$ in the identity token request of the honest user 
(Lemmas 2 and 3 in Appendix \ref{appendix-security}), nor the identity token $TK$ issued by the IdP to the honest user and the target honest RP (Lemmas 4, 5 and 6 in Appendix \ref{appendix-security}). \hfill $\square$
...
\vspace{2mm}
Next, we prove that \usso\ provides the ``RP Designation'' and ``User Identification'' properties that are used in Theorem 1. Let us consider an identity token $TK$ binding $PID_{RP}$ and $PID_U$, which is generated by the IdP upon a request from an authenticated user with $ID_U$.
...
\newc
\begin{thm}
\textsc{(User Identification of $TK$)} \emph{$PID_U= [ID_U]PID_{RP}$ in $TK$ uniquely identifies an account at the RP designated by $PID_{RP}$ if and only if it receives $t$, where $PID_{RP} = [t]ID_{RP}$,
 and this account is uniquely mapped to a user with $ID_U$.}
\label{thm-user-id}
\end{thm}

\noindent\textbf{\textsc{Proof.}}
To issue an identity token requested for $PID_{RP}$, the honest IdP calculates $PID_U = [ID_U]PID_{RP}$ following Equation \ref{equ:PIDU} after authenticating the user with $ID_U$.

When receiving $t$, the designated RP calculates $Acct = [t^{-1}]PID_{U} = [ID_U]ID_{RP}$ following Equation \ref{equ:AccountNotChanged}, where
$Acct = [ID_U]ID_{RP}$ is a \emph{permanent} identifier that is determined once the RP and the user have registered at the IdP. Therefore, $PID_U$ in $TK$ identifies an account at the designated RP and $Acct$ is mapped to a user with $ID_U$.

If receiving $t' \neq t$ from an adversary,
the designated RP calculates an account as $[t'^{-1}]PID_U = [t'^{-1}ut]ID_{RP}$.
Because a user's identity is randomly selected in $\mathbb{Z}_n$ by the IdP
    and it is known to only this user (and the honest IdP),
    the probability that $t'^{-1}ut$ happens to be another user's identity is negligible
     when $n$ is sufficiently large.
So it identifies no account mapped to some user, if the designated RP receives $t' \neq t$.

Next, we prove that $PID_{U}$ \emph{uniquely} identifies one account at the RP receiving $t$
    and one user in the system.
 Since $\mathbb{E}$ is a finite cyclic group, $ID_{RP} = [r]G$ is also a generator on $\mathbb{E}$ of order $n$. Given a user with $ID_U$, $Acct = [ID_U]ID_{RP}$ is a \emph{unique} point on $\mathbb{E}$ for any $u \in [1, n)$, which is \emph{uniquely} associated to $ID_U=u$.

Finally, 
    let us consider any RP not designated by $PID_{RP}$,
    or with $ID_{RP'} = [r']G \neq [r]G$.
From $PID_U = [ID_U]PID_{RP} = [utr]G$,
by using any $\tilde{t}$ this RP calculates $Acct' = [\tilde{t}^{-1}]PID_{U} = [\tilde{t}^{-1}utrr'^{-1}][r']G = [\tilde{t}^{-1}utrr'^{-1}]ID_{RP'}$.
%Because $r$ and $r'$ are unknown,
The probability that $\tilde{t}^{-1}utrr'^{-1}$ happens to be some user's identity is negligible
     when $n$ is sufficiently large.
So it identifies no account mapped to a user, at any RP not designated by $PID_{RP}$.
\hfill $\square$
...
\usso\ effectively prevents \emph{an honest-but-curious IdP} not colluding with other attackers,
 from tracing a user's login activities.
That is, the privacy threat of \emph{IdP-based login tracing} is prevented.

\newc
In each login, a user sends an identity-token request to the IdP, %(Step 3.1 in Section \ref{implementations})
which reveals only the target RP's \emph{ephemeral} pseudo-identity $PID_{RP}$ along with the user's identity $ID_U$.
In particular,
    with the formal model of \usso\ in Appendix \ref{appendix-model}, we conclude that, 
among the information related to the target RP in each login,  
    \emph{only} $PID_{RP}$ is accessible to the IdP.

%An honest-but-curious IdP cannot trace a user's login activities.
...
\usso\ prevents the privacy threats of \emph{RP-based identity linkage},
 even when \emph{malicious RPs collude with malicious users}, attempting to link the logins initiated by an honest user but visiting different colluding RPs. 
...
Therefore,
    the colluding adversaries share all logins visiting malicious RPs, and attempt to leverage the logins initiated by malicious users to link the other logins by honest users.



Following the formal model in Appendix \ref{appendix-model}, we conclude that, 
an RP receives \emph{only} a random number $t$ and an identity token enclosing $PID_{RP}$ and $PID_U$ in each login visiting this RP,
    and it receives nothing in the logins to other (honest) RPs.
 %Meanwhile, it knows its own identity $ID_{RP}$, from which it also derives the user's account $Acct$.
