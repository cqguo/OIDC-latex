...
In \usso, given the shared knowledge $\mathbb{V}=\{\mathbb{V}_j\}$ and $\mathbb{L}=\{\mathbb{L}_j\}$, where $j \in [1,c]$, from a group of $c$ colluding RPs and $v$ colluding users, any malicious RP cannot differentiate if an honest user who has logged in to itself is among the users who have logged in to any of the colluding RPs.}
...
\vspace{1mm}
\noindent\textsc{Lemma 2.} {\em When $u$, $r_j$, and $r_{j'}$ are randomly selected and unknown, the probability of distinguishing the association between $([r_j]G, t_{uj}, [ur_j]G)$ and $([r_{j'}]G, t_{uj'}, [ur_{j'}]G)$ and the association between $([r_j]G, t_{uj}, [ur_j]G)$ and $([r_{j'}]G, t, [z]G)$ is negligible, where $G$ is a point on an elliptic curve $\mathbb{E}$ and $t_{uj}$, $t_{uj'}$, and $z$ are random numbers.}

\vspace{0.75mm}
\noindent{\textsc{Proof.} First, we define an RP-based identity linkage game $\mathcal{G}_r$ between an adversary and a challenger based on the shared knowledge $\mathbb{V}$: the adversary receives $L'$ and $\mathfrak{L}$ from the challenger and outputs the result $s = 1$ if it decides $u' \in \{u_1, u_2, \cdots, u_v\}$ or $s = 0$ if $u'$ is randomly chosen in $\mathbb{Z}_n$.
define the game.
