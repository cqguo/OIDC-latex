...
If the adversary has non-negligible advantage in $\mathcal{G}_r$, then $\mathbf{Adv}^*_{A}={\mathbf{Adv}}_{A}$ is non-negligible regardless of the security parameter $\lambda$. This violates the ECDDH assumption. Therefore, the adversary has no advantage in $\mathcal{G}_r$ and cannot decide whether $L'_a$ is initiated by a user in $\mathbf{u}_w$ or in the universal user set. Because $RP_b$ can be any malicious RP, this proof can be easily extended from $RP_b$ to all $c$ malicious RPs.
\hfill $\square$
