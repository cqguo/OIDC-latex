...
When $c+1$ malicious RPs and $v$ malicious users collude in \usso, given a login $\mathcal{L'}$ initiated by an honest user $U'$ to a malicious RP, the adversaries cannot distinguish a login by $U'$ from the ones initiated by any other honest users to any of the remaining $c$ malicious RPs.}
