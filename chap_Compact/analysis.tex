...
\vspace{0.75mm}
\noindent{\textsc{Proof.} Without loss of generality, let us consider an arbitrary login instance in which a user with $ID_{U'}=u'$ logs in to the $c$th malicious RP. The RP obtains $L'=(ID_{RP_c}, t', Acct'_c)$, where $ID_{RP_c}=[r_c]G$ and $Acct'_c=[u'r_c]G$. If this user previously logged in to at least one colluding RP (e.g., $RP_k$), she should be in $\mathbb{U}_c$, and her login information $L_{u',k}=(ID_{RP_k}, t_{u'k}, Acct'_k)=([r_k]G, t_{u'k}, [u'r_k]G)$ should be in $\mathbb{V}_k$. Hence, in RP-based linkage, $RP_c$ attempts to associate $L'$ with 
$L_{u',k}$, which is to associate $(G, [r_c]G, t', [u'r_c]G)$ and $(G, [r_k]G, t_{u'k}, [u'r_k]G)$, where $G$ is a point on an elliptic curve $\mathbb{E}$.
