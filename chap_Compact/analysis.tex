...
Moreover, the adversary cannot retrieve the $PID_{RP}$ from the honest user login to an honest RP, as the user and RP would not send the $PID_{RP}$ and $t$ to any adversary party. Therefore, even the adversary lead the honest to send the adversary's token to an honest RP, the RP would not accept this token anyway.
