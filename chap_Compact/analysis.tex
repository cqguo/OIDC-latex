...
If the adversary has advantages in $\mathcal{G}_r$ to distinguish a login initiated by a user $U'$ chosen from \{${U_1}, {U_2}, \cdots, {U_w}$\} or randomly from the universal user set,
    $|{\rm Pr}^*_1-{\rm Pr}^*_2|=|{\rm Pr}_1-{\rm Pr}_2|$ is non-negligible regardless of $k$.
This violates the ECDDH assumption.
Thus, the probability of distinguishing a user $U'$ chosen from \{${U_1}, {U_2}, \cdots, {U_w}$\} or from the universal user set is negligible.
\hfill $\square$
...
\noindent\textsc{Lemma 3.} { Given
    the logins visiting $c$ RPs by $v$ users denoted as
 $\mathfrak{L}=\left \{ \begin{matrix}
L_{1,1},&L_{1,2},&\cdots,&L_{1,c}\\
L_{2,1},& L_{2,2},&\cdots,&L_{2,c}\\
\cdots,&\cdots,&L_{i,j},&\cdots\\
...
The advantage of solving the ECDDH problem using $\mathcal{D}^*_r$ is defined as $\epsilon_{r}(k)$, and
...
The advantage $\epsilon_{r}(k)$ of solving the ECDDH problem using $\mathcal{D}^*_r$ is deduced as:
\begin{equation*}
