...
With the pre-installed $PK$, an RP could verify the identity tokens it receives.
...
\vspace{1mm}
\noindent{\textsc{Theorem 4. (Identity Token Confidentiality)}} {\em An identity token $T$ %whose $PID_{RP}$ designates an RP with $ID_{RP}$ can
...
When the confidentiality and integrity of identity tokens are satisfied, an adversary cannot present an identity token issued for honest users and accepted by an honest RP. When RP designation and user identification are satisfied, an identity token issued to an adversary always derives the adversary's account at any RP. So, an adversary never presents an identity token that is accepted by an honest RP to derive an honest user's account at this RP.
...
According to the conclusions of the \dy~model, $PID_U$ is calculated based on the user when the IdP issues an identity token for an authenticated user.
Moreover, because the confidentiality and integrity of identity tokens are satisfied, an honest user never presents a token issued to other users and is accepted by an honest RP.
When the user identification user is satisfied,\footnote{It is worth noting that RP designation is a precondition of user identification in UPPRESSO. In other SSO systems, RP designation is not always a precondition of user identification.} this token always derives this honest user's account.
\hfill $\square$
...
In each login instance, an RP receives $t$ (Step 2.1 in Section~\ref{implementations}) and an identity token (Step 4.1) from which it can extract $PID_{RP}$ and $PID_U$. Meanwhile, the RP knows its own $ID_{RP}$ and the accounts of its users (i.e., $Acct$s), %Since $PID_{RP}= [t]{ID_{RP}}$ and $Acct = [t^{-1}]PID_{U}$, we can deduce $PID_{RP}$ and $PID_U$ from $ID_{RP}$ and $Acct$ with a valid $t$.
...
\vspace{1mm}
\noindent{\textsc{Theorem 7. (Privacy against Colluding RPs)}} {\em Consider $(c+1)$ RPs colluding in a system of $n$ RPs, where $1 \le c \le n-1$. Assume $w$ users have logged in to all the colluding RPs, among which $v$ users are malicious and colluding with these RPs. 
Using \usso, any colluding RP cannot distinguish the login instance of a user at itself from this user's login instances at any other colluding RPs, even if the colluding RPs have linked the login instances of $v$ colluding users and shared the knowledge with each other.}

\vspace{0.75mm}
\noindent{\textsc{Proof.} 
First, $v$ malicious users colluding with $c$ malicious RPs could link their login instances at these RPs. Without loss of generality, we consider the first $c$ RPs collecting information about the first $v$ malicious users and sharing the knowledge in term of $\mathfrak{L}$, which is defined as:
\begin{equation}
...
Now, consider an arbitrary login instance at the $(c+1)$th colluding RP, denoted as $L'=([r_{c+1}]G, t', [u'r_{c+1}]G)$. For $RP_{c+1}$, the user associated with $L'$, i.e., $u'$, is indistinguishable from any user associated with $\mathfrak{L}$ based on Lemma 2, and also from any user associated with $\mathfrak{L}_j$ for $1 \le j \le c$ based on Lemma 3. Therefore, $RP_{c+1}$ cannot link user logins that it observes with user logins at any other colluding RPs, even if $\mathfrak{L}$ and $\mathfrak{L}_j$ are shared among $(c+1)$ colluding RPs.

\vspace{1mm}
\noindent\textsc{Lemma 2.} {\em Given $\mathfrak{L}$ that is constructed from login instances observed by $c$ colluding RPs and $v$ colluding users, the $(c+1)$th colluding RP cannot distinguish two users $u'$ and $u_i$, who are associated with login instances $L'=([r_{c+1}]G, t', [u'r_{c+1}]G)$ and $L_{i, j}=([r_j]G, t_{ij}, [u_ir_j]G)$ for $1 \le i \le v$ and $1 \le j \le c$.}
...
We design a PPT algorithm $\mathcal{D}^*_r$ based on the game $\mathcal{G}_r$, as shown in Figure \ref{fig:dalgorithm}, to solve the elliptic curve decisional Diffie-Hellman (ECDDH) problem: given $a=(G, [x]G$, $[y]G$, $[xy]G)$ and $b=(G, [x]G$, $[y]G$, $[z]G)$, determine if a quaternion is an elliptic curve Diffie-Hellman (ECDH) group or a non-ECDH group, where $G$ is a point on an elliptic curve $\mathbb{E}$ of order $n$, and $x$, $y$, and $z$ are integers randomly and independently chosen in $\mathbb{Z}_n$. The probability of distinguishing $a$ and $b$ using any PPT algorithm $\mathcal{D}$ can be defined as $\epsilon_{r}(k) = |{\rm Pr}_1 - {\rm Pr}_2|$, where: %Let  ${\rm Pr}\{\}$ denote the probability and we define
\begin{align*}
...
The input of $\mathcal{D}^*_r$ is in the form of $(G$, $Q_1=[x]G$, $Q_2=[y]G$, $Q_3=[z]G)$. Upon receiving the input, the challenger of $\mathcal{G}_r$ randomly chooses $\{u_1, u_2, \cdots, u_v\}$, $\{r_1, r_2, \cdots, r_c\}$, $\{t_{1, 1}, t_{1, 2}, \cdots, t_{v, c}\}$, and $t'$ in $\mathbb{Z}_n$.
