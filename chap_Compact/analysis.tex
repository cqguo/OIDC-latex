...
Because $G$ is a generator of order $n$, $ID_{RP} = [r]G$ is also a generator on $\mathbb{E}$ of order $n$. According to the DYU model, $t$ is randomly chosen in $\mathbb{Z}_n$ and kept unknown to the IdP. Therefore, $PID_{RP} = [t]ID_{RP}$ is \emph{indistinguishable} from a point $Q$ that is randomly chosen on $\mathbb{E}$ \cite{oprf-proved,strong-oprf,voprf-proved}.\footnote{This property has been actually proved in OPRFs \cite{oprf-proved,strong-oprf,voprf-proved}, where the OPRF server works similarly to the IdP and learns \emph{nothing}.} \hfill $\square$
...
In the third adversarial scenario, $c+1$ malicious RPs collude with malicious users. They attempt to link the logins of honest users across these colluding RPs. Let us consider an honest user $U'$ who initiates a login $\mathcal{L'}$ to  one malicious RP. All the colluding entities, including this RP, aim to associate $\mathcal{L'}$ with any other logins from this user $U'$ to any of the $c$ colluding RPs. 
...
As the logins are indistinguishable from each other, given $\mathcal{L'}$ and a shared view of all the logins to the colluding RPs, the adversaries have no clue to link $\mathcal{L'}$ to one login instead of another. They even cannot link $\mathcal{L'}$ to one %|$w$| 
...
\vspace{3mm}
\noindent \textsc{Theorem 7. (Privacy against Colluding RPs)} {Given a login initiated by any honest user to any malicious RP, this RP, even colluding with other $c$ malicious RPs and $v$ malicious users, cannot distinguish if a login to any of the other $c$ malicious RPs is initiated by this user or another honest user.}
