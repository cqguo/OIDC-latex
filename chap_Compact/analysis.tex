...
Note that a login instance $L$ includes the user's account.
...
Malicious RPs, even colluding with malicious users, attempt to link login instances
...
and the login instance to the other RP is denoted as $L' = (ID_{RP'}, t', [ID_{U'}]ID_{RP'})$.
...
given a login instance $L'$ by an honest user $U'$ to one malicious RP,
    the adversaries cannot distinguish a login instance by $U'$ to other $c$ malicious RP
        from the login instances by any honest users to the $c$ malicious RPs.}
...
%
Consider a login instance $L'$ by an honest user $U'$ to one malicious RPs, denoted as $RP'$, and
    other malicious RP are denoted as $RP_j$ ($1 \leq j \leq c$).
According to Lemma 2, a pair of $RP_j$ and $RP'$ cannot decide whether a login instance to $RP'$,
...
After $RP'$ colludes with each $RP_j$ ($1 \leq j \leq c$), 
  we find that, given $L'$ initiated by $U'$ to $RP'$,
    the adversaries cannot distinguish a login instance by $U'$ to any $RP_j$,
        from the login instances by some users to $RP_j$.
That is, given a login instance $L'$ by an honest user $U'$ to one malicious RP,
    the adversaries cannot distinguish a login instance by $U'$ to other $c$ malicious RP
        from the login instances by any honest users to the $c$ malicious RPs.
...
Next, when colluding with $v$ malicious users,
    ...
    $RP_j$ ($1 \leq j \leq c$) and $RP'$ cannot decide whether a login instance by $U'$ to $RP'$,
        is initiated by one of the users visiting $RP_j$ ($1 \leq j \leq c$) or not;
 otherwise, $U'$ is malicious and reveal his identity to the $c+1$ RPs.
The collusion with malicious users does not bring any benefits to the adversaries in RP-based identity linkage.
