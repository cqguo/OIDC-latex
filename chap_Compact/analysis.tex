...
\vspace{0.75mm}
\noindent\textsc{Proof.}
Consider an arbitrary malicious RP denoted as $RP'$. When an honest user $U'$ initiates a login $\mathcal{L'}$ to $RP'$, it obtains $L' = (ID_{RP'}, t', [ID_{U'}]ID_{RP'})$. Suppose another malicious RP, denoted as $RP_j$, colludes with $RP'$ and shares the knowledge about all its $w$ logins, denoted as $\mathbb{L}_j = \{{L_{i,j}}\}$, where %$L_{i,j} = ([r]G, t_{ij}, [u_{ij} r]G)$ and 
$1 \leq i \leq w$. We denote the set of users who initiate these logins as $\mathcal{U}$. As the logins could be initiated by honest or malicious users, we denote the set of honest users in $\mathcal{U}$ as $\mathcal{U}_H$.

First, let us assume malicious users do not collude with malicious RPs, so they do not reveal the $PID_U$ of their logins. Therefore, even $RP_j$ and $RP'$ collude, they do not know if some of the logins in $\mathbb{L}_j$ are initiated by the same malicious user or not. According to Lemma 2, given $L'$ and $\mathbb{L}_j$, $RP'$ and $RP_j$ cannot determine if an arbitrary login in $\mathbb{L}_j$ is initiated by $U'$ or by another user in the system. Therefore, they cannot link $L'$ to any of these logins, as they appear equally likely to be initiated by $U'$. We could extend this proof to include all the $c$ malicious RPs since $RP_j$ could be any malicious RP. 
