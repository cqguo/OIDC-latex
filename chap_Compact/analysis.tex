...
given a shared view of the logins to the malicious RPs by colluding users,
...
To show that \usso\ prevents such linkage, we prove in Theorem 7 that the adversaries cannot decide whether two logins visiting different malicious RPs are initiated by one honest user or not. %a login initiated by $U'$ to some of the $c$ malicious RPs from the ones initiated by any other honest users to the $c$ malicious RPs.
...
They cannot even link a login visiting an RP by an honest user to a subset of logins visiting another RP.
...
Because $PID_{RP}$ and $PID_U$ can be deduced from $ID_{RP}$ and $Acct$ with $t$, respectively, we ignore duplicated elements and define an RP's knowledge about a login as $\mathcal{L} =(ID_{RP}, t, Acct)=(ID_{RP}, t, [ID_{U}]ID_{RP})$. %Note that a login $L$ includes the user's account at the RP.
...
the $c$ RPs cannot link a login visiting a malicious RP by some honest user 
    to any subset of logins visiting another  malicious RP;
  i.e., cannot decide whether $U''$ chosen from \{${U'_1}, {U'_2}, \cdots, {U'_w}$\} or randomly from the universal user set,
    where $L'' = ([r_{j''}]G, t'', [u''r_{j''}]G)$, $\mathbb{L}' = \{L'_k\}$,
     $L'_k = ([r_{j'}]G, t'_k, [u'_kr_{j'}]G)$, $1 \leq j' \leq c$, $1 \leq j'' \leq c$, $j' \neq j''$, $1 \leq k \leq w$, $u'' \neq u_i$, $u'_k \neq u_i$.}
...
\vspace{0.75mm}
\noindent\textsc{Proof.}
We define a game $\mathcal{G}_r$ between an adversary and a challenger,
    to describe this RP-based login linkage:
 the adversary receives $\mathfrak{L}$, $\mathbb{L}'$ and $L''$ from the challenger, and outputs the result $s = 1$ if it decides $U'' \in \{{U'_1}, {U'_2}, \cdots, {U'_w}\}$ or is randomly chosen in $\mathbb{Z}_n$.
The adversary succeeds in $\mathcal{G}_r$ with an advantage $\mathbf{Adv}_{A}$:
\begin{align*}
&{\rm Pr}_1={\rm Pr}\{\mathcal{G}_r(\mathfrak{L}, \mathbb{L}', L'' | U'' \in \{{U'_1}, {U'_2}, \cdots, {U'_w}\})=1\} \\
&{\rm Pr}_2={\rm Pr}\{\mathcal{G}_r(\mathfrak{L}, \mathbb{L}', L'' | U'' \in \mathbb{Z}_n)=1\}\\
...
Upon receiving the input, the challenger randomly chooses
 $\{u_1, u_2, \cdots, u_v\}$, $\{r_1, r_2, \cdots, r_c\}$, $\{t_{1, 1}, t_{1, 2}, \cdots, t_{v, c}\}$ in $\mathbb{Z}_n$,
  where $[r_{j}]G \neq Q_2$ (i.e., $r_j \neq y$), 
  and then construct $L_{i, j}=([r_j]G, t_{i,j}, [u_ir_j]G)$, $1 \le i \le v$ and $1 \le j \le c$.

It randomly chooses $j' \in [1, c]$.
For $1 \leq i \leq v$, replace $L_{i, j'}$ with $(Q_2, t_{i,j'}, [u_i]Q_2) = ([y]G, t_{i,j'}, [u_iy]G)$.

It chooses $\{u'_1, u'_2, \cdots, u'_w\}$, $\{t'_1, t'_2, \cdots, t'_w\}$ randomly in $\mathbb{Z}_n$,
    where $[u'_k]G \neq Q_1$ (i.e., $u'_k \neq x$) and $u'_k \neq u_i$,
    and constructs $L'_k = (Q_2, t'_k, [u'_k]Q_2) = ([y]G, t'_k, [u'_ky]G)$ for $1 \leq k \leq w$.
It randomly chooses $d \in [1, w]$, and replace $L'_d$ with $([r_{j'}]G, t'_d, [z]G)$.
This means $\{u'_1, u'_2, \cdots, u'_{d-1}, z/y, u'_{d+1}, \cdots, u'_w\}$ visits $[r_{j'}]G$.
 
Finally, the challenger randomly chooses $j'' \in [1, c]$ and $j' \neq j''$.
It randomly chooses $t''$ in $\mathbb{Z}_n$, to constructs $L'' = ([r_{j''}]G, t'', [r_{j''}]Q_1)$.
This means $x$ visits $[r_{j''}]G$.

When the adversary receives $\mathfrak{L}$, $\mathbb{L}'$ and $L''$, it outputs $s$.

According to the construction of $\mathfrak{L}$, $\mathbb{L}'$ and $L''$, 
 $x$ is actually inserted into $L''$ as $u''$ to visit $[r_{j''}]G$,
 and $z/y$ is inserted into $\mathbb{L}'$ as $u'_d$ to visit $[r_{j'}]G$ with $\{u'_1, u'_2, \cdots, u'_{d-1}, u'_{d+1}, \cdots, u'_w\}$, where $u'_k \neq x$.

Thus, if $z = xy$, then $z/y = x$ and $u'' \in \{u'_1, u'_2, \cdots, u'_w\}$; otherwise, $u'' \in \mathbb{Z}_n$.
...
=&{\rm Pr}\{\mathcal{G}_r(\mathfrak{L}, \mathbb{L}', L'' | U'' \in \{{U'_1}, {U'_2}, \cdots, {U'_w}\})=1\} = {\rm Pr}_1 \\
&{\rm Pr}^*_2= {\rm Pr}\{\mathcal{D}^*_r(G, [x]G, [y]G, [z]G)=1\} \\
=&{\rm Pr}\{\mathcal{G}_r(\mathfrak{L}, \mathbb{L}', L'' | U'' \in \mathbb{Z}_n)=1\} = {\rm Pr}_2\\
...
If the adversary has advantages in $\mathcal{G}_r$ to decide whether $L''$ is initiated by $U''$ chosen from \{${U'_1}, {U'_2}, \cdots, {U'_w}$\} or randomly from the universal user set,
    then $\mathbf{Adv}^*_{A}={\mathbf{Adv}}_{A}$ is \emph{non-negligible} regardless of $\lambda$.
This violates the ECDDH assumption.
Thus, the adversaries cannot decide whether $L''$ is initiated by a user in \{${U'_1}, {U'_2}, \cdots, {U'_w}$\} or randomly from the universal user set.
\hfill $\square$
