...
\vspace{0.75mm}
\noindent\textsc{Proof.}
Consider an arbitrary malicious RP, denoted as $RP'$, among $c+1$ colluding RPs. When an honest user $U'$ initiates a login $\mathcal{L'}$ to $RP'$, it obtains $L' = (ID_{RP'}, t', [ID_{U'}]ID_{RP'})$. Suppose one of the remaining malicious RP, denoted as $RP_j$, colludes with $RP'$ and shares its knowledge about all the $w$ logins it has observed, denoted as $\mathbb{L}_j = \{{L_{ij}}\}$, where $L_{ij} = ([r]G, t_{ij}, [u_{ij} r]G)$ and $1 \leq i \leq w$. $\mathbb{L}_j$ is about the logins to $RP_j$ that could be initiated by honest or malicious users. 

First, let us assume the malicious users do not collude with the malicious RPs by not revealing additional information about their own $PID_U$s to them. Therefore, two colluding RPs do not know if some of the $w$ logins are initiated by the same malicious users. According to Lemma 2, given $L'$ and $\mathbb{L}_j$, $RP'$ and $RP_j$ cannot determine if an arbitrary login among $w$ is initiated by $U'$ or by another honest user in the system. Therefore, they cannot link $L'$ to any of the $w$ logins, which appear equally likely to be initiated by $U'$. Then, we extend this proof to include all $c$ malicious RPs since $RP_j$ could be any malicious RP in the system. 
...
Next, consider $v$ malicious users colluding with malicious RPs and sharing their $PID_U$s with them. $c$ colluding RPs could link multiple logins of each malicious user $U_i$ as $(L_{i,1},..., L_{i,c})$. Denote their shared knowledge about $w$ logins as $\mathbb{L}$ and assume the first $v$ users are malicious. We have:
...
% L_{v,1},&L_{v,2},&\cdots,&L_{v,c}
% \end{matrix}\right\}$
\noindent where $L_{i,j} = (ID_{RP_j}, t_{i,j}, [ID_{U_i}]ID_{RP_j})$, $i \in [1, v]$ and $j \in [1, c]$. $\mathfrak{L}$ is an ordered subset of $\mathbb{L}$ denoting the linked logins initiated by all malicious users to all malicious RPs.
...
%$RP_j$ ($1 \leq j \leq c$) and $RP'$ cannot decide whether a login by $U'$ to $RP'$ is initiated by one of the $v$ malicious users visiting $RP_j$ ($1 \leq j \leq c$) or not, unless $U'$ is malicious and reveal his identity to $RP'$.So, the collusion with malicious users does not bring benefits for the adversaries, to link an honest user's logins across colluding RPs. So UPPRESSO prevents RP-based identity linkage, even when malicious RPs are colluding with malicious users.
