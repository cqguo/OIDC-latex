...
\vspace{1mm}
\noindent\textcolor{blue}{\textbf{RP Designation.}~~The RP pseudo-identity $PID_{RP} = [t]ID_{RP}$ in the identity token
     designates the target RP with $ID_{RP} = [r]G$, and only this RP,
     when $r$ is known to only the IdP.}
%That is,
...
\vspace{1mm}
\noindent\textcolor{blue}{\textbf{Privacy against the IdP.}~~When $t$ is random in $\mathbb{Z}_n$ and unknown to the IdP,
the IdP
...
So the prevention against the RP-based identity linkage in UPPRESSO is as follows.}
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.99\linewidth]{fig/dalgorithm.pdf}
