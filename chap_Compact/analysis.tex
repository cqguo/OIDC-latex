...
\vspace{0.75mm}
\noindent{\textsc{Proof.} 
First, $v$ malicious users colluding with $c$ malicious RPs could link their login instances at these RPs. Without loss of generality, we consider the first $c$ RPs collecting information about $v$ malicious users and sharing the knowledge:
\begin{equation*}
...
\end{matrix}\right\}
\end{equation*}
where $L_{i, j}=(ID_{RP_j}, t_{ij}, Acct_i)=([r_j]G, t_{ij}, [u_ir_j]G)$ according to Equation~\ref{equ:Account}. In $\mathfrak{L}$, each row denotes the shared knowledge about one malicious user's logins at $c$ colluding RPs. Now, consider the $(c+1)$th colluding RP and one arbitrary login instance at it, denoted as $RP_{c+1}$ observes $L'=([r_{c+1}]G, t', [u'r_{c+1}]G)$. According to Lemma 2, we need to prove that 
