...
Next, we prove that it is computationally impossible 
    for adversaries
    to find $t$ and $t'$
...
Given $G$ and $Q$,
    it is computationally impossible for any probabilistic polynomial time (PPT) algorithm $\mathcal{D}$
        to calculate $x$ (i.e., to solve the ECDLP).
Let $Pr\{\}$ denote the probability.
For any PPT algorithm $\mathcal{D}$, we define
\begin{align*}
Pr\{\mathcal{D}(G, [x]G)=x\} = \epsilon_{ECDLP}(k)
\end{align*}
So $\epsilon_{ECDLP}(k)$ becomes negligible with the security parameter $k$.
...
On receiving the input, the challenger of $\mathcal{G}_r$ randomly chooses
 $\{u_1, u_2, \cdots, u_v\}$, $\{r_1, r_2, \cdots, r_c\}$, $\{t_{1, 1}, t_{1, 2}, \cdots, t_{v, c}\}$, and $t'$ from $(1,n)$.
