...
\vspace{0.75mm}
\noindent\textsc{Proof.} From logins in $\mathbb{L}$ but not in $\mathfrak{L}$, we randomly select a login $L'$ to a malicious $RP_{j'}$, which is from an (unknown) honest user $U'$, and another malicious $RP_{j''}$ and its logins $\mathbb{L}'' = \{L''_k\}$, which are from (unknown) honest users $\{U''_k\}$, for $j', j'' \in [1,c]$ and $j' \neq j''$. Next, we prove that the colluding adversaries cannot decide if $U'$ is in any subset of $\{U''_k\}$, denoted as $\{{U''_1}, {U''_2}, \cdots, {U''_w}\}$, or not (i.e., $U'$ is randomly selected from the universal user set). This indicates that the colluding adversaries cannot link $L'$ to another login or any subset of logins in $\mathbb{L}''$. 

To prove this, we first define an RP-based linkage game $\mathcal{G}_r$ between an adversary and a challenger, which describes the RP-based login linkage privacy threat: the adversary receives $\mathfrak{L}$, $L'$ and $\mathbb{L}''$ from the challenger, and then outputs the result $s$, where $s = 1$ if it decides $U'$ is in $\{{U''_1}, {U''_2}, \cdots, {U''_w}\}$ or $s=0$ if it believes $U'$ is randomly chosen from the universal user set.
