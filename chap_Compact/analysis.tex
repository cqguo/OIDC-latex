...
%${\rm Pr}_s$ becomes  as $k$ increases. Because $m$ is a finite integer and , ${\rm Pr}\{\mathcal{D}^*_c(G, [x]G)=x\} = \frac{1}{m}{\rm Pr}_s$ is non-negligible. 
This violates the ECDLP assumption. Therefore, the probability of finding $t$ and $t'$ that satisfy $[t]ID_{RP_j} = [t']ID_{RP_{j'}}$ in UPPRESSO is negligible. %so this token binding $PID_{RP}$ is expected by \emph{only} the target RP. 
\hfill $\square$


\vspace{1mm}
\noindent\textcolor{blue}{\textsc{Theorem 2. (User Identification)} {\em Consider a login instance, in which an arbitrary RP receives an integer $t$ and an identity token $T$ that binds $PID_U$ and $PID_{RP}$. If $PID_{RP}$ designates this RP, then $PID_U$ uniquely identifies the user with $ID_U=u$ in the system.}
}
...
\vspace{0.5mm}
\noindent\textcolor{blue}{\textsc{Proof.}} The RP 
