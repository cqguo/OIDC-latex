...
Then, we consider the third adversarial scenario where $c+1$ malicious RPs collude with $v$ malicious users attempting to link login instances of honest users across these malicious RPs. That is, given an arbitrary login instance $L'$ initiated by an honest user $U'$ to one of the $c+1$ malicious RPs, the colluding adversaries aim to link $L'$ to $U'$'s other login instances to the remaining $c$ malicious RPs. To show that \usso~prevents such linkage, we will prove in Theorem 7 that the colluding adversaries cannot distinguish the login instances initiated by $U'$ from the ones initiated by any other honest users, requesting to log in to any of the remaining $c$ malicious RPs.
