...
Let $\mathbb{E}$ be an elliptic curve over a finite field $\mathbb{F}_q$,
    $P$ be a point on $\mathbb{E}$ of order $n$,
        and $Q = [x]P$ where $x$ is an integer number randomly  chosen from $(1,n)$.
Given $P$ and $Q$,
...
If $[t]ID_{RP_a}=[t']ID_{RP_b}$, the adversary succeeds in this game and RP designation is broken.
...
So, if the adversary is able to break RP designation,
...
\begin{figure}[tb]
  ...
  \caption{The algorithm to solve the ECDLP.}
...
The input of $\mathcal{D^*}$ is in the form of ($P$, $Q$).
On receiving the input, the challenger of $\mathcal{G}$ randomly chooses $r_1, r_2, \cdots, r_m$ from $(1,n)$,
 calculates $[r_1]P, [r_2]P, \cdots, [r_m]P$,
 and randomly replaces some $[r_j]P$ with the input $Q$.
Then,
...
If $[r_a]P$ happens to be replaced with $Q$ and the adversary succeeds,
    we find that $Q = [s]P$ and then $s=x$ because $[tr_a]P = [t]Q = [t'r_b]P$.
As $[r_j]P$ is randomly chosen by the challenger,
    $Q$ and other RP identities in the input set are indistinguishable to the adversary.
Thus,
\begin{align*}
Pr\{\mathcal{D^*}(P, [x]P)=x\} = Pr\{s = x\}=\frac{1}{m}Pr_s
\end{align*}
If $Pr_s$ is non-negligible, $Pr\{\mathcal{D^*}(P, [x]P)=x\}$ is also non-negligible.
It means $\mathcal{D^*}$ solves ECDLP.
...
An honest RP with $ID_{RP}$ always deterministically derives an identical permanent account from different identity tokens binding $PID_U$ and $PID_{RP}$.
That is,
