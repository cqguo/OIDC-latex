...
From each login request, an RP learns an ephemeral pseudo-identity $PID_{U}$ and a permanent account  $Acct = [ID_U]ID_{RP}$ of the user, but it cannot derive $ID_U$ from $Acct$ due to the ECDLP problem.
Therefore, the colluding adversaries share information about the logins to malicious RPs or initiated by malicious users,
    attempting to link multiple logins initiated by some honest user to multiple malicious RPs.
...
When $c$ malicious RPs collude with each other,
    they create a shared view of all their logins, denoted as $\mathbb{L}$.
 %       some of which are initiated by honest users and denoted as $\mathbb{L}^h$,
 %           and the others by $v$ malicious users  are $\mathbb{L}^m = \mathbb{L} \setminus \mathbb{L}^h$.
When the  $c$ RPs collude with $v$ malicious users,
 the logins by these malicious users are picked out and linked together as:
...
\noindent where $L^m_{i, j}=([r_j]G, t_{i,j}, [u_ir_j]G)$ for $1 \le i \le v$ and $1 \le j \le c$,
 and each $L^m_{i,j} \in \mathbb{L}$.
Thus, any other login in $\mathbb{L}$ but not linked in $\mathfrak{L}^m$ is initiated by an honest user to some malicious RP.
...
\vspace{2mm}
\noindent \textsc{Theorem 7. (Privacy against Colluding RPs)} {In \usso, given $\mathbb{L}$ and $\mathfrak{L}^m$, the $c$ malicious RPs and $v$ malicious users cannot
 link any login visiting a malicious RP by an honest user
  to any subset of logins visiting any other malicious RPs by honest users.}
...
\noindent\textsc{Proof.} From the logins in $\mathbb{L}$,
 we randomly choose one login $L'_a \neq L^m_{i,j}$,
 which is from an (unknown) honest user with $ID_{U'}=u'$ to a malicious $RP_a$, $a \in [1,c]$.
Then, we randomly choose another malicious $RP_b$, $b \in [1,c]$ and $b \neq a$.
Consider any subset $\mathbb{L}''_b$ of $w$ logins visiting $RP_b$ by unknown honest users,
  we denote the corresponding honest users who initiate these logins as $\mathbf{u}_w=\{{u''_1}, {u''_2}, \cdots, {u''_w}\}$.
Next, we prove that the colluding adversaries cannot decide if $u'$ is in $\mathbf{u}_w$ or not (i.e., $u'$ is randomly selected from the universal user set). This indicates that the colluding adversaries cannot link $L'_a$ to another login or even any subset of $\mathbb{L}''_b$.

To prove this, we first define an RP-based linkage game $\mathcal{G}_r$ between an adversary and a challenger, which describes this RP-based login linkage privacy threat: the adversary receives $\mathfrak{L}^m$, $L'_a$, and $\mathbb{L}''_b$ from the challenger and outputs $s$, where $s = 1$ if it decides $u'$ is in $\mathbf{u}_w$ %$\{{U''_1}, {U''_2}, \cdots, {U''_w}\}$
...
&{\rm Pr}_1={\rm Pr}(\mathcal{G}_r(\mathfrak{L}^m, L'_a, \mathbb{L}''_b)=1 \;| \; u' \in \mathbf{u}_w)  \\
%&{\rm Pr}_2={\rm Pr}\{\mathcal{G}_r(\mathfrak{L}, L', \mathbb{L}'' | u' \in \mathbb{Z}_n)=1\}\\
&{\rm Pr}_2={\rm Pr}(\mathcal{G}_r(\mathfrak{L}^m, L'_a, \mathbb{L}''_b)=1 \; | \; u' \in \mathbb{Z}_n)\\
...
, the challenger randomly chooses $a \in [1, c]$ and $t' \in \mathbb{Z}_n$ to assemble $L' = ([r_{a}]G, t', [r_{a}]Q_1) = ([r_{a}]G, t', [xr_{a}]G)$.
(2) It chooses random numbers in $\mathbb{Z}_n$ to construct $\{u_i\}$, $\{r_j\}$, and $\{t_{i, j}\}$, for $1 \le i \le v$ and $1 \le j \le c$, with which it assembles $L^m_{i, j}=([r_j]G, t_{i,j}, [u_ir_j]G)$.
...
Hence, for $1 \le i \le v$, the challenger replaces $L_{i, b}=([r_b]G, t_{i,b}, [u_ir_b]G)$ with $(Q_2, t_{i,b}, [u_i]Q_2) = ([y]G, t_{i,b}, [u_iy]G)$ and constructs $\mathfrak{L}^m$.
(3) The challenger chooses random numbers in $\mathbb{Z}_n$ to construct $\{u''_k\}$ and $\{t''_k\}$ for $1 \leq k \leq w$,
 from which it assembles $\mathbb{L}''_b = \{L''_{k,b}\} = \{(Q_2, t''_k, [u''_k]Q_2) = ([y]G, t''_k, [u''_ky]G)\}$.
In this process, the challenger ensures that $[u''_k]G \neq Q_1$ (i.e., $u''_k \neq x$) and $u''_k \neq u_i$ for any $1 \le i \le v$.
Then, it randomly chooses $d \in [1, w]$ and replaces $L''_{d,b}$ with $(Q_2, t''_d, Q_3) = ([y]G, t''_d, [z]G)$. Now, the challenger constructs $\mathbb{L}''_b = \{L_{k,b;1\leq k \leq w}\}$, representing logins from $w$ users, i.e., $\mathbf{u}_w=\{u''_1, u''_2, \cdots, u''_{d-1}, z/y, u''_{d+1}, \cdots, u''_w\}$. (4) When the adversary receives $\mathfrak{L}^m$, $L'_a$, and $\mathbb{L}''_b$, it returns $s$ as the output of $\mathcal{D}^*_r$.
...
while $z/y$ is embedded as $ID_{U''_d}$ in $\mathbb{L}''_b$ to the RP with $ID_{RP_{b}}=[y]G$,
...
% 这里不是if and only if. "if, 就变成了the adversary必胜了；并不是，而是“有显著的概率”"
 Therefore, using $\mathcal{D}^*_r$ to solve the ECDDH problem, the adversary has an $\mathbf{Adv}^*_{A}=|{\rm Pr}^*_1 - {\rm Pr}^*_2|$, where
...
\begin{align*}
&{\rm Pr}^*_1 =  {\rm Pr}(\mathcal{D}^*_r(G, [x]G, [y]G, [xy]G)=1) \\
=&{\rm Pr}(\mathcal{G}_r(\mathfrak{L}^m, L'_a, \mathbb{L}''_b)=1 \; | \; u' \in \mathbf{u}_w) = {\rm Pr}_1 \\
&{\rm Pr}^*_2= {\rm Pr}(\mathcal{D}^*_r(G, [x]G, [y]G, [z]G)=1) \\
=&{\rm Pr}(\mathcal{G}_r(\mathfrak{L}^m, L'_a, \mathbb{L}''_b)=1 \; | \; u' \in \mathbb{Z}_n) = {\rm Pr}_2 \\
&\mathbf{Adv}^*_{A}=|{\rm Pr}^*_1-{\rm Pr}^*_2|=|{\rm Pr}_1-{\rm Pr}_2|={\mathbf{Adv}}_{A}
\end{align*}
...
If the adversary has non-negligible advantage in $\mathcal{G}_r$, then $\mathbf{Adv}^*_{A}={\mathbf{Adv}}_{A}$ is non-negligible regardless of the security parameter $\lambda$. This violates the ECDDH assumption. Therefore, the adversary has no advantage in $\mathcal{G}_r$ and cannot decide whether $L'_a$ is initiated by a user in $\mathbf{u}_w$ or in the universal user set.
Because $RP_b$ is any malicious RP, this proof can be easily extended from $RP_b$ to more malicious RPs.
\hfill $\square$
