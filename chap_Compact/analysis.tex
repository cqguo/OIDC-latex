...
Then, we consider the third adversarial scenario where $c+1$ malicious RPs collude with $v$ malicious users attempting to link login instances of honest users across these malicious RPs. That is, given an arbitrary login instance $\mathcal{L'}$ initiated by an honest user $U'$ to one of the $c+1$ malicious RPs, the colluding adversaries aim to link $\mathcal{L'}$ to $U'$'s other login instances to the remaining $c$ malicious RPs. To show that \usso~prevents such linkage, we will prove in Theorem 7 that the colluding adversaries cannot distinguish the login instances initiated by $U'$ from the ones initiated by any other honest users, requesting to log in to any of the remaining $c$ malicious RPs. As a result, given $\mathcal{L'}$, the adversaries have no clue to link it to one login instance that has been observed by a colluding RP instead of another.
...
Let us define the knowledge that a malicious RP obtains from each login instance. In \usso, an RP receives $t$ and an identity token enclosing $PID_{RP}$ and $PID_U$ in each login instance. Meanwhile, it knows its own identity $ID_{RP}$ and derives the user's account $Acct$. Since $PID_{RP}$ and $PID_U$ can be deduced from $ID_{RP}$ and $Acct$ with $t$, respectively, we ignore the duplicate elements and define an RP's knowledge about a login instance as $L=(ID_{RP}, t, Acct)=(ID_{RP}, t, [ID_{U}]ID_{RP})$. %Note that a login instance $L$ includes the user's account at the RP.
