...
We define $Pr_c$ is the probability that
    the adversary outputs $s=1$ when $ID_U = ID_{U'}$ (i.e., a \emph{correct} identity linkage),
    and $Pr_{\bar{c}}$ is the probability that $s=1$ but $ID_U \neq ID_{U'}$ (i.e., an \emph{incorrect} case).
...
Equation \ref{eq:game-succed} is equal to $Pr_c$
 ...
 while Equation \ref{eq:game-fail} is $Pr_{\bar{c}}$ for it represents the incorrect case of $ID_{U} =y$ and $ID_{U'} = z/x$.
