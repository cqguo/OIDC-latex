...
honest RP as an honest user; and {\bf (B)} the attacker should not be able to authenticate an honest user to an honest RP on behalf of another user. For proof, we demonstrate that both properties are fulfilled for every $\mathcal{U\!W\!S}^{auth}$. 

To prove property {\bf (A)}, we first show that the attacker cannot steal any useful information from any login session of an honest user to an honest RP, which allows him to impersonate the honest user at the honest RP. In particular, we show two properties of $\mathcal{U\!W\!S}^{auth}$, i.e., a network attacker cannot alter encrypted communications over HTTPS between {\bf (A1)} the honest IdP and an honest user (Lemma~\ref{lemma-user-request} in Appendix~\ref{appendix-security}); and {\bf (A2)} the honest IdP and an honest RP, where the former indicates that $PID_{RP}$ in the identity token request of the honest user cannot be intercepted by the attacker and the latter indicates that a valid identity token of the honest user cannot be intercepted (i.e., {\em ID token confidentiality and integrity}). We adopted the general web system properties shown in the full version of \cite{FettKS14} and followed the approaches in \cite{BrowserID,SPRESSO}, which are based on a Dolev-Yao-style model, to prove properties {\bf (A1)} and {\bf (A2)}. A detailed proof is provide in Appendix~\ref{appendix-security}.
