...
\newc
For the analysis of security (w.r.t. authentication), we consider a formal model of \usso\ web systems (denoted as $\mathcal{U\!W\!S}^{Auth}$), which is defined in Appendix \ref{appendix-model}. It contains an IdP, a finite set of RPs, a finite set of users (i.e., browsers), and a network attacker. The attacker subsumes all web attackers and is assumed to be able to  corrupt a group of users and RPs.
...
For the proofs, we demonstrate that both properties (formally defined in Definition 1 in Appendix \ref{appendix-security}) are fulfilled for every $\mathcal{UWS}^{Auth}$.

To prove Property {\bf A}, we first show that the attacker cannot obtain any useful information, which allows him to impersonate an honest user at some honest RP, from any login of an honest user to an honest RP. 
%In particular, we show two properties of $\mathcal{UWS}^{Auth}$, i.e., a network attacker cannot break secure communications over HTTPS between {\bf (C1)} the honest IdP and an honest user (proved in Lemmas 2 and 3 in Appendix \ref{appendix-security}); and {\bf (C2)} the honest IdP and an honest RP (proved in Lemmas 4, 5, and 6 in Appendix \ref{appendix-security}). 
...
\newc
\begin{thm}
\textsc{(User Identification of $TK$)} \emph{$PID_U= [ID_U]PID_{RP}$ in $TK$ uniquely identifies an account at the RP designated by $PID_{RP}$ if and only if it receives the correct $t$ where $PID_{RP} = [t]ID_{RP}$ holds, and this account is uniquely mapped to a user with $ID_U$.}
\label{thm-user-id}
\end{thm}
...
To show \usso\ preventing such \emph{IdP-based login tracing}, we first prove that in $\mathcal{UWS}^{Auth}$, the IdP cannot obtain any information related to the target RP in each login (e.g., $ID_{RP}$, $t$, RP's $EndPoint$) except its \emph{ephemeral} pseudo-identity $PID_{RP}$ (with Lemma in Appendix~\ref{appendix-security}). Then, we prove in Theorem \ref{thm-idp-untraceability} that with these $PID_{RP}$s in the identity token requests from the users, the IdP cannot (\emph{a}) associate multiple logins to the same RP, or (\emph{b}) distinguish a login to one RP from other logins to another RP, because $PID_{RP}$ is \emph{indistinguishable} from any random variables to the IdP. As a result, \usso\ prevents \emph{an honest-but-curious IdP} from tracing a user's login activities.
