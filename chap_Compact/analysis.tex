...
So, we can design a PPT algorithm $\mathcal{D'^*}$, shown in Figure~\ref{fig:dalgorithm}, to solve the ECDDH problem as below. The input of $\mathcal{D'^*}$ is in the form of $(Q_1=[x]G$, $Q_2=[y]G$, $Q_3=[z]G)$ and the output is $s'$ generated by adversary. On receiving the input, the challenger of $\mathcal{G'}$ randomly chooses $u_1, u_2, \cdots, u_a$, $r_1, r_2, \cdots, r_b$ and $t_1, t_2, \cdots, t_{a*b}, t'$ from $(1,n)$. Then challenger calculates $ID_{RP_{1\sim b}}$ as $[r_1]G, [r_2]G, \cdots, [r_b]G$ and $[ID_{U_{1\sim a}}]ID_{RP_{1\sim b}}$ as $[u_1][r_1]G, [u_1][r_2]G, \cdots, [u_a][r_b]G$, while randomly replacing the $[u_*][r_j]G$ with the $[r_j]Q_1$. Let $ID_{RP_{b+1}} = Q_2$ and $[ID_U']ID_{RP_{b+1}}=Q_3$.
There are 
