...
\noindent\textbf{Security.} Malicious users might collude with each other or with malicious RPs, attempting to (\emph{a}) impersonate an honest user to log into an honest RP or (\emph{b}) entice an honest user to log into an honest RP under a malicious user's account.

\noindent\textbf{Privacy against the IdP.}
The honest-but-curious IdP tries to infer the identities of the RPs that a user requests to access. %or link multiple login instances to any RP initiated by a user.

\noindent\textbf{Privacy against RPs.}
Malicious RPs might collude with each other or with malicious users, attempting to link login instances across these RPs that are initiated by a user.
...
Then, we consider the third adversarial scenario where $c+1$ malicious RPs collude with $v$ malicious users attempting to link login instances of honest users across these malicious RPs.
That is, given an arbitrary login instance $\mathcal{L'}$ initiated by an honest user $U'$ to one of the $c+1$ malicious RPs, the colluding adversaries aim to link $\mathcal{L'}$ to $U'$'s other login instances to the remaining $c$ malicious RPs. To show that \usso~prevents such linkage, we will prove in Theorem 7 that the colluding adversaries cannot distinguish the login instances initiated by $U'$ from the ones initiated by any other honest users, requesting to log into any of the remaining $c$ malicious RPs. As a result, given $\mathcal{L'}$, the adversaries have no clue to link it to one login instance that has been observed by a colluding RP instead of another.
%Malicious RPs colluding with malicious users, attempt to link login instances of honest users across these malicious RPs. We prove that UPPRESSO prevents RP-based identity linkage, by showing that,  the colluding adversaries cannot distinguish a login instance by $U'$ to any of other $c$ malicious RP from the login instances by any honest users to the $c$ malicious RPs.

Let us define the knowledge that an RP obtains from each login instance. In \usso, an RP receives a random number $t$ and an identity token enclosing $PID_{RP}$ and $PID_U$ in each login instance. Meanwhile, it knows its own identity $ID_{RP}$ and derives the user's account $Acct$. Since $PID_{RP}$ and $PID_U$ can be deduced from $ID_{RP}$ and $Acct$ with $t$, respectively, we ignore the duplicate elements and define an RP's knowledge about a login instance as $L=(ID_{RP}, t, Acct)=(ID_{RP}, t, [ID_{U}]ID_{RP})$. %Note that a login instance $L$ includes the user's account at the RP.
...
% L_{v,1},&L_{v,2},&\cdots,&L_{v,c}
% \end{matrix}\right\}$
\noindent where $L_{i,j} = (ID_{RP_j}, t_{i,j}, [ID_{U_i}]ID_{RP_j})$ is about the login instance initiated by $U_i$ ($1 \le i \le v$) to $RP_j$ ($1 \le j \le c$). %We denote the login instance initiated by $U'$ to the $c+1$th RP as $L' = (ID_{RP'}, t', [ID_{U'}]ID_{RP'})$.
...
When $c+1$ malicious RPs and $v$ malicious users collude in \usso, given a login instance $\mathcal{L'}$ initiated by an honest user $U'$ to a malicious RP, the adversaries cannot distinguish a login instance by $U'$ from the ones initiated by any other honest users to any of the remaining $c$ malicious RPs.}
...
After $RP'$ colludes with each $RP_j$ ($1 \leq j \leq c$),  we find that, given $L'$ initiated by $U'$ to $RP'$, the adversaries cannot distinguish a login instance by $U'$ to any $RP_j$, from the login instances by any users to $RP_j$ ($1 \leq j \leq c$).
...
If the probability of distinguishing a login instance initiated by a user $U'$ chosen from \{${U_1}, {U_2}, \cdots, {U_v}$\} or randomly from the universal user set is non-negligible,
