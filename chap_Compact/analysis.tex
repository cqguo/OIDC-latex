...
On receiving the input, the challenger of $\mathcal{G}_c$ randomly chooses $r_1, r_2, \cdots, r_m$ from $(1,n)$,
...
Thus,
\begin{align*}
Pr\{\mathcal{D}^*_c(G, [x]G)=x\} = Pr\{s = x\}=\frac{1}{m}Pr_s
\end{align*}
With the security parameter $k$,
    if $Pr_s$ is non-negligible, $Pr\{\mathcal{D}^*_c(G, [x]G)=x\} =\frac{1}{m}Pr_s$ is also non-negligible because $m << 2^k$.
It means $\mathcal{D}^*_c$ solves the ECDLP.
...
For any PPT algorithm $\mathcal{D}$,
...
The prevention against the RP-based identity linkage is proved
    by the proposition:
when $c$ collusive RPs hold the information of login instances by $v$ users,
...
after receiving $\mathfrak{L}$ and $L'$ from a challenger,
    an adversary outputs the result $s = 1$ if it determines that $u'$ is in the set $\{u_1, u_2, \cdots, u_v\}$,
        or $s = 0$ otherwise.
...
The input of $\mathcal{D}^*_r$ is in the form of $(Q_1=[x]G$, $Q_2=[y]G$, $Q_3=[z]G)$.
On receiving the input, the challenger of $\mathcal{G}_r$ randomly chooses
 $\{u_1, u_2, \cdots, u_v\}$, $\{r_1, r_2, \cdots, r_c\}$, $\{t_{1, 1}, t_{1, 2}, \cdots, t_{a, b}\}$, and $t'$ from $(1,n)$.
Then the challenger constructs $\mathfrak{L}$ and $L'$ based on $Q_1$, $Q_2$ and $Q_3$ as below.
...
That is, $x$ is actually inserted into $\mathfrak{L}$ as $u_k$,
and $\mathfrak{L}=\left \{ \begin{matrix}
L_{1,1},&L_{1,2},&\cdots,&L_{1,c}\\
L_{2,1},& L_{2,2},&\cdots,&L_{2,c}\\
\cdots,&\cdots,&\cdots,&\cdots\\
...
According to the construction of $\mathfrak{L}$ and $L'$,
    $z/y$ is actually assigned to $u'$.
So, if $z = xy$, $ID_{U'}$ is in the set \{$ID_{U_1}, ID_{U_2}, \cdots, ID_{U_v}$\}.
Thus,
\begin{align*}
&Pr_1=Pr\{\mathcal{D}^*_r([x]G, [y]G, [xy]G)=1\}\\=&Pr'_1 =  Pr\{\mathcal{G}_r(\mathfrak{L}, L'|ID_{U'} \in \{ID_{U_1}, ID_{U_2}, \cdots, ID_{U_v}\})=1\} \\
&Pr_2=Pr\{\mathcal{D'^*}([x]G, [y]G, [z]G)=1\}\\ =&Pr'_2 =  Pr\{\mathcal{G}_r(\mathfrak{L}, L'|ID_{U'} \in (1, n))=1\}
\end{align*}
...
So the RP-based identity linkage is computationally impossible.
