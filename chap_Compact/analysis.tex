...
\noindent\textbf{AS1: Security.} Malicious users collude with each other or with malicious RPs, attempting to (\emph{a}) impersonate an honest user to log into an honest RP or (\emph{b}) entice an honest user to log into an honest RP under a malicious user's account.

\noindent\textbf{AS2: Privacy against the IdP.}
The honest-but-curious IdP tries to infer the identities of the RPs accessed by a user. %requests to access. %or link multiple logins to any RP initiated by a user.

\noindent\textbf{AS3: Privacy against RPs.}
Malicious RPs collude with each other or with malicious users, attempting to link the logins initiated by the same user across these RPs.
...
Because $G$ is a generator of order $n$, $ID_{RP} = [r]G$ is also a generator on $\mathbb{E}$ of order $n$. According to the \dyu\ model, $t$ is randomly chosen in $\mathbb{Z}_n$ and kept unknown to the IdP. Therefore, $PID_{RP} = [t]ID_{RP}$ is \emph{indistinguishable} from a point $Q$ that is randomly chosen on $\mathbb{E}$ \cite{oprf-proved,voprf-proved}.\footnote{This property has been actually proved in OPRFs \cite{oprf-proved,voprf-proved}, where the OPRF server works similarly to the IdP and learns \emph{nothing} about a client's inputs or outputs of the evaluated pseudorandom function.} \hfill $\square$
...
\vspace{0.75mm}
\noindent\textsc{Proof.} From logins in $\mathbb{L}$ but not in $\mathfrak{L}$, we randomly select a login $L'$ to a malicious $RP_{j'}$, which is from an (unknown) honest user $U'$, and another malicious $RP_{j''}$ and its logins $\mathbb{L}'' = \{L''_k\}$, which are
from (unknown) honest users $\{U''_k\}$, for $j', j'' \in [1,c]$ and $j' \neq j''$. Next, we prove that the colluding adversaries cannot decide if $U'$ is in any subset of $\{U''_k\}$, denoted as $\{{U''_1}, {U''_2}, \cdots, {U''_w}\}$, or not (i.e., $U'$ is randomly selected from the universal user set). This indicates that the colluding adversaries cannot link $L'$ to any subset of $\mathbb{L}''$. 
...
Then,
 it also randomly chooses $j' \in [1, c]$ and $t'$ in $\mathbb{Z}_n$, to construct $L' = ([r_{j'}]G, t', [r_{j'}]Q_1) = ([r_{j'}]G, t', [xr_{j'}]G)$.
...
It replace $L_{i, j''}$ with $(Q_2, t_{i,j''}, [u_i]Q_2) = ([y]G, t_{i,j''}, [u_iy]G)$ for $1 \leq i \leq v$;
...
Because $x$ is not in $\{u''_1, u''_2, \cdots, u''_{d-1}, u''_{d+1}, \cdots, u''_w\}$,
    the adversary outputs $s=1$ if and only if it decides $x = z/y$.
