...
\oldc
\vspace{0.75mm}
\noindent\textsc{Proof.} Let $Q$ be a point on $\mathbb{E}$,
    and finding a number $x \in \mathbb{Z}_n$ satisfying $Q = [x]G$ results in an elliptic curve discrete logarithm problem (ECDLP),
    as $G$ is a generator on $\mathbb{E}$ of order $n$.
...
If the adversary succeeds in $\mathcal{G}_c$ and $[r_a]G$ happens to be replaced with $Q$, we find $\mathcal{D}^*_c$ outputs $s=x$ because $[tr_a]G = [t]Q = [t'r_b]G$. For the adversary, $Q$ is indistinguishable from any other RP identities in the input set, as $[r_j]G$ is randomly replaced by the challenger.
...
${\rm Pr}\{\mathcal{D}^*_c(G, [x]G)=x\}$ is also non-negligible as $m$ is a finite integer and $m \ll 2^k$, when $k$ is sufficiently large.
\oldc
...
To issue an identity token requested for $PID_{RP}$,
    the honest IdP calculates $PID_U = [ID_U]PID_{RP}$ following Equation \ref{equ:PIDU},
and the designed RP calculates $Acct = [t^{-1}]PID_{U} = [ID_U]ID_{RP}$ according to Equation \ref{equ:AccountNotChanged}.
$Acct = [ID_U]ID_{RP}$ is a permanent identifier determined when the RP and the user register,
so $PID_U$ identifies $Acct$ at the RP and $Acct$ is mapped to a user with $ID_U$ at the IdP.



Hence, $PID_U$ is associated only with $ID_U=u$ but not any other user with $ID_{U'}$.
Then, using $t$ and $PID_U$, $Acct$ can be calculated as $Acct = [t^{-1}]PID_{U}$. %following Eq.~\ref{equ:Account}.
According to Equation \ref{equ:AccountNotChanged}, $Acct$ is determined by $ID_U$ and $ID_{RP}$,
 which are permanent identifiers issued by the IdP during registration.
Hence, $Acct$ cannot be manipulated by adversaries and is independent of login instances.
Therefore, in a user's multiple login instances to the same RP, $PID_U$s are always mapped to one and only one $Acct$ at this RP.
...
\vspace{0.75mm}
\noindent{\textsc{Proof.} Identity tokens are generated and signed by the IdP using its private key. $SK$ is protected against any adversaries. Meanwhile, the IdP's public key $PK$ is sent from the IdP to the RPs over HTTPS according to the \dy~model. $PK$ cannot be manipulated by adversaries either. With $PK$, any arbitrary RP could verify the identity tokens it receives.  \hfill $\square$
...
\vspace{0.75mm}
\noindent\textsc{Proof.}
A valid identity token can only be generated by the IdP and sent to the requesting user (i.e., its IdP script). \oldc~The IdP script forwards the token to the RP script, which should be downloaded only from $Enpt_{RP}$ specified in the valid certificate of the RP. This RP should be designated by $PID_{RP}$ and its certificate should be verified with $PK$. According to the \dy~model, all the communications between the IdP, RPs, and users are protected by HTTPS and two scripts communicate with each other using dedicated \verb+postMessage+ HTML5 channels, which ensures that the identity tokens are not leaked to or eavesdropped by any adversaries. \hfill $\square$
...
\vspace{1mm}
\vspace{1mm}
Meanwhile, $PID_U$ can only be calculated by the IdP and the user, since no one else knows or could intercept $u$ according to the \dy~model. \oldc
...
\vspace{1mm}
\noindent{\textsc{Theorem 5. (Privacy against the IdP)}} {\em Consider $PID_{RP} = [t]ID_{RP}$, where $t$ is random in $\mathbb{Z}_n$ and unknown to the IdP. The pseudo-identities of RPs in the identity-token requests from honest users are indistinguishable to the IdP.}
