...
Finally, $t$ is proved unaccessible to the IdP by this model,
 which is necessary for user privacy (see Section \ref{sec-:analysis} for details).
...
The information accessible to the IdP and derived from the RP's identity,
    is only $PID_{RP}$ in identity-token requests, where $PID_{RP} = [t]ID_{RP}$ is calculated by the user and $t$ is kept secret to the IdP.
...
Because (\emph{a}) $ID_{RP} = [r]G$ is also a generator of order $n$,
    ...
    and (\emph{b}) $t$ is a random number from $\mathbb{Z}_n$ and unknown to the IdP,
 from the IdP's view,
 $PID_{RP}$
...
\vspace{1mm}
\noindent\textbf{Privacy against colluding RPs.}~~After
