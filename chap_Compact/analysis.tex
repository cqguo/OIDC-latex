...
As a result, even if $RP_j$ and $RP'$ collude, they are not told if some of the logins are initiated by the same malicious user. According to Lemma 2, given $L'$ and ``any subset'' of $\mathbb{L}$, $RP'$ and $RP_j$ cannot decide if $U' \in$ ``any subset'' of $\mathcal{U}$ or not.
...
Consider any any subset of $\mathbb{L}^{\Sigma}$.
     ...
     Consequently, they cannot determine if $U' \in$ \{``the corresponding subset'' of $\mathcal{U}^{\Sigma}$ $\setminus$ ``the corresponding subset'' of $\mathcal{U}_M$\}, combined with the conclusions according to Lemma 2.
