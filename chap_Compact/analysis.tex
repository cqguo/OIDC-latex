...
However, $([r_c]G, t', [u'r_c]G)$ is indistinguishable from $([r_k]G, t_{u'k}, [u'r_k]G)$ based on Lemma 2. So, $RP_c$ cannot tell if the user is in the set $\mathbb{U}_c$ nor link her to any other logins. 

Besides, when a malicious user (denoted by $u''$) logs in to $RP_c$ while not revealing his identity, $RP_c$ cannot tell if he is in the malicious user group $\{u_1,...,u_v\}$, according to Lemma 3. This indicates that the additional knowledge $\mathbb{L}_j$s from the colluding users does not facilitate RP-based linkage.
