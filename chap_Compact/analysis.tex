...
\item \textbf{Confidentiality}~~An identity token
    is accessible to only
                the authenticated user and the target RP, in addition to the IdP.
...
%\vspace{1mm}
Finally, we analyzed the security properties of UPPRESSO, %especially confidentiality and integrity,
...
Based on this model
    we trace the lifecycle of an identity token in UPPRESSO,
        starting when it is generated and ending when accepted by an RP,
...
as we locate the generation of a token, and trace all places
    where $PID_U$, $PID_{RP}$ and other parameters in the token are generated, calculated and transmitted,
     to ensure no adversary able to retrieve or manipulate them.
Moreover, $t$ is proved unaccessible to the IdP for privacy.
