...
In this section, we formally prove the security and privacy guarantees provided by \usso.
...
\newc
For the security analysis (w.r.t. authentication), we consider the formal model of \usso\ web systems (denoted as $\mathcal{U\!W\!S}^{auth}$), which is defined in Appendix~\ref{appendix-model}. It contains an IdP, a finite set of RPs, a finite set of users (i.e., browsers), and a network attacker. The attacker subsumes all web attackers, who can corrupt user browsers and RPs. 

As described in the threat model in Section \ref{subsec:threatmodel}, a malicious user, by colluding with malicious RPs, attempts to (\emph{a}) impersonate an honest user to log into an honest RP (impersonation); (\emph{b}) tamper with the login session of an honest user to an honest RP such that the honest user is authenticated as another user (identity injection). 
...
honest RP as an honest user; and {\bf (B)} the attacker should not be able to authenticate an honest user to an honest RP on behalf of another user. For proof, we demonstrate that both properties are fulfilled for every $\mathcal{UWS}^{auth}$. 

To prove property {\bf (A)}, we first show that the attacker cannot steal any useful information from any login session of an honest user to an honest RP, which allows him to impersonate the honest user at the honest RP. In particular, we show two properties of $\mathcal{U\!W\!S}^{auth}$, i.e., a network attacker cannot alter encrypted communications over HTTPS between {\bf (A1)} the honest IdP and an honest user (proved in Lemmas 2 and 3 in Appendix~\ref{appendix-security}); and {\bf (A2)} the honest IdP and an honest RP (proved in Lemmas 4-6 in Appendix~\ref{appendix-security}). The former indicates that $PID_{RP}$ in the identity token request of the honest user cannot be known or altered by the attacker and the latter indicates that a valid identity token of the honest user cannot be known or altered by the attacker (i.e., {\em ID token confidentiality and integrity}). We adopted the general web system properties shown in the full version of \cite{FettKS14} and followed the approaches in \cite{BrowserID,SPRESSO}, which are based on a Dolev-Yao-style model, to prove properties {\bf (A1)} and {\bf (A2)}. A detailed proof is provide in Appendix~\ref{appendix-security}.

Meanwhile, we prove that the property {\bf (A)} is still fulfilled in two adversarial scenarios. First, when a malicious user initiates a login session to an honest $RP$, where he selects a random $t$ to share with $RP$ and then manipulates $PID_{RP}$ to be $[t']ID_{RP}$ instead of $[t]ID_{RP}$ when requesting for an identity token, we prove in Theorem 3 that by showing the identity token with a manipulated $PID_{RP}$ to the honest $RP$, the malicious user cannot be recognized under the account of any existing honest users at that RP (i.e., {\em User Identification}). 

Secondly, when a malicious $RP$ shares with a malicious user $U'$ the identity token $TK$ that it received from an honest user $U$. With $TK$, if the malicious $RP$ is able to find a $t'$ satisfying $[t']ID_{RP'}=[t]ID_{RP}$ for any honest $RP'$, it could share $TK$ and $t'$ with the malicious user, enabling him to log into $RP'$ on behalf of $U$. We prove in Lemma 1 and Theorem 2 that a malicious RP knowing $t$ and $[t]ID_{RP}$ from $U$'s login session cannot find such a $t'$, and $PID_{RP}$ in $TK$ is associated with $t$ for the login session to the designated $RP$ (i.e., {\em RP Designation}).
...
To prove property {\bf (B)}, we first prove in Theorem 3: when an honest user logs in to an honest RP, the identity token $TK$ that binds $PID_U$ and $PID_{RP}$ uniquely identifies a user account for this user at the designated RP (i.e., {\em User Identification}). Therefore, an honest user will not be authenticated by an honest RP as any other users in the system.

Meanwhile, we show that the attacker cannot manipulate any login session of an honest user to an honest RP in a way that the honest user is mistakenly authenticated as another user. 
...
Then, we show that the attacker cannot manipulate $PID_{RP}$ in the identity token request of the honest user based on the property {\bf (A1)}, nor the identity token $TK$ issued by the IdP to the honest user and the target honest RP based on the property {\bf (A2)}. \hfill $\square$
...
\noindent\textbf{\textsc{Proof.}}
To the RP designated (Theorem 2),
...
\subsection{RP Privacy}
\label{subsec:RP-privacy}

In the third adversarial scenario, malicious RPs collude with malicious users, attempting to link the logins initiated by an honest user but visiting different colluding RPs. 
...
Therefore, the colluding adversaries share all logins to malicious RPs, and attempt to leverage the logins initiated by malicious users to link the other logins by honest users.
