...
MISO \cite{miso} decouples the calculation of PPIDs from an IdP
    ...
    based on the user's identity, the visited RP's identity and a secret key,
    after the mixer receives an identity token from the IdP.
MISO prevents RP-based identity linkag for the RPs receives only PPIDs,
...
It protects a user's online profile against even collusive attacks by the IdP and RPs,
    but it requires the extra mixer server to be fully trusted.
