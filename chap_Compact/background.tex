...
Finally, the RP verifies the received identity token and
 allows the user to login as the identity enclosed in this token.
...
This feature of commonly-used SSO protocols brings advantages as below.
It enables the IdP to authenticate a user by any appropriate means (e.g., password, two-factor authentication, one-time password, etc.).
If a credential was lost or compromised,
    the user only needs renew it at the IdP;
        however, if a user needs to also prove something to the RP (i.e., some authentication steps are involved),
                the user has to notify each RP in this case.
...
When a user logins to different RPs, the RPs could still extract the identical user identities
    from different these tokens and link these login instances.
Meanwhile,
    an RP of SPRESSO creates a one-time but verifiable pseudo-identity for itself in each login instance.
...
Similarly, the collusive RPs could link a user's multiple login instances based on his permanent user identity in these tokens.
