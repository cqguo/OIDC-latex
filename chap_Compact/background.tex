...
\footnotesize
  ...
  Opaak & $\LEFTcircle$$^4$ & $\Circle$ & $\Circle$ & $\CIRCLE$ & $\CIRCLE$ & $\CIRCLE$ \\ \hline
...
4. Opaak implements two exclusive options: (\emph{a}) login with pseudonyms linkable within an RP but unlinkable across multiple RPs and (\emph{b}) pseudonyms unlinkability for any two actions.\\
...
And then the PRIMA user provides selective IdP-confirmed attributes to any RP using his signing key \cite{Oblivion}, cooperatively with the IdP.
...
The Idemix anonymous credential system \cite{idemix}
 is integrated in Hyperledger \cite{hyperledge-idemix} to implement completely-unlinkable pseudonyms
...
A ``pure'' SSO protocol allows the user to authenticate himself to the IdP a \emph{single} time,
    but need no keep any permanent secret to login to an RP.
On the other hand,
...
Some identity federation solutions \cite{prima,ELPASSO,UnlimitID,idemix,PseudoID} protect user privacy against collusive attacks by the IdP and RPs,
