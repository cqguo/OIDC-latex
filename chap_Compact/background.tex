...
We discuss existing privacy-preserving identity management solutions and other related works.
...
Section \ref{sec:discussion} discusses the extensions to support the authorization code flow.
...
The following features are supported by widely-used popular SSO solutions \cite{NIST2017draft,OpenIDConnect,rfc6749,SAML,SAMLIdentifier}.
...
On the contrary, in anonymous SSO systems \cite{WangWS13,HanCSTW18,HanCSTWW20}
        the RP only verifies whether he is a legitimate user authenticated by the IdP
            and receives no information to distinguish the users.
...
So a user holds only the credential to authenticate himself to the IdP,
    and the burden is mitigated.
...
On the other hand, if a user proves some non-ephemeral secret to the RP and this secret is valid across multiple login instances
    (i.e., some authentication steps are actually involved),
                the user has to notify each RP if this secret was lost or leaked. %during its validity period. %(or even the user logins from another computer).
...
These attributes are maintained by users at the IdP;
    for example,
        when Facebook provides social networking services,
         it also runs as an IdP with identity tokens enclosing user identities and also various attributes.
...
Thus,
    no distinctive attributes such as telephone number, Email, etc.,
...
3. Blindly-signed user attributes can be selectively provided using zero-knowledge proofs,
    but not implemented in PseudoID.\\
4. Opaak implements exclusive pseudonym options: (\emph{a}) linkable within an RP but unlinkable across multiple RPs and (\emph{b}) unlinkability for any two actions.\\
5. In the original design of Idemix \cite{idemix}, every user logins to an RP as a permanent account.\\
6. A U-Prove token may contain some attributes \emph{invisible} to the IdP, in addition the ones confirmed by the IdP and selectively disclosed by the user.}
\end{table*}
...
In BrowserID \cite{BrowserID} (and its prototypes known as Firefox Accounts \cite{FirefoxAccount} and Mozilla Persona \cite{persona}),
 ...
 so that the user utilizes the corresponding private key to sign a ``subsidiary'' token
...
In PRIMA \cite{prima} the IdP signs an authentication credential binding a verification key and a set of user attributes, where the key is considered as the user's unique identity.
Then the PRIMA user selectively provides IdP-confirmed attributes to some RP using his signing key \cite{Oblivion}, cooperatively with the IdP.
In SPRESSO \cite{SPRESSO} an RP assigns a one-time but verifiable pseudo-identity to itself in each login instance.
...
Moreover, collusive privacy attacks by the IdP and RPs are also prevented,
    because even the IdP and the token service cannot link two blindly-signed tokens.
...
Anonymous credentials \cite{anon-credential-2001,anon-credential} can be utilized in flexible ways.
...
After a user retrieves a U-Prove token \cite{uprov,uprove-conference} from the IdP,
    it enables the user to authenticate himself and disclose attributes to an RP,
...
Commonly-used popular SSO protocols \cite{OpenIDConnect,rfc6749,SAML,SAMLIdentifier} allow a user to login to RPs,
    after being authenticated by the IdP,
        without by himself holding any permanent secret verified by RPs
        or maintain the accounts at these RPs.
%Such designs simplify the user burdens greatly.
Privacy-preserving SSO \cite{BrowserID,SPRESSO,NIST2017draft} and UPPRESSO
            keep this convenience for users,
                while some privacy threats are prevented.
%
On the other hand,
    identity federation enables a user registered at the IdP to be accepted by other parties,
        ...
        while more operations by the user may be involved.
Privacy-preserving identity federation \cite{ELPASSO,UnlimitID,hyperledge-idemix,PseudoID,Opaak}
    ...
    but requires a user to (\emph{a}) hold long-term secrets
            in addition to the authentication credentials for the IdP
                and (\emph{b}) manage the accounts at different RPs.
There are actually some authentication steps between the user and RPs,
so
    a user needs to notify the RPs if the secret and credentials (or tokens) were lost or leaked.
