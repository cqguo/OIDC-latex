...
Then, we discuss existing privacy-preserving SSO solutions and also other related works.
...
BrowserID \cite{BrowserID} and SPRESSO \cite{SPRESSO} are proposed to defend SSO services against the IdP-based login tracing,
    but both solutions are still vulnerable to the threat of RP-based identity linkage.
In BrowserID (and its prototypes known as Mozilla Persona \cite{persona} and Firefox Accounts \cite{FirefoxAccount}),
 ...
 so that the user uses the corresponding private key to sign a subsidiary identity token
    to bind his identity with the RP identity and send both tokens to the RP.
When a user logins different RPs, the RPs could extract the identical user identities
...
Permanent PPIDs are assigned in identity tokens based on the visited RP \cite{NIST2017draft,OpenIDConnect, SAMLIdentifier},
    but the IdP receives (\emph{a}) nothing about the visited RP in BrowserID
     or (\emph{b}) a one-time pseudo-identity of the RP in SPRESSO.
