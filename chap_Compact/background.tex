...
As shown in Figure \ref{fig:OpenID}, when a user initiates a login request to an RP, the RP constructs an identity-token request with its own identity and the scope of requested user attributes.
This request is redirected to a trusted IdP.
Once the IdP authenticates the user,
 it issues an identity token that encloses the identities (or pseudo-identities) of the user and the visited RP, the requested user attributes, a validity period, etc. The user then forwards the identity token to the RP's endpoint. The RP verifies the token and allows the holder to login as the enclosed (pseudo-)identity.
...
Privacy-preserving SSO is expected to offer the desired features listed in Section \ref{subsec:OIDC},
...
On the other hand,
  identity federation enables a user registered at a trusted IdP to be accepted by other parties, potentially with different accounts,
   but \emph{additional user operations for the authentication steps between the user and RPs} may be involved,\footnote{SSO protocols \cite{OpenIDConnect,rfc6749, SAML, SAMLIdentifier} allow users to login to an RP without maintaining an account at the RP or holding a permanent secret to be verified by the RP.
Although the term ``single sign-on (SSO)'' was used in several schemes \cite{PseudoID, Opaak, ELPASSO, WangWS13, HanCSTW18, HanCSTWW20},
    they are very different from the widely-used SSO protocols,
...
So in this paper we call them \emph{identity federation} to emphasize this difference.}
    while more privacy threats are prevented in privacy-preserving identity federation.
However, as shown in
    Table \ref{tbl:comparison-protocol}, none of the existing solutions satisfies all expectations.
...
issues a ``user certificate'' token that binds a user identity to an \emph{ephemeral} public key.
The user then signs a subsidiary ``identity assertion'' token that binds the target RP's identity and sends both tokens to the RP.
In SPRESSO \cite{SPRESSO} the RP creates a one-time pseudo-identity in each login instance, which is enclosed in identity tokens along with the user's unique identity.
...
The users prove ownership of the anonymous credentials using the secrets,
 and disclose IdP-confirmed attributes in the credentials in most approaches except Opaak.
...
These approaches prevent both IdP-based tracing and RP-based identity linkage,
 for (\emph{a}) the RP's identity is not enclosed in the anonymous credentials and
...
\noindent\textbf{Anonymous identity federation.} Such schemes protect even more user privacy:
...
Some special features (e.g., designated verifier \cite{HanCSTW18} and proxy re-verification \cite{HanCSTWW20}) are also designed in these schemes.
...
These completely-anonymous services only work for special scenarios,
