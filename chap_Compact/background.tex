...
In the widely-used SSO protocols \cite{OpenIDConnect,rfc6749,SAML},
where RPs verify only the identity tokens issued by the IdP,
    the authentication between a user and an IdP is usually conducted \emph{independently}
    of the steps dealing with identity tokens.
...
An IdP usually provides user attributes in identity tokens \cite{OpenIDConnect,rfc6749}, in addition to user (pseudo-)identities.
...
The IdP obtains a user's authorization before enclosing attributes in a token, or provides only attributes pre-selected by the user.
...
Table \ref{tbl:comparison-protocol} summarizes existing privacy-preserving solutions for SSO and identity federation.
...
That is, there are actually some authentication steps between a user and RPs (called asynchronous authentication \cite{ELPASSO} sometimes).
...
In SPRESSO \cite{SPRESSO} an RP creates a verifiable one-time pseudo-identity for itself in each login instance,
 which is enclosed in the identity token issued by an IdP. %and the user identity.
...
In EL PASSO \cite{ELPASSO}, after authenticating a user,
    an IdP signs an anonymous credential \cite{anon-credential} binding a secret,
...
Although one credential is proved to multiple RPs,
        \emph{user-managed pseudonyms} and \emph{anonymous credentials} prevent the RPs, even when collusive with the IdP, from linking a user across the RPs.
...
Opaak \cite{Opaak} designs IdP-signed anonymous credentials %in mobile phones as pseudonym tokens,
        with different pseudonym options,
    all of which bind a user's secret key.
...
With a U-Prove token \cite{uprov,uprove-conference} from an IdP,
    a user is enabled to authenticate himself to an RP and disclose selective attributes.
