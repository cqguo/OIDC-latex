...
SSO protocols \cite{OpenIDConnect,rfc6749, SAML, SAMLIdentifier} allow users to login to an RP \emph{without maintaining an account at the RP or holding a permanent secret to be verified by the RP}.
...
On the other hand,
  identity federation enables a user registered with a trusted IdP to be accepted by other parties, potentially with different accounts, but more user operations are involved than those in SSO.
As shown in
    Table \ref{tbl:comparison-protocol}, none of the existing solutions satisfies all the requirements.



\noindent\textbf{Privacy-preserving SSO.} While supporting these features, existing privacy-preserving SSO approaches \cite{BrowserID, SPRESSO, NIST2017draft} prevent either IdP-based login tracing or RP-based identity linkage, but not both. %and UPPRESSO prevents both of them.
