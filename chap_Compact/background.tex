...
In contrast, privacy-preserving identity federation offers more privacy protections but introduces extra complexity in the authentication process.
...
Such schemes offer an higher level of user privacy protections
 as an IdP issues credentials for a user to access RPs with pseudonyms that cannot be linked for any two access actions.
\oldc
Anonymous identity federation is formalized \cite{WangWS13},
 and solutions \cite{WangWS13, HanCSTW18, HanCSTWW20} were proposed using cryptographic primitives such as group signature and zero-knowledge proof. Special features such as designated verification \cite{HanCSTW18} and proxy re-verification \cite{HanCSTWW20} are also supported. %in these schemes.
