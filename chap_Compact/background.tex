...
Table \ref{tbl:comparison-protocol} lists privacy-preserving solutions of SSO and also federated identity management.
...
Existing privacy-preserving SSO \cite{BrowserID,SPRESSO,NIST2017draft} prevents the IdP-based login tracing or the RP-based identity linkage,
    but UPPRESSO prevents both of them while keeping this user convenience.
...
Identity federation enables a user registered at the IdP to be accepted by other parties,
        ...
        while more user operations may be involved.
...
That is, there are actually some authentication steps between the user and RPs (called asynchronous authentication \cite{ELPASSO} sometimes).
...
PPID-based approaches cannot prevent the IdP-based login tracing,
 for in the generation of identity tokens the IdP needs to know which RP is visited.
...
In BrowserID \cite{BrowserID} (and its prototypes known as Firefox Accounts \cite{FirefoxAccount} and Mozilla Persona \cite{persona}),
 ...
 so the user utilizes the corresponding private key to sign a ``subsidiary'' token (called identity assertion)
...
Collusive privacy attacks by the IdP and RPs are also prevented,
