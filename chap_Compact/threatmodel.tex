...
The honest IdP follows the protocols to process the requests from users and RPs,
...
For example, IdP ensures the uniqueness of $ID_{RP}$ and $ID_{U}$ when an RP or a user registers,
    and calculates  the pseudo-identifier as the UPPRESSO protocol specifies.

However, the curious IdP tries to break the user's privacy without violating the protocol.
...
The adversaries could control a set of users, by stealing the users' credentials~\cite{WangZWYH16, SunCL12}
...
That is, they  attempt to impersonate  an uncompromised user at some correct RP,
 or trick a victim user to log into a correct RP under the identity of a compromised user.
...
The adversary could compromise a set of RPs,
...
For example,
    collusive users and RPs may conduct impersonation or identity injection attacks,
...
We assume that the user agent deployed at an honest user is correctly implemented,
    and transmits messages to the destination correctly.
...
UPPRESSO considers the RP-based identity linkage based on the user identifiers at RPs,
    so other RP-based identity linkages based on the distinctive user attributes at RPs (e.g., telephone number, address and driver license) are out of the scope of this paper.
