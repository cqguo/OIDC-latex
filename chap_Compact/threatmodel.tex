...
First of all,
    we assume that a user never authorizes the IdP to enclose any distinctive attributes in identity tokens,
        while distinctive attributes are identifiable information such as telephone number, Email, driver license, etc.
The user does not register distinctive attributes at the RP, either.
Therefore, the privacy leakage due to user re-identification by distinctive attributes is considered out of the scope of this work.

We focus on the privacy attacks enabled by the design of SSO protocols,
    but not network attacks such as the traffic analysis that traces a user's login activities at different RPs from network packets.
Such network attacks shall prevented by other existing defenses.
The software stack deployed at honest entities is correctly implemented,
     so that it transmits messages to the receivers as expected.

Finally, 
the user agent deployed at honest users is correctly implemented
     so that it transmits messages to the receivers as expected.
We also assume TLS is adopted to secure the communications between honest entities,
 and the cryptographic primitives are correctly implemented.
