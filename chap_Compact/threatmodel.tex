...
We do not consider the collusion of the IdP and RPs in this paper.
    In fact, if the IdP could collude with an RP,
        a user would finish a login instance completely with collusive entities 
        and it is rather impossible to prevent the IdP-based login tracing.
...
For example, an adversary could first pretend to be an RP and allure victim users to forward an identity token to him.
    With this identity token, it could try to impersonate the victim user and login to some honest RP.
