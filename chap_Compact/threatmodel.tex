...
We do not consider the collusion of the IdP and RPs.
    If the IdP could collude with some RPs,
        ...
        and it is rather difficult to prevent the IdP-based login tracing across these RPs
            while keeping the desirable features of SSO.
Note that we aim to design a solution for SSO users without any non-ephemeral secret or credential verified by the RPs.
...
We focus on the privacy threats introduced by the design of SSO protocols,
    but not network attacks such as the traffic analysis that track a user's activities from network packets.
