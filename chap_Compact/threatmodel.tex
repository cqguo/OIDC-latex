...
In UPPRESSO, we study the RP-based identity linkage caused by a same user identifier used across different RPs. While the RPs may be able to re-identify a user from some distinctive user attributes, such as telephone numbers, addresses, or driver licenses, we consider it out of the scope of this paper. Also, we focus on IdP-based login tracing attacks that are enabled by SSO protocols, but do not consider other network attacks such as traffic analysis that trace a user's logins at different RPs.
