...
Then,
    we design identity-transformation functions satisfying the requirements,
...
We do not consider the collusion of the IdP and RPs.
    If the IdP could collude with some RPs,
        ...
        and it is rather difficult to prevent the IdP-based login tracing across these RPs
            while keeping the features of SSO as many as possible.
Note that we aim to design schemes for a user without any permeant secret verified by the RPs.
...
A user never authorizes the IdP to enclose any \emph{distinctive attributes} in identity tokens,
        where distinctive attributes are identifiable information such as telephone number, Email, etc.
         %driver license, 
...
We focus on the privacy threats introduced by the design of SSO protocols,
    but not network attacks such as the traffic analysis that trace a user's login activities from network packets.
