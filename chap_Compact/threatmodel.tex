...
The compatibility with OIDC is also discussed,
    which helps the adoption and deployment of UPPRESSO.
...
They want to impersonate a victim user at honest RPs, or allure a victim user to login to an honest RP under the adversary's account.
...
For example, an adversary could first pretend to be an honest RP and allure victim users to forward an identity token to it.
    With the valid identity token, it could impersonate the victim user and login to the honest RP.
...
The user does not register distinctive attributes at any RP, either.
