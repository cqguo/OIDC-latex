...
Then,
    we design three identity-transformation functions satisfying with the privacy requirements,
    and present the detailed protocols.
The compatibility with OIDC is also discussed,
    which will helps the adoption and deployment of UPPRESSO.
...
%However,
We do not consider the collusion of the IdP and RPs in this paper.
    If the IdP could collude with an RP,
        a user would finish a login instance completely with collusive entities 
        and it is rather impossible to prevent the IdP-based login tracing in this case.
...
For example, an adversary could first pretend to be an honest RP and allure victim users to forward an identity token to him.
